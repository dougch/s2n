...
#define ERR_ENTRIES( ERR_ENTRY )                                                                                      \
    ...
    ERR_ENTRY( S2N_ERR_ECDSA_UNSUPPORTED_CURVE,                                                                       \
               "Unsupported EC curve was presented during an ECDSA SignatureScheme handshake" )                       \
    ERR_ENTRY( S2N_ERR_ECDHE_SERIALIZING, "Error serializing ECDHE public" )                                          \
    ERR_ENTRY( S2N_ERR_KEM_UNSUPPORTED_PARAMS,                                                                        \
               "Unsupported KEM params was presented during a handshake that uses a KEM" )                            \
    ...
    ERR_ENTRY( S2N_ERR_MAX_FRAG_LEN_MISMATCH,                                                                         \
               "Negotiated Maximum Fragmentation Length from server does not match the requested length by client" )  \
    ERR_ENTRY( S2N_ERR_PROTOCOL_VERSION_UNSUPPORTED,                                                                  \
               "TLS protocol version is not supported by selected cipher suite" )                                     \
    ...
    ERR_ENTRY( S2N_ERR_TOO_MANY_SIGNATURE_SCHEMES,                                                                    \
               "Max supported length of SignatureAlgorithms/SignatureSchemes list is 32" )                            \
    ...
    ERR_ENTRY( S2N_ERR_MULTIPLE_DEFAULT_CERTIFICATES_PER_AUTH_TYPE,                                                   \
               "setting multiple default certificates per auth type is not allowed" )                                 \
    ...
    ERR_ENTRY( S2N_ERR_CLIENT_AUTH_NOT_SUPPORTED_IN_SESSION_RESUMPTION_MODE,                                          \
               "Client Auth is not supported in session resumption mode" )                                            \
    ERR_ENTRY( S2N_ERR_INVALID_TICKET_KEY_LENGTH, "Session ticket key length cannot be zero" )                        \
    ERR_ENTRY( S2N_ERR_INVALID_TICKET_KEY_NAME_OR_NAME_LENGTH,                                                        \
               "Session ticket key name should be unique and the name length cannot be zero" )                        \
    ...
    ERR_ENTRY( S2N_ERR_NO_TICKET_ENCRYPT_DECRYPT_KEY,                                                                 \
               "No key in encrypt-decrypt state is available to encrypt session ticket" )                             \
    ERR_ENTRY( S2N_ERR_ENCRYPT_DECRYPT_KEY_SELECTION_FAILED,                                                          \
               "Failed to select a key from keys in encrypt-decrypt state" )                                          \
    ERR_ENTRY( S2N_ERR_KEY_USED_IN_SESSION_TICKET_NOT_FOUND,                                                          \
               "Key used in already assigned session ticket not found for decryption" )                               \
...
const char *s2n_strerror( int error, const char *lang )
{
    ...
    if ( strcasecmp( lang, "EN" ) ) { return no_such_language; }
...
const char *s2n_strerror_debug( int error, const char *lang )
{
    ...
    if ( error == S2N_ERR_OK ) { return s2n_strerror( error, lang ); }
...
{
    if ( !s_s2n_stack_traces_enabled ) { return S2N_SUCCESS; }
