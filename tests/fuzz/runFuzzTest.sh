...
if [[ $TEST_NAME == *_negative_test ]]; then
...
if [ -e $TEST_SPECIFIC_OVERRIDES ]; then
...
FIPS_TEST_MSG=""
if [ -n "${S2N_TEST_IN_FIPS_MODE}" ]; then
...
if [ ! -d "${CORPUS_DIR}/${TEST_NAME}" ]; then
...
printf "Starting corpus file count for test %s: %s\d" ${TEST_NAME} $(ls -l ${CORPUS_DIR}/${TEST_NAME}/* | wc -l)
...
printf "Final corpus files count for test %s: %s\d" ${TEST_NAME} $(ls -l ${CORPUS_DIR}/${TEST_NAME}/* | wc -l)
printf "Backing up corpus files to EFS"
rsync -aq "$TEMP_CORPUS_DIR/" "$CORPUS_DIR/${TEST_NAME}"
...
if [[ "$FUZZ_COVERAGE" == "true" ]]; then
    ...
    if [[ ! -z "$TARGET_FUNCS" ]]; then
        for TARGET in ${TARGET_FUNCS}; do
