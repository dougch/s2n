...
static int set_x509_cns(X509 *x509_cert, const char **cns, size_t num_cns)
{
    ...
    if (!x509_name) { return -1; }

    for (int i = 0; i < num_cns; i++) {
        X509_NAME_add_entry_by_NID(x509_name, NID_commonName, MBSTRING_ASC, ( unsigned char * )( uintptr_t )cns[ i ],
                                   -1, -1, 1);
    }
...
static struct s2n_cert_chain_and_key *create_cert(const char **names, int num_names)
{
    ...
    if (!x509_cert || !cert) { goto cert_cleanup; }
    ...
    if (is_san) {
        if (set_x509_sans(x509_cert, names, num_names) < 0) { goto cert_cleanup; }
        if (s2n_cert_chain_and_key_load_sans(cert, x509_cert) < 0) { goto cert_cleanup; }
    } else {
        if (set_x509_cns(x509_cert, names, num_names) < 0) { goto cert_cleanup; }
        if (s2n_cert_chain_and_key_load_cns(cert, x509_cert) < 0) { goto cert_cleanup; }
    }
    X509_free(x509_cert);
    x509_cert = NULL;
...
int s2n_fuzz_test(const uint8_t *buf, size_t len)
{
    ...
    if (!config || !conn || len == 0) { goto cleanup; }
    ...
    if (num_certs == 0) { goto cleanup; }

    /* Add all of the certs created by fuzz input to store. */
    for (int i = 0; i < num_certs; i++) {
        if (s2n_config_add_cert_chain_and_key_to_store(config, certs[ i ]) < 0) { goto cleanup; }
    }
