...
int RSA_verify( int dtype, const unsigned char *m, unsigned int m_len, const unsigned char *sigbuf, unsigned int siglen,
                RSA *rsa )
{
...
int s2n_rsa_server_key_recv( struct s2n_connection *conn )
{
    ...
    orig_s2n_rsa_server_key_recv =
        ( orig_s2n_rsa_server_key_recv_func_type )dlsym( RTLD_NEXT, "s2n_rsa_server_key_recv" );
