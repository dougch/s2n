...
static int check_client_server_agreed_kem(const uint8_t iana_value[S2N_TLS_CIPHER_SUITE_LEN], uint8_t *client_kem_ids,
                                          const uint8_t num_client_kems, const struct s2n_kem *server_kem_pref_list[],
                                          const uint8_t num_server_supported_kems, kem_extension_size expected_kem_id)
{
    S2N_ERROR_IF(s2n_is_in_fips_mode(), S2N_ERR_PQ_KEMS_DISALLOWED_IN_FIPS);
    ...
    GUARD(s2n_choose_kem_with_peer_pref_list(iana_value, &client_kem_blob, server_kem_pref_list,
                                             num_server_supported_kems, &negotiated_kem));
...
int main(int argc, char **argv)
{
    ...
    {/* The order of the client kem list should always be ignored; the server chooses based on the
     ...
     {uint8_t client_kems[] = {/* BIKE1_L1_R1 */
                               0x00, 0x01,
                               /* BIKE1_L1_R2 */
                               0x00, 0x0d,
                               /* SIKE_P503_R1 */
    ...
    EXPECT_SUCCESS(
        check_client_server_agreed_kem(bike_iana, client_kems, 4, pq_kems_r1, 2, TLS_PQ_KEM_EXTENSION_ID_BIKE1_L1_R1));
    EXPECT_SUCCESS(check_client_server_agreed_kem(bike_iana, client_kems, 4, pq_kems_r2r1, 4,
                                                  TLS_PQ_KEM_EXTENSION_ID_BIKE1_L1_R2));
    EXPECT_SUCCESS(
        check_client_server_agreed_kem(sike_iana, client_kems, 4, pq_kems_r1, 2, TLS_PQ_KEM_EXTENSION_ID_SIKE_P503_R1));
    EXPECT_SUCCESS(check_client_server_agreed_kem(sike_iana, client_kems, 4, pq_kems_r2r1, 4,
                                                  TLS_PQ_KEM_EXTENSION_ID_SIKE_P434_R2));
}
{
    uint8_t client_kems[] = {/* SIKE_P503_R1 */
                             0x00, 0x0a,
                             /* BIKE1_L1_R1 */
                             0x00, 0x01,
                             /* SIKE_P434_R2 */
...
}
{
    ...
    EXPECT_SUCCESS(
        check_client_server_agreed_kem(bike_iana, client_kems, 2, pq_kems_r1, 2, TLS_PQ_KEM_EXTENSION_ID_BIKE1_L1_R1));
    EXPECT_SUCCESS(check_client_server_agreed_kem(bike_iana, client_kems, 2, pq_kems_r2r1, 4,
                                                  TLS_PQ_KEM_EXTENSION_ID_BIKE1_L1_R1));
    EXPECT_SUCCESS(
        check_client_server_agreed_kem(sike_iana, client_kems, 2, pq_kems_r1, 2, TLS_PQ_KEM_EXTENSION_ID_SIKE_P503_R1));
    EXPECT_SUCCESS(check_client_server_agreed_kem(sike_iana, client_kems, 2, pq_kems_r2r1, 4,
                                                  TLS_PQ_KEM_EXTENSION_ID_SIKE_P503_R1));
}
{
    uint8_t client_kems[] = {/* BIKE1_L1_R2 */
                             0x00, 0x0d,
                             /* SIKE_P434_R2 */
                             0x00, 0x13};

    EXPECT_FAILURE_WITH_ERRNO(
        check_client_server_agreed_kem(bike_iana, client_kems, 2, pq_kems_r1, 2, TLS_PQ_KEM_EXTENSION_ID_BIKE1_L1_R2),
        S2N_ERR_KEM_UNSUPPORTED_PARAMS);
    EXPECT_SUCCESS(check_client_server_agreed_kem(bike_iana, client_kems, 2, pq_kems_r2r1, 4,
                                                  TLS_PQ_KEM_EXTENSION_ID_BIKE1_L1_R2));
    EXPECT_FAILURE_WITH_ERRNO(
        check_client_server_agreed_kem(sike_iana, client_kems, 2, pq_kems_r1, 2, TLS_PQ_KEM_EXTENSION_ID_SIKE_P434_R2),
        S2N_ERR_KEM_UNSUPPORTED_PARAMS);
    EXPECT_SUCCESS(check_client_server_agreed_kem(sike_iana, client_kems, 2, pq_kems_r2r1, 4,
                                                  TLS_PQ_KEM_EXTENSION_ID_SIKE_P434_R2));
}
...
}
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        check_client_server_agreed_kem(sike_iana, client_kems, 2, pq_kems_r1, 2, TLS_PQ_KEM_EXTENSION_ID_SIKE_P503_R1),
        S2N_ERR_KEM_UNSUPPORTED_PARAMS);
    EXPECT_FAILURE_WITH_ERRNO(check_client_server_agreed_kem(sike_iana, client_kems, 2, pq_kems_r2r1, 4,
                                                             TLS_PQ_KEM_EXTENSION_ID_SIKE_P434_R2),
                              S2N_ERR_KEM_UNSUPPORTED_PARAMS);
}
{
    uint8_t client_kems[] = {/* SIKE_P434_R2 */
                             0x00, 0x13,
                             /* SIKE_P503_R1 */
                             0x00, 0x0a};
    ...
    EXPECT_FAILURE_WITH_ERRNO(
        ...
        S2N_ERR_KEM_UNSUPPORTED_PARAMS);
    EXPECT_FAILURE_WITH_ERRNO(
        check_client_server_agreed_kem(bike_iana, client_kems, 2, pq_kems_r2r1, 4, TLS_PQ_KEM_EXTENSION_ID_BIKE1_L1_R2),
        S2N_ERR_KEM_UNSUPPORTED_PARAMS);
    EXPECT_SUCCESS(
        check_client_server_agreed_kem(sike_iana, client_kems, 2, pq_kems_r1, 2, TLS_PQ_KEM_EXTENSION_ID_SIKE_P503_R1));
    EXPECT_SUCCESS(check_client_server_agreed_kem(sike_iana, client_kems, 2, pq_kems_r2r1, 4,
...
{
    const struct s2n_kem *sike_only_server_pref_list[] = {&s2n_sike_p434_r2, &s2n_sike_p503_r1};

    const struct s2n_kem *bike_r2_only_server_pref_list[] = {&s2n_bike1_l1_r2};

    const struct s2n_kem *negotiated_kem = NULL;
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_choose_kem_without_peer_pref_list(bike_iana, sike_only_server_pref_list, 2, &negotiated_kem),
        S2N_ERR_KEM_UNSUPPORTED_PARAMS);
    EXPECT_NULL(negotiated_kem);

    negotiated_kem = NULL;
    EXPECT_FAILURE_WITH_ERRNO(
        s2n_choose_kem_without_peer_pref_list(sike_iana, bike_r2_only_server_pref_list, 1, &negotiated_kem),
        S2N_ERR_KEM_UNSUPPORTED_PARAMS);
    EXPECT_NULL(negotiated_kem);
}
}
{
    ...
    EXPECT_FAILURE_WITH_ERRNO(s2n_cipher_suite_to_kem(classic_ecdhe_iana, &compatible_params),
                              S2N_ERR_KEM_UNSUPPORTED_PARAMS);
