                           ...
                           const struct s2n_kem *negotiated_kem)
{
    ...
    /* 2 extra bytes for the kem extension id and 2 additional bytes for the length of the public key sent over the
     * wire. */
    ...
    struct s2n_blob server_key_message = {
        .size = KEM_PUBLIC_KEY_MESSAGE_SIZE,
    ...
    struct s2n_blob client_key_message = {
        .size = KEM_CIPHERTEXT_MESSAGE_SIZE,
...
static int assert_kex_fips_checks(struct s2n_cipher_suite *cipher_suite, const char *security_policy_version,
                                  const struct s2n_kem *negotiated_kem)
{
