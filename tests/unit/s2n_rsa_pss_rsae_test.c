...
const char *kye_param_n =
    "bcb47b2e0dafcba81ff2a2b5cb115ca7e757184c9d72bcdc"
...
const char *key_param_d =
    "383a6f19e1ea27fd08c7fbc3bfa684bd6329888c0bbe4c98625e"
...
int main( int argc, char **argv )
{
    ...
    EXPECT_SUCCESS( s2n_test_cert_chain_and_key_new( &rsa_pss_cert_chain, S2N_RSA_PSS_2048_SHA256_LEAF_CERT,
                                                     S2N_RSA_PSS_2048_SHA256_LEAF_KEY ) );
    ...
    /* Self-Talk tests */
    {
        ...
        /* Test: RSA cert can't verify with PSS what it signed with PKCS1v1.5 */
        {
            ...
            EXPECT_SUCCESS(
                rsa_public_key.sign( rsa_cert_chain->private_key, S2N_SIGNATURE_RSA_PSS_RSAE, &sign_hash, &result ) );
            EXPECT_FAILURE_WITH_ERRNO(
                rsa_public_key.verify( &rsa_public_key, S2N_SIGNATURE_RSA, &verify_hash, &result ),
        ...
        /* Test: RSA cert can't verify with PKCS1v1.5 what it signed with PSS */
        {
            ...
            EXPECT_SUCCESS(
                rsa_public_key.sign( rsa_cert_chain->private_key, S2N_SIGNATURE_RSA, &sign_hash, &result ) );
            EXPECT_FAILURE_WITH_ERRNO(
                rsa_public_key.verify( &rsa_public_key, S2N_SIGNATURE_RSA_PSS_RSAE, &verify_hash, &result ),
                S2N_ERR_VERIFY_SIGNATURE );
        }
         ...
         * an RSA cert and an RSA_PSS cert are equivalent for PSS signatures. */
        {
            ...
            EXPECT_SUCCESS( s2n_asn1der_to_public_key_and_type( &rsa_pss_public_key, &rsa_pss_pkey_type,
            ...
            EXPECT_SUCCESS(
                RSA_set0_key( EVP_PKEY_get0_RSA( rsa_pss_public_key.pkey ), BN_dup( n ), BN_dup( e ), BN_dup( d ) ) );
            ...
            /* RSA signed with PSS, RSA_PSS verified with PSS */
            {
                ...
                EXPECT_SUCCESS( rsa_public_key.sign( rsa_cert_chain->private_key, S2N_SIGNATURE_RSA_PSS_RSAE,
                                                     &sign_hash, &result ) );
                EXPECT_SUCCESS(
            ...
            /* RSA_PSS signed with PSS, RSA verified with PSS */
            {
                ...
                EXPECT_SUCCESS( rsa_pss_public_key.sign( rsa_cert_chain->private_key, S2N_SIGNATURE_RSA_PSS_PSS,
                                                         &sign_hash, &result ) );
                EXPECT_SUCCESS(
                    rsa_public_key.verify( &rsa_public_key, S2N_SIGNATURE_RSA_PSS_RSAE, &verify_hash, &result ) );
            }
    ...
    /* Test: NIST test vectors */
    {
        ...
        for ( int i = 0; i < sizeof( test_cases ) / sizeof( struct test_case ); i++ ) {
            ...
            EXPECT_SUCCESS( rsa_public_key.verify( &rsa_public_key, S2N_SIGNATURE_RSA_PSS_RSAE, &verify_hash,
                                                   &signature_stuffer.blob ) );
