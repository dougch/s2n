...
static int custom_mem_init( void ) { return 0; }

static int custom_mem_cleanup( void ) { return 0; }

static int custom_mem_malloc( void **ptr, uint32_t requested, uint32_t *allocated )
{
    int i;
    for ( i = 0; i < HISTOGRAM_SIZE; i++ ) {
        if ( histogram_values[ i ] == 0 ) { histogram_values[ i ] = requested; }

        if ( histogram_values[ i ] == requested ) { break; }
    }

    if ( i < HISTOGRAM_SIZE ) { histogram_counts[ i ] += 1; }
...
void mock_client( struct s2n_test_piped_io *piped_io )
{
    ...
    if ( bytes_written != conn->active_application_bytes_consumed ) { exit( 0 ); }
...
int main( int argc, char **argv )
{
    ...
    for ( int is_dh_key_exchange = 0; is_dh_key_exchange <= 1; is_dh_key_exchange++ ) {
        ...
        for ( int cert = 0; cert < SUPPORTED_CERTIFICATE_FORMATS; cert++ ) {
            ...
            EXPECT_SUCCESS(
                s2n_cert_chain_and_key_load_pem( chain_and_keys[ cert ], cert_chain_pem, private_key_pem ) );
        ...
        for ( int i = 1; i < 0xffff; i += 100 ) {
            ...
            for ( int j = 0; j < i; j++ ) { EXPECT_EQUAL( buffer[ j ], 33 ); }
    ...
    for ( int i = 0; i < HISTOGRAM_SIZE; i++ ) {
        if ( histogram_counts[ i ] == 0 ) { break; }

        for ( int j = i + 1; j < HISTOGRAM_SIZE; j++ ) {
            if ( histogram_counts[ j ] == 0 ) { break; }
