     ...
     * RFC: https://tools.ietf.org/html/rfc8446#appendix-D.4 */
{
    ...
    for (size_t i = 0; i < valid_tls13_handshakes_size; i++) {
        ...
        for (size_t j = 1; j < S2N_MAX_HANDSHAKE_LENGTH; j++) {
            ...
            if (!IS_NEGOTIATED(handshake_type)) { continue; }
        ...
        if (IS_NEGOTIATED(handshake_type)) { EXPECT_TRUE(change_cipher_spec_found); }
    }
...
/* Test: TLS 1.3 handshake types are all properly printed */
{
    ...
    const char *all_flags_handshake_type_name =
        "NEGOTIATED|FULL_HANDSHAKE|CLIENT_AUTH|WITH_SESSION_TICKET|NO_CLIENT_CERT";
...
/* Test: TLS 1.3 message types are all properly printed */
{
    ...
    const char *expected[]          = {
        "CLIENT_HELLO",       "SERVER_HELLO",    "SERVER_CHANGE_CIPHER_SPEC", "ENCRYPTED_EXTENSIONS", "SERVER_CERT",
        "SERVER_CERT_VERIFY", "SERVER_FINISHED", "CLIENT_CHANGE_CIPHER_SPEC", "CLIENT_FINISHED",      "APPLICATION_DATA"
    };
...
{
    for (int i = CLIENT_HELLO; i <= APPLICATION_DATA; i++) { EXPECT_NOT_NULL(message_names[ i ]); }
}
