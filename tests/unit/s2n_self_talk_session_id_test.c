                         ...
                         const void *value, uint64_t value_size)
{
    ...
    if (key_size == 0 || key_size > MAX_KEY_LEN) { return -1; }
    if (value_size == 0 || value_size > MAX_VAL_LEN) { return -1; }
                            ...
                            uint64_t *value_size)
{
    ...
    if (cache[ index ].key_len != key_size) { return -1; }

    if (memcmp(cache[ index ].key, key, key_size)) { return -1; }

    if (*value_size < cache[ index ].value_len) { return -1; }
...
int cache_delete_callback(struct s2n_connection *conn, void *ctx, const void *key, uint64_t key_size)
{
    ...
    if (cache[ index ].key_len == 0) { return 0; }
...
{
    for (int i = 0; i < 256; i++) { session_cache[ i ].lock = 1; }
}
...
void mock_client(struct s2n_test_io_pair *io_pair)
{
    ...
    if (s2n_negotiate(conn, &blocked) != 0) { result = 1; }

    /* Make sure we did a full handshake */
    if (!IS_FULL_HANDSHAKE(conn->handshake.handshake_type)) { result = 2; }
    ...
    if (serialized_session_state_length > sizeof(serialized_session_state)) { result = 3; }

    /* Send very low session buffer size and see that you can get an error */
    if (s2n_connection_get_session(conn, serialized_session_state, 1) == 0) { result = 4; }

    if (s2n_errno != S2N_ERR_SERIALIZED_SESSION_STATE_TOO_LONG) { result = 5; }
    ...
    if (memcmp(conn->session_id, SESSION_ID, S2N_TLS_SESSION_ID_MAX_LEN) == 0) { result = 7; }

    if (s2n_send(conn, MSG, sizeof(MSG), &blocked) != sizeof(MSG)) { result = 8; }

    int shutdown_rc = -1;
    while (shutdown_rc != 0) { shutdown_rc = s2n_shutdown(conn, &blocked); }
    ...
    if (s2n_connection_set_session(conn, serialized_session_state, serialized_session_state_length) < 0) { result = 9; }

    if (s2n_negotiate(conn, &blocked) != 0) { result = 10; }

    /* Make sure we did a abbreviated handshake */
    if (!IS_RESUMPTION_HANDSHAKE(conn->handshake.handshake_type)) { result = 11; }

    if (s2n_send(conn, MSG, sizeof(MSG), &blocked) != sizeof(MSG)) { result = 12; }
    ...
    if (s2n_errno != S2N_ERR_INVALID_SERIALIZED_SESSION_STATE) { result = 14; }

    serialized_session_state[ 0 ] = 0;
    /* Change the protocol version (36th byte) in session state */
    if (serialized_session_state_length < 36) { result = 15; }
    ...
    if (s2n_negotiate(conn, &blocked) == 0) { result = 17; }

    if (s2n_errno != S2N_ERR_BAD_MESSAGE) { result = 18; }
...
int main(int argc, char **argv)
{
    ...
    uint8_t ticket_key[ 32 ]      = { 0x19, 0xef, 0x24, 0xa3, 0x2c, 0x71, 0x7b, 0x16, 0x7f, 0x33, 0xa9,
