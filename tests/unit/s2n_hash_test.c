...
int main( int argc, char **argv )
{
    ...
    if ( s2n_hash_is_available( S2N_HASH_MD5 ) ) {
        ...
        for ( int i = 0; i < 16; i++ ) { EXPECT_SUCCESS( s2n_stuffer_write_uint8_hex( &output, digest_pad[ i ] ) ); }
    ...
    EXPECT_SUCCESS( s2n_stuffer_init( &output, &out ) );
    for ( int i = 0; i < 20; i++ ) { EXPECT_SUCCESS( s2n_stuffer_write_uint8_hex( &output, digest_pad[ i ] ) ); }
    ...
    EXPECT_SUCCESS( s2n_stuffer_init( &output, &out ) );
    for ( int i = 0; i < 28; i++ ) { EXPECT_SUCCESS( s2n_stuffer_write_uint8_hex( &output, digest_pad[ i ] ) ); }
    ...
    EXPECT_SUCCESS( s2n_stuffer_init( &output, &out ) );
    for ( int i = 0; i < 32; i++ ) { EXPECT_SUCCESS( s2n_stuffer_write_uint8_hex( &output, digest_pad[ i ] ) ); }
    ...
    EXPECT_SUCCESS( s2n_stuffer_init( &output, &out ) );
    for ( int i = 0; i < 48; i++ ) { EXPECT_SUCCESS( s2n_stuffer_write_uint8_hex( &output, digest_pad[ i ] ) ); }

    /* Reference value from command line sha384sum */
    EXPECT_EQUAL(
        memcmp( output_pad,
                "f7f8f1b9d5a9a61742eeda26c20990282ac08dabda14e70376fcb4c8b46198a9959ea9d7d194b38520eed5397ffe6d8e",
                48 * 2 ),
        0 );
    ...
    EXPECT_SUCCESS( s2n_stuffer_init( &output, &out ) );
    for ( int i = 0; i < 64; i++ ) { EXPECT_SUCCESS( s2n_stuffer_write_uint8_hex( &output, digest_pad[ i ] ) ); }

    /* Reference value from command line sha512sum */
    EXPECT_EQUAL( memcmp( output_pad,
                          "32c07a0b3a3fd0dd8f28021b4eea1c19d871f4586316b394124f3c99fb68e59579e05039c3bd9aab9841214f1c13"
                          "2f7666eb8800f14be8b9b091a7dba32bfe6f",
                          64 * 2 ),
                  0 );
