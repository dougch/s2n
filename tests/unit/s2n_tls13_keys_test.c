...
int main(int argc, char **argv)
{
    ...
    EXPECT_SUCCESS(s2n_tls13_derive_handshake_secrets(&secrets, &ecdhe, &hash_state_copy, &client_handshake_secret,
                                                      &server_handshake_secret));

    /* this checks that the original hash state can still be used to derive a hash without being affected by the derive
     * function */
    ...
    EXPECT_SUCCESS(
        s2n_tls13_calculate_finished_mac(&secrets, &server_finished_key, &hash_state, &server_finished_verify));
    ...
    EXPECT_SUCCESS(
        s2n_tls13_calculate_finished_mac(&secrets, &client_finished_key, &hash_state, &client_finished_verify));

    /* Test Client Finished MAC hash */
    S2N_BLOB_EXPECT_EQUAL(expect_client_finished_verify, client_finished_verify);

    EXPECT_SUCCESS(s2n_tls13_derive_application_secrets(&secrets, &hash_state, &client_application_secret,
    ...
    EXPECT_SUCCESS(s2n_tls13_derive_traffic_keys(&secrets, &server_handshake_secret, &handshake_traffic_server_key,
                                                 &handshake_traffic_server_iv));

    EXPECT_SUCCESS(s2n_tls13_derive_traffic_keys(&secrets, &client_handshake_secret, &handshake_traffic_client_key,
