...
static int try_handshake(struct s2n_connection *server_conn, struct s2n_connection *client_conn)
{
    ...
    do {
        ...
        if (!(client_rc == 0 || (client_blocked && s2n_error_get_type(s2n_errno) == S2N_ERR_T_BLOCKED))) { return -1; }

        int server_rc = s2n_negotiate(server_conn, &server_blocked);
        if (!(server_rc == 0 || (server_blocked && s2n_error_get_type(s2n_errno) == S2N_ERR_T_BLOCKED))) { return -1; }
...
int test_cipher_preferences(struct s2n_config *server_config, struct s2n_config *client_config,
                            ...
                            s2n_signature_algorithm        expected_sig_alg)
{
    ...
    for (int cipher_idx = 0; cipher_idx < cipher_preferences->count; cipher_idx++) {
        ...
        if (!expected_cipher->available) { expect_failure = 1; }
...
int main(int argc, char **argv)
{
    ...
    for (test_type = TEST_TYPE_START; test_type < TEST_TYPE_END; test_type++) {
        ...
        /*  Test: ECDSA cert */
        {
            ...
            struct s2n_cert_chain_and_key *chain_and_key;
            EXPECT_SUCCESS(s2n_test_cert_chain_and_key_new(&chain_and_key, S2N_ECDSA_P384_PKCS1_CERT_CHAIN,
        ...
        if (s2n_is_rsa_pss_certs_supported()) {
            ...
            struct s2n_cert_chain_and_key *chain_and_key;
            EXPECT_SUCCESS(s2n_test_cert_chain_and_key_new(&chain_and_key, S2N_RSA_PSS_2048_SHA256_LEAF_CERT,
