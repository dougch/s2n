...
int main(int argc, char **argv)
{
    ...
    /* test invalid date fails */
    {
        ...
        uint64_t timestamp = 0;
        EXPECT_FAILURE_WITH_ERRNO(s2n_asn1_time_to_nano_since_epoch_ticks(time_str, strlen(time_str), &timestamp),
     ...
     * Also note, the moment timezones come into play, so does daylight savings time. So there are two tests here,
     * across the timezone boundaries.*/
    {
        char *tz = getenv("TZ");
