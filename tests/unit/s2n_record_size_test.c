...
#define ONE_HUNDRED_K 100000
    ...
    /* Test s2n_record_max_write_payload_size() have proper checks in place */
    {
        ...
        EXPECT_FAILURE_WITH_ERRNO( s2n_record_max_write_payload_size( server_conn ),
    ...
    /* Test large fragment/record sending for TLS 1.3 */
    {
        ...
        for ( int i = 0; i < iv.size; i++ ) { implicit_iv[ i ] = iv.data[ i ]; }
        ...
        EXPECT_TRUE( bytes_taken <= ( S2N_TLS_MAXIMUM_FRAGMENT_LENGTH
                                      + 255 ) ); /* Max record size for TLS 1.3 - 2^14 + 255 = 16639 */
