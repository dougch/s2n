...
int main(int argc, char **argv)
{
    ...
    uint8_t serialized_session_state[S2N_PARTIAL_SESSION_STATE_INFO_IN_BYTES + S2N_TICKET_SIZE_IN_BYTES
    ...
    uint8_t ticket_key1[32] = {0x07, 0x77, 0x09, 0x36, 0x2c, 0x2e, 0x32, 0xdf, 0x0d, 0xdc, 0x3f,
                               0x0d, 0xc4, 0x7b, 0xba, 0x63, 0x90, 0xb6, 0xc7, 0x3b, 0xb5, 0x0f,
                               0x9c, 0x31, 0x22, 0xec, 0x84, 0x4a, 0xd7, 0xc2, 0xb3, 0xe5};
    uint8_t ticket_key2[32] = {0x06, 0xa6, 0xb8, 0x8c, 0x58, 0x53, 0x36, 0x1a, 0x06, 0x10, 0x4c,
                               0x9c, 0xeb, 0x35, 0xb4, 0x5c, 0xef, 0x76, 0x00, 0x14, 0x90, 0x46,
                               0x71, 0x01, 0x4a, 0x19, 0x3f, 0x40, 0xc1, 0x5f, 0xc2, 0x44};
    uint8_t ticket_key3[32] = {0x19, 0xef, 0x24, 0xa3, 0x2c, 0x71, 0x7b, 0x16, 0x7f, 0x33, 0xa9,
     ...
     * 2) Client sends empty ST extension. Server does a full handshake, but is unable to issue NST due to absence of an
     * encrypt-decrypt key. 3) Client sends non-empty ST extension. Server does an abbreviated handshake without issuing
     * NST. 4) Client sends non-empty ST extension. Server does an abbreviated handshake and issues a NST because the
     * key is in decrypt-only state. 5) Client sends non-empty ST extension. Server does an abbreviated handshake, but
     * does not issue a NST even though the key is in decrypt-only state due to absence of encrypt-decrypt key. 6)
     * Client sends non-empty ST extension. Server does a full handshake and issues a NST because the key is not found.
     * 7) Client sends non-empty ST extension. Server does a full handshake and issues a NST because the key has
     * expired. 8) Client sends non-empty ST extension. Server does a full handshake and issues a NST because the ticket
     * has non-standard size. 9) Client sends non-empty ST extension, but server cannot or does not want to honor the
     * ticket. 10) Client sets corrupted ST extension. 11) User tries adding a duplicate key to the server. 12) Testing
     * expired keys are removed from the server config while adding new keys. 13) Scenario 1: Client sends empty ST and
     * server has multiple encrypt-decrypt keys to choose from for encrypting NST. 14) Scenario 2: Client sends empty ST
     * and server has multiple encrypt-decrypt keys to choose from for encrypting NST. 15) Testing
     * s2n_config_set_ticket_encrypt_decrypt_key_lifetime and s2n_config_set_ticket_decrypt_key_lifetime calls. 16) Add
     * keys out of order and pre-emptively add a key. 17) Handshake with client auth and session ticket enabled.
     */
    ...
    /* Client sends empty ST extension. Server issues NST. */
    {
        ...
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name1,
                                                        strlen((char *)ticket_key_name1), ticket_key1,
                                                        sizeof(ticket_key1), now / ONE_SEC_IN_NANOS));
    ...
    /* Client sends non-empty ST extension. Server does an abbreviated handshake without issuing NST. */
    {
        ...
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(
            server_config, ticket_key_name1, strlen((char *)ticket_key_name1), ticket_key1, sizeof(ticket_key1), 0));
        ...
        EXPECT_MEMCPY_SUCCESS(old_session_ticket, serialized_session_state,
                              S2N_PARTIAL_SESSION_STATE_INFO_IN_BYTES + S2N_TICKET_SIZE_IN_BYTES);
     ...
     */
    {
        ...
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name2,
                                                        strlen((char *)ticket_key_name2), ticket_key2,
                                                        strlen((char *)ticket_key2), 0));
     ...
     */
    {
        ...
        EXPECT_BYTEARRAY_EQUAL(s2n_set_get(server_config->ticket_keys, 0), ticket_key_name2,
                               strlen((char *)ticket_key_name2));
    ...
    /* Client sends non-empty ST extension. Server does a full handshake and issues a NST because the ticket has
     * non-standard size. */
    {
        EXPECT_NOT_NULL(client_config = s2n_config_new());
        EXPECT_SUCCESS(s2n_config_set_session_tickets_onoff(client_config, 1));
        ...
        EXPECT_MEMCPY_SUCCESS(tampered_session_state + 3, serialized_session_state + 4,
                              sizeof(tampered_session_state) - 4);
    ...
    /* Client sends non-empty ST extension, but server cannot or does not want to honor the ticket. */
    {
        ...
        EXPECT_EQUAL(s2n_connection_get_session(client_conn, serialized_session_state, serialized_session_state_length),
                     1 + 1 + client_conn->session_id_len + S2N_STATE_SIZE_IN_BYTES);
        ...
        /* Verify the lifetime hint from the server */
        EXPECT_FAILURE_WITH_ERRNO(s2n_connection_get_session_ticket_lifetime_hint(client_conn),
    ...
    /* User tries adding a duplicate key to the server */
    {
        ...
        EXPECT_EQUAL(-1,
                     s2n_config_add_ticket_crypto_key(server_config, ticket_key_name2, strlen((char *)ticket_key_name2),
        ...
        EXPECT_EQUAL(-1,
                     s2n_config_add_ticket_crypto_key(server_config, ticket_key_name1, strlen((char *)ticket_key_name1),
                                                      ticket_key2, sizeof(ticket_key2), 0));
        EXPECT_EQUAL(s2n_errno, S2N_ERR_INVALID_TICKET_KEY_NAME_OR_NAME_LENGTH);

        /* Try adding a key with invalid key length */
        EXPECT_EQUAL(-1, s2n_config_add_ticket_crypto_key(server_config, ticket_key_name2,
                                                          strlen((char *)ticket_key_name2), ticket_key2, 0, 0));
    ...
    /* Testing expired keys are removed from the server config while adding new keys. */
    {
        ...
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(
            server_config, ticket_key_name1, strlen((char *)ticket_key_name1), ticket_key1, sizeof(ticket_key1), 0));
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(
            server_config, ticket_key_name2, strlen((char *)ticket_key_name2), ticket_key2, sizeof(ticket_key2), 0));
        ...
        /* Add a third ST key */
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(
            server_config, ticket_key_name3, strlen((char *)ticket_key_name3), ticket_key3, sizeof(ticket_key3), 0));

        /* Try adding the expired keys */
        EXPECT_EQUAL(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name2, strlen((char *)ticket_key_name2),
                                                      ticket_key2, sizeof(ticket_key2), 0),
                     -1);
        EXPECT_EQUAL(s2n_errno, S2N_ELEMENT_ALREADY_IN_ARRAY);
        EXPECT_EQUAL(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name1, strlen((char *)ticket_key_name1),
                                                      ticket_key1, sizeof(ticket_key1), 0),
                     -1);
        EXPECT_EQUAL(s2n_errno, S2N_ELEMENT_ALREADY_IN_ARRAY);

        /* Verify that the config has only one unexpired key */
        EXPECT_BYTEARRAY_EQUAL(s2n_set_get(server_config->ticket_keys, 0), ticket_key_name3,
                               strlen((char *)ticket_key_name3));
        EXPECT_EQUAL(s2n_set_size(server_config->ticket_keys), 1);
    ...
    /* Add keys out of order and pre-emptively add a key */
    {
        ...
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name1,
                                                        strlen((char *)ticket_key_name1), ticket_key1,
                                                        sizeof(ticket_key1), (now / ONE_SEC_IN_NANOS) + 3600));

        /* Add a key. After 1 hour it will reach it's peak */
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name2,
                                                        strlen((char *)ticket_key_name2), ticket_key2,
                                                        sizeof(ticket_key2), now / ONE_SEC_IN_NANOS));
        ...
        EXPECT_SUCCESS(s2n_config_add_ticket_crypto_key(server_config, ticket_key_name3,
                                                        strlen((char *)ticket_key_name3), ticket_key3,
                                                        sizeof(ticket_key3), now / ONE_SEC_IN_NANOS + 36000));
        ...
        EXPECT_BYTEARRAY_EQUAL(((struct s2n_ticket_key *)s2n_set_get(server_config->ticket_keys, 0))->key_name,
                               ticket_key_name2, strlen((char *)ticket_key_name2));
        EXPECT_BYTEARRAY_EQUAL(((struct s2n_ticket_key *)s2n_set_get(server_config->ticket_keys, 1))->key_name,
                               ticket_key_name1, strlen((char *)ticket_key_name1));
        EXPECT_BYTEARRAY_EQUAL(((struct s2n_ticket_key *)s2n_set_get(server_config->ticket_keys, 2))->key_name,
                               ticket_key_name3, strlen((char *)ticket_key_name3));
