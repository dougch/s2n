...
int main( int argc, char **argv )
{
    ...
    /* Test 0 length application data record handled gracefully in client and server mode */
    {
        ...
        EXPECT_SUCCESS(
            s2n_stuffer_write_bytes( &client_to_server, record_header_blob.data, record_header_blob.size ) );
        EXPECT_SUCCESS(
            s2n_stuffer_write_bytes( &server_to_client, record_header_blob.data, record_header_blob.size ) );
