...
void process_safety_tester( int write_fd )
{
    ...
    if ( write( write_fd, pad, 100 ) != 100 ) { _exit( 100 ); }
...
int main( int argc, char **argv )
{
    ...
    for ( int i = 0; i < 5120; i++ ) {
            ...
            for ( int j = 0; j < i; j++ ) {
                for ( int k = 0; k < 8; k++ ) {
                    if ( data[ j ] & bits[ k ] ) {
                        ...
                        if ( j >= 64 ) { EXPECT_TRUE( bit_set_run[ k ] < 64 ); }
                    } else {
                        bit_set_run[ k ] = 0;
                    }
                }
        ...
        for ( int t = i - remainder; t < i; t++ ) { non_zero_found |= data[ t ]; }
        if ( !non_zero_found ) { trailing_zeros[ remainder ]++; }
    }
    for ( int t = 1; t < 8; t++ ) { EXPECT_TRUE( trailing_zeros[ t ] < 5120 / 16 ); }
