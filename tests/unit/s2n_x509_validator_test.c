...
int main(int argc, char **argv)
{
    ...
    /* test validator in unsafe mode */
    {
        ...
        EXPECT_EQUAL(S2N_CERT_OK, s2n_x509_validator_validate_cert_chain(&validator, connection, chain_data, chain_len,
    ...
    /* test validator in safe mode, but no configured trust store */
    {
        ...
        EXPECT_SUCCESS(s2n_pkey_zero_init(&public_key_out));
        s2n_pkey_type pkey_type;
        int err_code = s2n_x509_validator_validate_cert_chain(&validator, connection, chain_data, chain_len, &pkey_type,
                                                              &public_key_out);
    ...
    /* test validator in safe mode, with properly configured trust store, but max chain depth is exceeded*/
    {
        ...
        EXPECT_EQUAL(S2N_CERT_ERR_MAX_CHAIN_DEPTH_EXCEEDED,
                     s2n_x509_validator_validate_cert_chain(&validator, connection, chain_data, chain_len, &pkey_type,
                                                            &public_key_out));
        s2n_stuffer_free(&chain_stuffer);
        EXPECT_EQUAL(0, verify_data.callback_invoked);
        EXPECT_EQUAL(S2N_PKEY_TYPE_RSA, pkey_type);
     ...
     */
    {
        ...
        EXPECT_SUCCESS(s2n_pkey_zero_init(&public_key_out));
        s2n_pkey_type pkey_type;
        int ret_val = s2n_x509_validator_validate_cert_chain(&validator, connection, chain_data, chain_len, &pkey_type,
                                                             &public_key_out);
    ...
    /* test validator in safe mode, with properly configured trust store. host name validation succeeds, using s2n PEM
     * Parser */
    {
        struct s2n_x509_trust_store trust_store;
        s2n_x509_trust_store_init_empty(&trust_store);
    ...
    /* test validator in safe mode, with properly configured trust store. host name via alternative name validation
     * succeeds note: in this case, we don't have valid certs but it's enough to make sure we are properly pulling
     * alternative names from the certificate. */
    {
        struct s2n_x509_trust_store trust_store;
        s2n_x509_trust_store_init_empty(&trust_store);
        EXPECT_SUCCESS(s2n_x509_trust_store_from_ca_file(&trust_store, S2N_DEFAULT_TEST_CERT_CHAIN, NULL));
    ...
    /* test validator in safe mode, with properly configured trust store. host name via alternative name validation
     * fails, and no Common Name validation happens as DNS alternative name is present. note: in this case, we don't
     * have valid certs but it's enough to make sure we are properly validating alternative names and common name.*/
    {
        struct s2n_x509_trust_store trust_store;
        s2n_x509_trust_store_init_empty(&trust_store);
        EXPECT_SUCCESS(s2n_x509_trust_store_from_ca_file(&trust_store, S2N_DEFAULT_TEST_CERT_CHAIN, NULL));
    ...
    /* Test valid OCSP date range */
    {
        ...
        EXPECT_EQUAL(S2N_CERT_OK, s2n_x509_validator_validate_cert_stapled_ocsp_response(
                                      &validator, connection, s2n_stuffer_raw_read(&ocsp_data_stuffer, ocsp_data_len),
                                      ocsp_data_len));
    ...
    /* Test invalid OCSP date range (after is off) */
    {
        ...
        uint32_t ocsp_data_len = s2n_stuffer_data_available(&ocsp_data_stuffer);
        EXPECT_TRUE(ocsp_data_len > 0);
        EXPECT_EQUAL(S2N_CERT_ERR_EXPIRED, s2n_x509_validator_validate_cert_stapled_ocsp_response(
    ...
    /* Test valid OCSP date range, but the data itself is untrusted */
    {
        ...
        EXPECT_EQUAL(S2N_CERT_ERR_EXPIRED, s2n_x509_validator_validate_cert_stapled_ocsp_response(
                                               &validator, connection, raw_data, ocsp_data_len));
...
#endif /* S2N_OCSP_STAPLING_SUPPORTED */
    /* test validator in safe mode, with default host name validator. Connection server name matches alternative name on
     * a certificate. */
    {
        struct s2n_x509_trust_store trust_store;
        s2n_x509_trust_store_init_empty(&trust_store);
    ...
    /* test validator in safe mode, with default host name validator. Connection server name matches wildcard
     * alternative name on a certificate. */
    {
        struct s2n_x509_trust_store trust_store;
        s2n_x509_trust_store_init_empty(&trust_store);
        EXPECT_EQUAL(0, s2n_x509_trust_store_from_ca_file(&trust_store, S2N_RSA_2048_SHA256_WILDCARD_CERT, NULL));
    ...
    /* test validator in safe mode, with default host name validator. Connection server does not match alternative names
     * on a certificate. */
    {
        struct s2n_x509_trust_store trust_store;
        s2n_x509_trust_store_init_empty(&trust_store);
