...
/* Key update is triggered by encryption limits */
{
    ...
    EXPECT_EQUAL(client_conn->key_update_pending, false);
    EXPECT_BYTEARRAY_EQUAL(client_conn->secure.client_sequence_number, zeroed_sequence_number,
...
/* Key update is not triggered */
{
    ...
    EXPECT_EQUAL(client_conn->key_update_pending, false);
    EXPECT_BYTEARRAY_EQUAL(client_conn->secure.client_sequence_number, expected_sequence_number,
                           S2N_TLS_SEQUENCE_NUM_LEN);
...
/* s2n_check_record_limit */
{
    ...
    if (s2n_chacha20_poly1305.is_available()) {
        ...
        for (size_t i = 0; i < S2N_TLS_SEQUENCE_NUM_LEN; i++) { conn->secure.server_sequence_number[ i ] = UINT8_MAX; }
