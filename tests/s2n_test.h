...
#define EXPECT_TRUE_WITHOUT_COUNT(condition)                        \
    do {                                                            \
        if (!(condition)) { FAIL_MSG(#condition " is not true "); } \
    } while (0)
...
#define EXPECT_MEMCPY_SUCCESS(d, s, n)                                                            \
    do {                                                                                          \
        ...
        if (__tmp_n) {                                                                            \
            if (memcpy((d), (s), (__tmp_n)) == NULL) { FAIL_MSG(#d "is NULL, memcpy() failed"); } \
        }                                                                                         \
    } while (0)
...
#define S2N_FUZZ_TARGET(fuzz_init, fuzz_entry, fuzz_cleanup)                                      \
    ...
    {                                                                                             \
        if (fuzz_cleanup) { (( void (*)() )fuzz_cleanup)(); }                                     \
        s2n_cleanup();                                                                            \
    }                                                                                             \
    ...
    {                                                                                             \
        ...
        if (!fuzz_init) { return S2N_SUCCESS; }                                                   \
        int result = (( int (*)(int *argc, char **argv[]) )fuzz_init)(argc, argv);                \
        if (result != S2N_SUCCESS) { FAIL_MSG_PRINT(#fuzz_init " did not return S2N_SUCCESS"); }  \
        return result;                                                                            \
    }                                                                                             \
    int LLVMFuzzerTestOneInput(const uint8_t *buf, size_t len)                                    \
    {                                                                                             \
        int result = fuzz_entry(buf, len);                                                        \
        if (result != S2N_SUCCESS) { FAIL_MSG_PRINT(#fuzz_entry " did not return S2N_SUCCESS"); } \
        return result;                                                                            \
    }
