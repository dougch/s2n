...
static inline int FindMarker( FILE *infile, const char *marker )
{
    ...
    for ( i = 0; i < len; i++ ) {
        if ( ( line[ i ] = fgetc( infile ) ) == EOF ) { return -1; }
    }
    line[ len ] = '\0';

    while ( 1 ) {
        if ( !strncmp( line, marker, len ) ) { return 0; }

        for ( i = 0; i < len - 1; i++ ) { line[ i ] = line[ i + 1 ]; }
        if ( ( line[ len - 1 ] = fgetc( infile ) ) == EOF ) { return -1; }
        line[ len ] = '\0';
    }
}
...
static inline int ReadHex( FILE *infile, uint8_t *buf, uint32_t len, const char *str )
{
    ...
    if ( FindMarker( infile, str ) == -1 ) { return -1; }
    ...
    while ( ( ch = fgetc( infile ) ) != EOF ) {
        ...
        for ( uint32_t i = 0; i < len - 1; i++ ) { buf[ i ] = ( buf[ i ] << 4 ) | ( buf[ i + 1 ] >> 4 ); }
