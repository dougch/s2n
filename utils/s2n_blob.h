...
#define S2N_BLOB_LABEL(name, str)                    \
  ...
  const struct s2n_blob name = {.data = name##_data, \
                                .size = sizeof(name##_data) - 1};

/* The S2N_BLOB_FROM_HEX macro creates a s2n_blob with the contents of a hex
 * string. It is allocated on a stack so there no need to free after use. hex
 * should be a const char[]. This function checks against using char*, because
 * sizeof needs to refer to the buffer length rather than a pointer size */
