...
/* A blob is growable if it is either explicitly marked as such, or if it
 * contains no data */
bool s2n_blob_is_growable(const struct s2n_blob *b) {
  return b && (b->growable ||
               (b->data == NULL && b->size == 0 && b->allocated == 0));
}
...
int s2n_get_memory(struct s2n_blob *b, uint32_t size) {
  if (use_mlock) {
    ...
    *b = (struct s2n_blob){.data = NULL,
                           .size = size,
                           .allocated = allocate,
                           .mlocked = 1,
                           .growable = 1};
    S2N_ERROR_IF(posix_memalign((void **)&b->data, page_size, allocate),
                 S2N_ERR_ALLOC);
#ifdef MADV_DONTDUMP
    ...
    *b = (struct s2n_blob){.data = calloc(size, 1),
                           .size = size,
                           .allocated = size,
                           .mlocked = 0,
                           .growable = 1};
...
int s2n_free(struct s2n_blob *b) {
  ...
  /* To avoid memory leaks, still free the data even if we can't unlock / wipe
   * it */
...
int s2n_free_object(uint8_t **p_data, uint32_t size) {
  ...
  struct s2n_blob b = {
      .data = *p_data, .size = size, .mlocked = use_mlock, .growable = 1};
