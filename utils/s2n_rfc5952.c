...
int s2n_inet_ntop(int af, const void *addr, struct s2n_blob *dst) {
  ...
  if (af == AF_INET6) {
    S2N_ERROR_IF(dst->size < sizeof("1111:2222:3333:4444:5555:6666:7777:8888"),
     ...
     *   3/ or if there are two or more equal length longest runs, go with the
     * left-most run 4/ Make that run :: 5/ Print the remaining 16-bit fields in
     * lowercase hex, no leading zeroes
     */
