...
#define notnull_check( ptr )                                  \
    do {                                                      \
        if ( ( ptr ) == NULL ) { S2N_ERROR( S2N_ERR_NULL ); } \
    } while ( 0 )
#define notnull_check_ptr( ptr )                                  \
    do {                                                          \
        if ( ( ptr ) == NULL ) { S2N_ERROR_PTR( S2N_ERR_NULL ); } \
    } while ( 0 )

static inline void *trace_memcpy_check( void *restrict to, const void *restrict from, size_t size,
...
#define char_to_digit( c, d )                                 \
    do {                                                      \
        if ( !isdigit( c ) ) { S2N_ERROR( S2N_ERR_SAFETY ); } \
        d = c - '0';                                          \
    } while ( 0 )
...
#define gte_check( n, min )                                 \
    do {                                                    \
        if ( ( n ) < min ) { S2N_ERROR( S2N_ERR_SAFETY ); } \
    } while ( 0 )
#define lte_check( n, max )                                 \
    do {                                                    \
        if ( ( n ) > max ) { S2N_ERROR( S2N_ERR_SAFETY ); } \
    } while ( 0 )
...
#define eq_check( a, b )                                       \
    do {                                                       \
        if ( ( a ) != ( b ) ) { S2N_ERROR( S2N_ERR_SAFETY ); } \
    } while ( 0 )
#define ne_check( a, b )                                       \
    do {                                                       \
        if ( ( a ) == ( b ) ) { S2N_ERROR( S2N_ERR_SAFETY ); } \
...
#define GUARD_NONBLOCKING( x )                                      \
    do {                                                            \
        int __tmp_r = ( x );                                        \
        if ( ERR_IS_BLOCKING( __tmp_r ) ) { S2N_ERROR( __tmp_r ); } \
        GUARD( __tmp_r );                                           \
    } while ( 0 )
 ...
 * Always returns zero. */
extern int s2n_constant_time_pkcs1_unpad_or_dont( uint8_t *dst, const uint8_t *src, uint32_t srclen,
                                                  uint32_t expectlen );
