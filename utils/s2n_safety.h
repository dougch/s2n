...
#define notnull_check(ptr)     \
  do {                         \
    if ((ptr) == NULL) {       \
      S2N_ERROR(S2N_ERR_NULL); \
    }                          \
  } while (0)
#define notnull_check_ptr(ptr)     \
  do {                             \
    if ((ptr) == NULL) {           \
      S2N_ERROR_PTR(S2N_ERR_NULL); \
    }                              \
  } while (0)

static inline void *trace_memcpy_check(void *restrict to,
                                       const void *restrict from, size_t size,
...
#define gte_check(n, min)        \
  do {                           \
    if ((n) < min) {             \
      S2N_ERROR(S2N_ERR_SAFETY); \
    }                            \
  } while (0)
#define lte_check(n, max)        \
  do {                           \
    if ((n) > max) {             \
      S2N_ERROR(S2N_ERR_SAFETY); \
    }                            \
  } while (0)
#define gt_check(n, min)         \
  do {                           \
    if ((n) <= min) {            \
      S2N_ERROR(S2N_ERR_SAFETY); \
    }                            \
  } while (0)
#define lt_check(n, max)         \
  do {                           \
    if ((n) >= max) {            \
      S2N_ERROR(S2N_ERR_SAFETY); \
    }                            \
...
#define GUARD(x)                     \
  do {                               \
    if ((x) < 0) return S2N_FAILURE; \
  } while (0)
#define GUARD_STRICT(x)               \
  do {                                \
    if ((x) != 0) return S2N_FAILURE; \
  } while (0)
...
/* Similar to GUARD, but preserves the blocking error code. S2N_CALLBACK_BLOCKED
 * is left to preserve backwards compatibility .*/
...
#define GUARD_NONNULL(x)                 \
  do {                                   \
    if ((x) == NULL) return S2N_FAILURE; \
  } while (0)
#define GUARD_NONNULL_GOTO(x, label) \
  do {                               \
    if ((x) == NULL) goto label;     \
  } while (0)
#define GUARD_NONNULL_PTR(x)      \
  do {                            \
...
/* TODO: use the OSSL error code in error reporting
...
extern int s2n_constant_time_equals(const uint8_t *a, const uint8_t *b,
                                    uint32_t len);

/* Copy src to dst, or don't copy it, in constant time */
extern int s2n_constant_time_copy_or_dont(uint8_t *dst, const uint8_t *src,
                                          uint32_t len, uint8_t dont);
 ...
 * Always returns zero. */
extern int s2n_constant_time_pkcs1_unpad_or_dont(uint8_t *dst,
                                                 const uint8_t *src,
                                                 uint32_t srclen,
                                                 uint32_t expectlen);
...
/**
 * Rounds "initial" up to a multiple of "alignment", and stores the result in
 * "out". Raises an error if overflow would occur. NOT CONSTANT TIME.
 */
