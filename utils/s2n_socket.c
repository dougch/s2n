...
int s2n_socket_quickack(struct s2n_connection *conn) {
  ...
  struct s2n_socket_read_io_context *r_io_ctx =
      (struct s2n_socket_read_io_context *)conn->recv_io_context;
  ...
  int optval = 1;
  if (setsockopt(r_io_ctx->fd, IPPROTO_TCP, TCP_QUICKACK, &optval,
                 sizeof(optval)) == 0) {
...
int s2n_socket_write_snapshot(struct s2n_connection *conn) {
  ...
  struct s2n_socket_write_io_context *w_io_ctx =
      (struct s2n_socket_write_io_context *)conn->send_io_context;
  notnull_check(w_io_ctx);

  getsockopt(w_io_ctx->fd, IPPROTO_TCP, S2N_CORK, &w_io_ctx->original_cork_val,
             &corklen);
...
int s2n_socket_read_snapshot(struct s2n_connection *conn) {
  ...
  getsockopt(r_io_ctx->fd, SOL_SOCKET, SO_RCVLOWAT,
             &r_io_ctx->original_rcvlowat_val, &watlen);
...
int s2n_socket_write_restore(struct s2n_connection *conn) {
  ...
  setsockopt(w_io_ctx->fd, IPPROTO_TCP, S2N_CORK, &w_io_ctx->original_cork_val,
             sizeof(w_io_ctx->original_cork_val));
...
int s2n_socket_read_restore(struct s2n_connection *conn) {
  ...
  setsockopt(r_io_ctx->fd, SOL_SOCKET, SO_RCVLOWAT,
             &r_io_ctx->original_rcvlowat_val,
...
int s2n_socket_was_corked(struct s2n_connection *conn) {
  /* If we're not using custom I/O and a send fd has not been set yet, return
   * false*/
  if (!conn->managed_io || !conn->send) {
    return 0;
  }

  struct s2n_socket_write_io_context *io_ctx =
      (struct s2n_socket_write_io_context *)conn->send_io_context;
