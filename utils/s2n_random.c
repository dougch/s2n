...
/* clang can define gcc version to be < 4.3, but cpuid.h exists for most
 * releases */
#if ((defined(__x86_64__) || defined(__i386__)) && \
     (defined(__clang__) || S2N_GCC_VERSION_AT_LEAST(4, 3, 0)))
#include <cpuid.h>
#endif
...
static inline int s2n_defend_if_forked(void) {
  ...
  struct s2n_blob public = {.data = s2n_public_drbg,
                            .size = sizeof(s2n_public_drbg)};
  struct s2n_blob private = {.data = s2n_private_drbg,
                             .size = sizeof(s2n_private_drbg)};
...
OPEN:
...
#if defined(MAP_INHERIT_ZERO)
  zero_if_forked_ptr = mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE,
                            MAP_ANON | MAP_PRIVATE, -1, 0);
  S2N_ERROR_IF(zero_if_forked_ptr == MAP_FAILED, S2N_ERR_OPEN_RANDOM);

  S2N_ERROR_IF(
      minherit(zero_if_forked_ptr, sizeof(int), MAP_INHERIT_ZERO) == -1,
      S2N_ERR_OPEN_RANDOM);
#else

  S2N_ERROR_IF(pthread_atfork(NULL, NULL, s2n_on_fork) != 0,
               S2N_ERR_OPEN_RANDOM);
#endif
  ...
  /* Create an engine */
  ENGINE *e = ENGINE_new();
  if (e == NULL || ENGINE_set_id(e, "s2n_rand") != 1 ||
      ...
      ENGINE_set_RAND(e, &s2n_openssl_rand_method) != 1 || ENGINE_add(e) != 1 ||
      ENGINE_free(e) != 1) {
    S2N_ERROR(S2N_ERR_OPEN_RANDOM);
  }
  ...
  S2N_ERROR_IF(e == NULL || ENGINE_init(e) != 1 ||
 ...
 * overwritten in s2n_defend_if_forked if it is forked. This was added to
...
#if defined(__i386__)
    struct {
      /* since we check first that we're on intel, we can safely assume little
       * endian. */
      uint32_t u_low;
      uint32_t u_high;
    } i386_fields;
...
#if defined(__i386__)
      /* execute the rdrand instruction, store the result in a general purpose
       * register (it's assigned to output.i386_fields.u_low). Check the carry
       * bit, which will be set on success. Then clober the register and reset
       * the carry bit. Due to needing to support an ancient assembler we use
       * the opcode syntax. the %b1 is to force compilers to use c1 instead of
       * ecx. Here's a description of how the opcode is encoded: 0x0fc7 (rdrand)
...
#else
      /* execute the rdrand instruction, store the result in a general purpose
       * register (it's assigned to output.u64). Check the carry bit, which will
       * be set on success. Then clober the carry bit. Due to needing to support
       * an ancient assembler we use the opcode syntax. the %b1 is to force
       * compilers to use c1 instead of ecx. Here's a description of how the
       * opcode is encoded: 0x48 (pick a 64-bit register it does more too, but
       * that's all that matters there) 0x0fc7 (rdrand) 0xf0 (store the result
       * in rax). */
      __asm__ __volatile__(
