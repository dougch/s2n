...
int64_t s2n_public_random( int64_t bound )
{
    ...
    while ( 1 ) {
         ...
         */
        if ( r < ( UINT64_MAX - ( UINT64_MAX % bound ) ) ) { return r % bound; }
    }
}
...
int s2n_openssl_compat_rand( unsigned char *buf, int num )
{
    ...
    if ( s2n_get_private_random_data( &out ) < 0 ) { return 0; }
    return 1;
}
...
int s2n_cpu_supports_rdrand()
{
    ...
    if ( ecx & RDRAND_ECX_FLAG ) { return 1; }
#endif
    return 0;
}
...
#    endif /* defined(__i386__) */
             ...
             * similar bugs. */
            if ( output.u64 == UINT64_MAX || output.u64 == 0 ) { success = 0; }
