...
static int s2n_array_enlarge(struct s2n_array *array, uint32_t capacity) {
  ...
  GUARD(s2n_mul_overflow(array->element_size, array->num_of_elements,
                         &array_elements_size));
  memset_check(array->mem.data + array_elements_size, 0,
               array->mem.size - array_elements_size);
...
int s2n_array_insert_and_copy(struct s2n_array *array, void *element,
                              uint32_t index) {
...
void *s2n_array_insert(struct s2n_array *array, uint32_t index) {
  ...
  /* We are about to add one more element to the array. Add capacity if
   * necessary */
...
int s2n_array_free(struct s2n_array *array) { return s2n_array_free_p(&array); }
