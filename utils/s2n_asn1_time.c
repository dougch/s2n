...
/* this is just a standard state machine for ASN1 date format... nothing
 * special. just do a character at a time and change the state per character
 * encountered. when finished the above time structure should be filled in along
 * with some crazy timezone info we'll need shortly afterwards.*/
static parser_state process_state(parser_state state, char current_char,
...
int s2n_asn1_time_to_nano_since_epoch_ticks(const char *asn1_time, uint32_t len,
                                            uint64_t *ticks) {
  /* figure out if we are on something other than UTC since timegm is not
  ...
  /* state on subsecond means no timezone info was found and we assume local
   * time */
  S2N_ERROR_IF(!(state == FINISHED || state == ON_SUBSECOND),
               S2N_ERR_INVALID_ARGUMENT);

  time_t clock_data = 0;
  /* ASN1 + and - is in format HHMM. We need to convert it to seconds for the
   * adjustment */
  ...
  /* if we detected UTC is being used (please always use UTC), we need to add
   * the detected timezone on the local machine back to the offset. Also, the
   * offset includes an offset for daylight savings time. When the time being
   * parsed and the local time are on different sides of the dst barrier, the
   * offset has to be adjusted to account for it. */
  if (!args.local_time_assumed) {
    gmt_offset -= gmt_offset_current;
    gmt_offset -=
        args.time.tm_isdst != is_dst ? (args.time.tm_isdst - is_dst) * 3600 : 0;
  }
