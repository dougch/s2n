...
void* s2n_set_get(struct s2n_set* set, uint32_t index) { return s2n_array_get(set->data, index); }

int s2n_set_remove(struct s2n_set* set, uint32_t index) { return s2n_array_remove(set->data, index); }
...
int s2n_set_free(struct s2n_set* set) { return s2n_set_free_p(&set); }

int s2n_set_size(struct s2n_set* set) { return set->data->num_of_elements; }
