...
bool s2n_stuffer_is_valid( const struct s2n_stuffer *stuffer )
{
    ...
    return S2N_OBJECT_PTR_IS_READABLE( stuffer ) && s2n_blob_is_valid( &stuffer->blob ) &&
           /* <= is valid because we can have a fully written/read stuffer */
           stuffer->high_water_mark <= stuffer->blob.size && stuffer->write_cursor <= stuffer->high_water_mark
...
{
    if ( stuffer->alloced ) { GUARD( s2n_free( &stuffer->blob ) ); }
...
int s2n_stuffer_resize( struct s2n_stuffer *stuffer, const uint32_t size )
{
    ...
    if ( size == stuffer->blob.size ) { return 0; }

    if ( size < stuffer->blob.size ) { GUARD( s2n_stuffer_wipe_n( stuffer, stuffer->blob.size - size ) ); }
...
{
    if ( stuffer->blob.data == NULL ) { GUARD( s2n_stuffer_resize( stuffer, size ) ); }
...
{
    if ( stuffer->read_cursor < size ) { S2N_ERROR( S2N_ERR_STUFFER_OUT_OF_DATA ); }
...
{
    if ( size >= stuffer->write_cursor ) { return s2n_stuffer_wipe( stuffer ); }
...
{
    if ( stuffer->blob.data == NULL ) { return 0; }

    S2N_ERROR_IF( stuffer->read_cursor != stuffer->write_cursor, S2N_ERR_STUFFER_HAS_UNPROCESSED_DATA );
...
int s2n_stuffer_writev_bytes( struct s2n_stuffer *stuffer, const struct iovec *iov, int iov_count, size_t offs,
                              size_t size )
{
...
int s2n_stuffer_extract_blob( struct s2n_stuffer *stuffer, struct s2n_blob *out )
{
    ...
    if ( s2n_stuffer_data_available( stuffer ) > 0 ) {
        memcpy_check( out->data, stuffer->blob.data + stuffer->read_cursor, s2n_stuffer_data_available( stuffer ) );
    }
    return S2N_SUCCESS;
}
