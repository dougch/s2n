...
int s2n_choose_sig_scheme_from_peer_preference_list(struct s2n_connection *conn,
    ...
    S2N_ERROR_IF(conn->actual_protocol_version <= S2N_TLS12 && chosen_scheme.signature_curve != NULL,
                 S2N_ERR_INVALID_SIGNATURE_SCHEME);
    ...
    S2N_ERROR_IF(conn->actual_protocol_version == S2N_TLS13 && chosen_scheme.sig_alg == S2N_SIGNATURE_ECDSA &&
                     chosen_scheme.hash_alg != S2N_HASH_SHA1 && chosen_scheme.signature_curve == NULL,
