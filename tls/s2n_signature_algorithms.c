...
static int s2n_signature_scheme_valid_to_offer( struct s2n_connection *conn, const struct s2n_signature_scheme *scheme )
{
    ...
    if ( !s2n_is_rsa_pss_signing_supported() ) { ne_check( scheme->sig_alg, S2N_SIGNATURE_RSA_PSS_RSAE ); }

    if ( !s2n_is_rsa_pss_certs_supported() ) { ne_check( scheme->sig_alg, S2N_SIGNATURE_RSA_PSS_PSS ); }

    return 0;
}

static int s2n_signature_scheme_valid_to_accept( struct s2n_connection *            conn,
                                  ...
                                  struct s2n_signature_scheme *chosen_scheme_out )
{
    ...
    for ( int i = 0; i < signature_preferences->count; i++ ) {
        ...
        if ( s2n_signature_scheme_valid_to_accept( conn, candidate ) != S2N_SUCCESS ) { continue; }

        if ( s2n_is_sig_scheme_valid_for_auth( conn, candidate ) != S2N_SUCCESS ) { continue; }
                                                      ...
                                                      struct s2n_signature_scheme *chosen_sig_scheme )
{
    ...
    for ( int i = 0; i < signature_preferences->count; i++ ) {
        ...
        if ( 0 != s2n_signature_scheme_valid_to_accept( conn, candidate ) ) { continue; }
...
int s2n_choose_default_sig_scheme( struct s2n_connection *conn, struct s2n_signature_scheme *sig_scheme_out )
{
    ...
    *sig_scheme_out = s2n_rsa_pkcs1_md5_sha1;
    ...
    if ( cipher_suite_auth_method == S2N_AUTHENTICATION_ECDSA ) { *sig_scheme_out = s2n_ecdsa_sha1; }
...
int s2n_recv_supported_sig_scheme_list( struct s2n_stuffer *in, struct s2n_sig_scheme_list *sig_hash_algs )
{
    ...
    if ( pairs_available > TLS_SIGNATURE_SCHEME_LIST_MAX_LEN ) { S2N_ERROR( S2N_ERR_TOO_MANY_SIGNATURE_SCHEMES ); }
