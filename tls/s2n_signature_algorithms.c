...
static int s2n_signature_scheme_valid_to_offer(
    struct s2n_connection *conn, const struct s2n_signature_scheme *scheme) {
  /* We don't know what protocol version we will eventually negotiate, but we
   * know that it won't be any higher. */
...
static int s2n_signature_scheme_valid_to_accept(
    struct s2n_connection *conn, const struct s2n_signature_scheme *scheme) {
...
int s2n_choose_sig_scheme(struct s2n_connection *conn,
  ...
  const struct s2n_signature_preferences *signature_preferences =
      conn->config->signature_preferences;
  ...
  for (int i = 0; i < signature_preferences->count; i++) {
    const struct s2n_signature_scheme *candidate =
        signature_preferences->signature_schemes[i];

    if (s2n_signature_scheme_valid_to_accept(conn, candidate) != S2N_SUCCESS) {
      continue;
    }

    if (s2n_is_sig_alg_valid_for_auth(conn, candidate->sig_alg) !=
        S2N_SUCCESS) {
      continue;
    }
...
int s2n_get_and_validate_negotiated_signature_scheme(
    struct s2n_connection *conn, struct s2n_stuffer *in,
...
int s2n_choose_default_sig_scheme(struct s2n_connection *conn,
  ...
  s2n_authentication_method cipher_suite_auth_method =
      conn->secure.cipher_suite->auth_method;

  /* Default our signature digest algorithms. For TLS 1.2 this default is
   * different and may be overridden by the signature_algorithms extension. If
   * the server chooses an ECDHE_ECDSA cipher suite, this will be overridden to
   * SHA1.
   */
  *sig_scheme_out = s2n_rsa_pkcs1_md5_sha1;
  ...
  /* Default RSA Hash Algorithm is SHA1 (instead of MD5_SHA1) if TLS 1.2 or FIPS
   * mode */
...
int s2n_choose_sig_scheme_from_peer_preference_list(
    struct s2n_connection *conn, struct s2n_sig_scheme_list *peer_wire_prefs,
    struct s2n_signature_scheme *sig_scheme_out) {
  notnull_check(conn);
  notnull_check(sig_scheme_out);
  ...
  /* SignatureScheme preference list was first added in TLS 1.2. It will be
   * empty in older TLS versions. */
  ...
  /* In TLS 1.3, SigScheme also defines the ECDSA curve to use (instead of
   * reusing whatever ECDHE Key Exchange curve was negotiated). In TLS 1.2 and
   * before, chosen_scheme.signature_curve must *always* be NULL, if it's not,
   * it's a bug in s2n's preference list. */
  S2N_ERROR_IF(conn->actual_protocol_version <= S2N_TLS12 &&
                   chosen_scheme.signature_curve != NULL,
               S2N_ERR_INVALID_SIGNATURE_SCHEME);

  /* If TLS 1.3 is negotiated, then every ECDSA SigScheme must also define an
   * ECDSA Curve *except* ECDSA_SHA1, which uses the same curve negotiated in
   * the ECDHE SupportedGroups Extension. */
  S2N_ERROR_IF(conn->actual_protocol_version == S2N_TLS13 &&
...
int s2n_send_supported_sig_scheme_list(struct s2n_connection *conn,
  ...
  GUARD(
      s2n_stuffer_write_uint16(out, s2n_supported_sig_scheme_list_size(conn)));

  for (int i = 0; i < signature_preferences->count; i++) {
    const struct s2n_signature_scheme *const scheme =
        signature_preferences->signature_schemes[i];
...
int s2n_supported_sig_schemes_count(struct s2n_connection *conn) {
  ...
  uint8_t count = 0;
  for (int i = 0; i < signature_preferences->count; i++) {
    if (0 == s2n_signature_scheme_valid_to_offer(
                 conn, signature_preferences->signature_schemes[i])) {
      count++;
    }
  }
  return count;
...
int s2n_recv_supported_sig_scheme_list(
    struct s2n_stuffer *in, struct s2n_sig_scheme_list *sig_hash_algs) {
