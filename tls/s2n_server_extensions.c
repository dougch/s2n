...
#define s2n_server_can_send_server_name(conn) \
  ((conn)->server_name_used && !s2n_connection_is_session_resumed((conn)))

#define s2n_server_can_send_secure_renegotiation(conn) \
  ((conn)->secure_renegotiation && (conn)->actual_protocol_version < S2N_TLS13)

#define s2n_server_can_send_nst(conn) \
  (s2n_server_sending_nst((conn)) &&  \
   (conn)->actual_protocol_version < S2N_TLS13)

int s2n_server_extensions_send(struct s2n_connection *conn,
  ...
  if (s2n_server_can_send_kex(conn)) {
    total_size += s2n_kex_server_extension_size(
        conn->secure.cipher_suite->key_exchange_alg, conn);
  }
  ...
  if (s2n_server_can_send_kex(conn)) {
    GUARD(s2n_kex_write_server_extension(
        conn->secure.cipher_suite->key_exchange_alg, conn, out));
  }
  ...
  if (s2n_server_can_send_secure_renegotiation(conn)) {
    ...
    /* renegotiated_connection length. Zero since we don't support
     * renegotiation. */
  ...
  if (application_protocol_len) {
    ...
    GUARD(s2n_stuffer_write_bytes(out, (uint8_t *)conn->application_protocol,
  ...
  if (s2n_server_can_send_sct_list(conn)) {
    ...
    GUARD(s2n_stuffer_write_uint16(
        out, conn->handshake_params.our_chain_and_key->sct_list.size));
    GUARD(s2n_stuffer_write_bytes(
        out, conn->handshake_params.our_chain_and_key->sct_list.data,
...
int s2n_server_extensions_recv(struct s2n_connection *conn,
    ...
    switch (extension_type) {
      ...
      case TLS_EXTENSION_SUPPORTED_VERSIONS:
        if (s2n_is_tls13_enabled()) {
          GUARD(
              s2n_extensions_server_supported_versions_recv(conn, &extension));
        }
        break;
