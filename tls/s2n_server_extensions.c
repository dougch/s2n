...
int s2n_server_extensions_send_size( struct s2n_connection *conn )
{
    ...
    GUARD_UINT16_AND_INCREMENT( s2n_kex_server_extension_size( conn->secure.cipher_suite->key_exchange_alg, conn ),
                                total_size );
    ...
    while ( s2n_stuffer_data_available( &in ) ) {
        ...
        switch ( extension_type ) {
            ...
            case TLS_EXTENSION_KEY_SHARE:
                if ( s2n_is_tls13_enabled() ) { GUARD( s2n_extensions_server_key_share_recv( conn, &extension ) ); }
                break;
            case TLS_EXTENSION_COOKIE:
                if ( s2n_is_tls13_enabled() ) { GUARD( s2n_extensions_cookie_recv( conn, &extension ) ); }
                break;
        }
    }
