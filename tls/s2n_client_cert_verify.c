...
int s2n_client_cert_verify_recv(struct s2n_connection *conn) {
  ...
  if (conn->actual_protocol_version == S2N_TLS12) {
    /* Verify the Server picked one of the SignatureSchemes that we sent in the
  ...
  /* Use a copy of the hash state since the verify digest computation may modify
   * the running hash state we need later. */
  struct s2n_hash_state hash_state = {0};
  GUARD(s2n_handshake_get_hash_state(conn, chosen_sig_scheme.hash_alg,
                                     &hash_state));
  GUARD(s2n_hash_copy(&conn->handshake.ccv_hash_copy, &hash_state));

  /* Verify the signature */
  GUARD(s2n_pkey_verify(&conn->secure.client_public_key,
                        chosen_sig_scheme.sig_alg,
                        &conn->handshake.ccv_hash_copy, &signature));
...
int s2n_client_cert_verify_send(struct s2n_connection *conn) {
  ...
  if (conn->actual_protocol_version == S2N_TLS12) {
    ...
    GUARD(s2n_stuffer_write_uint16(
        out, conn->secure.client_cert_sig_scheme.iana_value));
  }
  ...
  GUARD(s2n_pkey_sign(cert_chain_and_key->private_key,
                      chosen_sig_scheme.sig_alg, &conn->handshake.ccv_hash_copy,
                      &signature));
