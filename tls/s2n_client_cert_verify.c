...
int s2n_client_cert_verify_recv(struct s2n_connection *conn)
{
    ...
    GUARD(s2n_pkey_verify(&conn->secure.client_public_key, chosen_sig_scheme.sig_alg, &conn->handshake.ccv_hash_copy,
                          &signature));
...
int s2n_client_cert_verify_send(struct s2n_connection *conn)
{
    ...
    GUARD(s2n_pkey_sign(cert_chain_and_key->private_key, chosen_sig_scheme.sig_alg, &conn->handshake.ccv_hash_copy,
                        &signature));
