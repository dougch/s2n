...
/* This module should contain any logic related to choosing a valid combination
 * of signature algorithm, authentication method, and certificate to use for
 * authentication.
 *
 * We choose our auth methods by:
 * 1. Finding a cipher suite with an auth method that we have valid certs for.
 * In TLS1.3, this is a no-op -- cipher suites do not specify an auth method.
 * 2. Choosing a signature algorithm that matches both the auth method (if set)
 * and the available certs.
 * 3. Selecting the cert that matches the chosen signature algorithm.
 *
 * This is a break from the original s2n pre-TLS1.3 flow, when we could choose
 * certs and ciphers at the same time. Our cipher suites differentiate between
 * "RSA" and "ECDSA", but not between "RSA" and "RSA-PSS". To make that
 * decision, we need to wait until we've chosen a signature algorithm. This
 * allows us to use RSA-PSS with existing TLS1.2 cipher suites.
 */

static int s2n_get_auth_method_for_cert_type(
    s2n_pkey_type cert_type, s2n_authentication_method *auth_method) {
  switch (cert_type) {
...
static int s2n_get_cert_type_for_sig_alg(s2n_signature_algorithm sig_alg,
...
static int s2n_is_sig_alg_valid_for_cipher_suite(
    s2n_signature_algorithm sig_alg, struct s2n_cipher_suite *cipher_suite) {
  ...
  /* Non-ephemeral key exchange methods require encryption, and RSA-PSS
   * certificates do not support encryption.
   *
   ...
   */
  if (cipher_suite->key_exchange_alg != NULL &&
      !cipher_suite->key_exchange_alg->is_ephemeral) {
  ...
  if (cipher_suite->auth_method != S2N_AUTHENTICATION_METHOD_SENTINEL) {
    ...
    GUARD(s2n_get_auth_method_for_cert_type(cert_type_for_sig_alg,
                                            &auth_method_for_sig_alg));
...
static int s2n_certs_exist_for_sig_alg(struct s2n_connection *conn,
...
static int s2n_certs_exist_for_auth_method(
    struct s2n_connection *conn, s2n_authentication_method auth_method) {
 ...
 */
int s2n_is_cipher_suite_valid_for_auth(struct s2n_connection *conn,
 ...
 * - At least one compatible cert is configured.
 * - The signature algorithm is allowed by the cipher suite's auth method (if
 * present).
 *
 * This method is called by the both server and client when choosing a signature
 * algorithm.
 */
int s2n_is_sig_alg_valid_for_auth(struct s2n_connection *conn,
                                  s2n_signature_algorithm sig_alg) {
  notnull_check(conn);
 ...
 */
int s2n_is_cert_type_valid_for_auth(struct s2n_connection *conn,
                                    s2n_pkey_type cert_type) {
  notnull_check(conn);
  ...
  if (conn->secure.cipher_suite->auth_method !=
      S2N_AUTHENTICATION_METHOD_SENTINEL) {
    S2N_ERROR_IF(auth_method != conn->secure.cipher_suite->auth_method,
 ...
 *
 * This method is called by the server after configuring its cipher suite and
 * sig algs.
 */
int s2n_select_certs_for_server_auth(
  ...
  s2n_pkey_type cert_type;
  GUARD(s2n_get_cert_type_for_sig_alg(conn->secure.conn_sig_scheme.sig_alg,
                                      &cert_type));
