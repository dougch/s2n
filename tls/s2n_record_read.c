...
int s2n_sslv2_record_header_parse(struct s2n_connection *conn,
                                  uint8_t *record_type,
                                  uint8_t *client_protocol_version,
                                  uint16_t *fragment_length) {
  struct s2n_stuffer *in = &conn->header_in;

  S2N_ERROR_IF(s2n_stuffer_data_available(in) < S2N_TLS_RECORD_HEADER_LENGTH,
               S2N_ERR_BAD_MESSAGE);
...
int s2n_record_header_parse(struct s2n_connection *conn, uint8_t *content_type,
                            uint16_t *fragment_length) {
  struct s2n_stuffer *in = &conn->header_in;
  ...
  /* https://tools.ietf.org/html/rfc5246#appendix-E.1 states that servers must
   * accept any value {03,XX} as the record layer version number for the first
   * TLS record. There is some ambiguity here because the client does not know
   * what version to use in the record header prior to receiving the
   * ServerHello. Some client implementations may use a garbage value(not
   * {03,XX}) in the ClientHello. Choose to be lenient to these clients. After
   * protocol negotiation, we will enforce that all record versions match the
   * negotiated version.
   */

  S2N_ERROR_IF(conn->actual_protocol_version_established &&
                   MIN(conn->actual_protocol_version,
                       S2N_TLS12) /* check against legacy record version (1.2)
                                     in tls 1.3 */
                       != version,
               S2N_ERR_BAD_MESSAGE);
  GUARD(s2n_stuffer_read_uint16(in, fragment_length));
...
int s2n_record_parse(struct s2n_connection *conn) {
  ...
  switch (cipher_suite->record_alg->cipher->type) {
    case S2N_AEAD:
      GUARD(s2n_record_parse_aead(cipher_suite, conn, content_type,
                                  encrypted_length, implicit_iv, mac,
                                  sequence_number, session_key));
      break;
    case S2N_CBC:
      GUARD(s2n_record_parse_cbc(cipher_suite, conn, content_type,
                                 encrypted_length, implicit_iv, mac,
                                 sequence_number, session_key));
      break;
    case S2N_COMPOSITE:
      GUARD(s2n_record_parse_composite(cipher_suite, conn, content_type,
                                       encrypted_length, implicit_iv, mac,
                                       sequence_number, session_key));
      break;
    case S2N_STREAM:
      GUARD(s2n_record_parse_stream(cipher_suite, conn, content_type,
                                    encrypted_length, implicit_iv, mac,
                                    sequence_number, session_key));
      break;
    default:
...
int s2n_parse_record_type(struct s2n_stuffer *stuffer, uint8_t *record_type) {
  GUARD(
      s2n_stuffer_skip_read(stuffer, s2n_stuffer_data_available(stuffer) - 1));
