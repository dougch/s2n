...
#define S2N_KEY_EXCHANGE_DH \
  0x01 /* Diffie-Hellman key exchange, including ephemeral */
...
#define S2N_CIPHER_SUITE_COUNT \
  38 /* Kept up-to-date by s2n_cipher_suite_match_test */
...
/* Used by TLS 1.3 CipherSuites (Eg TLS_AES_128_GCM_SHA256 "0x1301") where the
 * Auth method will be specified by the SignatureScheme Extension, not the
 * CipherSuite. */
...
struct s2n_cipher_suite {
  ...
  const struct s2n_record_algorithm
      *all_record_algs[S2N_MAX_POSSIBLE_RECORD_ALGS];
  ...
  /* RFC 5426(TLS1.2) allows cipher suite defined PRFs. Cipher suites defined in
   * and before TLS1.2 will use P_hash with SHA256 when TLS1.2 is negotiated.
   */
...
extern struct s2n_cipher_suite *s2n_cipher_suite_from_wire(
    const uint8_t cipher_suite[S2N_TLS_CIPHER_SUITE_LEN]);
extern int s2n_set_cipher_as_client(struct s2n_connection *conn,
                                    uint8_t wire[S2N_TLS_CIPHER_SUITE_LEN]);
extern int s2n_set_cipher_as_sslv2_server(struct s2n_connection *conn,
                                          uint8_t *wire, uint16_t count);
extern int s2n_set_cipher_as_tls_server(struct s2n_connection *conn,
                                        uint8_t *wire, uint16_t count);
