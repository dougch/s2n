...
/* Enough to support TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384, 2*SHA384_DIGEST_LEN
 * + 2*AES256_KEY_SIZE */
...
/* The s2n p_hash implementation is abstracted to allow for separate
 * implementations, using either s2n's formally verified HMAC or OpenSSL's EVP
 * HMAC, for use by the TLS PRF. */
struct s2n_p_hash_hmac {
  int (*new)(struct s2n_prf_working_space *ws);
  int (*init)(struct s2n_prf_working_space *ws, s2n_hmac_algorithm alg,
              struct s2n_blob *secret);
  int (*update)(struct s2n_prf_working_space *ws, const void *data,
                uint32_t size);
...
extern int s2n_tls_prf_master_secret(struct s2n_connection *conn,
                                     struct s2n_blob *premaster_secret);
extern int s2n_hybrid_prf_master_secret(struct s2n_connection *conn,
                                        struct s2n_blob *premaster_secret);
