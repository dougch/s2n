...
static uint16_t overhead(struct s2n_connection *conn) {
  ...
  uint8_t extra;
  GUARD(
      s2n_hmac_digest_size(active->cipher_suite->record_alg->hmac_alg, &extra));
...
int s2n_record_rounded_write_payload_size(struct s2n_connection *conn,
                                          uint16_t size_without_overhead) {
  ...
  if (active->cipher_suite->record_alg->cipher->type == S2N_CBC) {
    max_fragment_size -=
        max_fragment_size %
        active->cipher_suite->record_alg->cipher->io.cbc.block_size;
  } else if (active->cipher_suite->record_alg->cipher->type == S2N_COMPOSITE) {
    max_fragment_size -=
        max_fragment_size %
        active->cipher_suite->record_alg->cipher->io.comp.block_size;
    /* Composite digest length */
    max_fragment_size -=
        active->cipher_suite->record_alg->cipher->io.comp.mac_key_size;
    /* Padding length byte */
    max_fragment_size -= 1;
  }
...
int s2n_record_max_write_payload_size(struct s2n_connection *conn) {
  return s2n_record_rounded_write_payload_size(
      conn, conn->max_outgoing_fragment_length);
}

int s2n_record_min_write_payload_size(struct s2n_connection *conn) {
  uint16_t min_outgoing_fragement_length =
      ETH_MTU - (conn->ipv6 ? IP_V6_HEADER_LENGTH : IP_V4_HEADER_LENGTH) -
      TCP_HEADER_LENGTH - TCP_OPTIONS_LENGTH - S2N_TLS_RECORD_HEADER_LENGTH;
  return s2n_record_rounded_write_payload_size(conn,
                                               min_outgoing_fragement_length);
}
...
int s2n_record_write_protocol_version(struct s2n_connection *conn) {
  ...
  if (conn->server_protocol_version == s2n_unknown_protocol_version) {
     ...
     * version higher than TLS1.0. To provide maximum compatibility, send record
     * version as TLS1.0 if server protocol version isn't established yet, which
     * happens only during ClientHello message. Note, this has no effect on
     * protocol version in ClientHello, so we're still able to negotiate
     * protocol versions above TLS1.0 */
    record_protocol_version = MIN(record_protocol_version, S2N_TLS10);
  }

  /* In accordance to TLS 1.3 spec,
   * https://tools.ietf.org/html/rfc8446#section-5.1 tls record version should
   * never be greater than 33 (legacy TLS 1.2 version).
   */
  ...
  GUARD(s2n_stuffer_write_bytes(&conn->out, protocol_version,
...
static inline int s2n_record_encrypt(
    struct s2n_connection *conn, const struct s2n_cipher_suite *cipher_suite,
    struct s2n_session_key *session_key, struct s2n_blob *iv,
    struct s2n_blob *aad, struct s2n_blob *en, uint8_t *implicit_iv,
    uint16_t block_size) {
  notnull_check(en->data);

  switch (cipher_suite->record_alg->cipher->type) {
    case S2N_STREAM:
      GUARD(cipher_suite->record_alg->cipher->io.stream.encrypt(session_key, en,
                                                                en));
      break;
    case S2N_CBC:
      GUARD(cipher_suite->record_alg->cipher->io.cbc.encrypt(session_key, iv,
                                                             en, en));
      ...
      break;
    case S2N_AEAD:
      GUARD(cipher_suite->record_alg->cipher->io.aead.encrypt(session_key, iv,
                                                              aad, en, en));
      break;
    case S2N_COMPOSITE:
      /* This will: compute mac, append padding, append padding length, and
       * encrypt */
      GUARD(cipher_suite->record_alg->cipher->io.comp.encrypt(session_key, iv,
                                                              en, en));
...
int s2n_record_writev(struct s2n_connection *conn, uint8_t content_type,
                      const struct iovec *in, int in_count, size_t offs,
                      size_t to_write) {
  struct s2n_blob iv;
  uint8_t padding = 0;
  ...
  const int is_tls13_record =
      cipher_suite->record_alg->flags & S2N_TLS13_RECORD_AEAD_NONCE;
  s2n_stack_blob(aad, is_tls13_record ? S2N_TLS13_AAD_LEN : S2N_TLS_MAX_AAD_LEN,
                 S2N_TLS_MAX_AAD_LEN);
  ...
  uint16_t data_bytes_to_take =
      MIN(to_write, s2n_record_max_write_payload_size(conn));
  ...
  GUARD(s2n_stuffer_write_uint8(
      &conn->out, is_tls13_record ?
  ...
  if (conn->actual_protocol_version > S2N_SSLv3) {
    GUARD(s2n_hmac_update(mac, conn->out.blob.data,
  ...
  /* Compute non-payload parts of the MAC(seq num, type, proto vers, fragment
   * length) for composite ciphers. Composite "encrypt" will MAC the payload
   * data and fill in padding.
   */
  if (cipher_suite->record_alg->cipher->type == S2N_COMPOSITE) {
    /* Only fragment length is needed for MAC, but the EVP ctrl function needs
     * fragment length + eiv len. */
    ...
    int pad_and_mac_len;
    GUARD(cipher_suite->record_alg->cipher->io.comp.initial_hmac(
        session_key, sequence_number, content_type,
  ...
  if (cipher_suite->record_alg->cipher->type == S2N_AEAD) {
    ...
    if (cipher_suite->record_alg->flags & S2N_TLS12_AES_GCM_AEAD_NONCE) {
      ...
      GUARD(s2n_stuffer_write_bytes(&conn->out, sequence_number,
                                    S2N_TLS_SEQUENCE_NUM_LEN));
      GUARD(s2n_stuffer_write_bytes(
          &iv_stuffer, implicit_iv,
          cipher_suite->record_alg->cipher->io.aead.fixed_iv_size));
      GUARD(s2n_stuffer_write_bytes(&iv_stuffer, sequence_number,
                                    S2N_TLS_SEQUENCE_NUM_LEN));
    } else if (cipher_suite->record_alg->flags &
                   S2N_TLS12_CHACHA_POLY_AEAD_NONCE ||
               is_tls13_record) {
      ...
      GUARD(s2n_stuffer_write_bytes(&iv_stuffer, sequence_number,
                                    S2N_TLS_SEQUENCE_NUM_LEN));
      for (int i = 0;
           i < cipher_suite->record_alg->cipher->io.aead.fixed_iv_size; i++) {
    ...
    if (is_tls13_record) {
      GUARD(s2n_tls13_aead_aad_init(
          data_bytes_to_take + TLS13_CONTENT_TYPE_LENGTH,
          cipher_suite->record_alg->cipher->io.aead.tag_size, &ad_stuffer));
    } else {
      GUARD(s2n_aead_aad_init(conn, sequence_number, content_type,
                              data_bytes_to_take, &ad_stuffer));
    }
  ...
  struct s2n_blob seq = {.data = sequence_number,
                         .size = S2N_TLS_SEQUENCE_NUM_LEN};
  GUARD(s2n_increment_sequence_number(&seq));

  /* Write the plaintext data */
  GUARD(s2n_stuffer_writev_bytes(&conn->out, in, in_count, offs,
                                 data_bytes_to_take));
  void *orig_write_ptr =
      conn->out.blob.data + conn->out.write_cursor - data_bytes_to_take;
  ...
  switch (cipher_suite->record_alg->cipher->type) {
    case S2N_AEAD:
      GUARD(s2n_stuffer_skip_write(
          &conn->out,
          cipher_suite->record_alg->cipher->io.aead.record_iv_size));
    ...
    case S2N_CBC:
      ...
      break;
    case S2N_COMPOSITE:
      /* Composite CBC expects a pointer starting at explicit IV: [Explicit IV |
       * fragment | MAC | padding | padding len ] extra will account for the
       * explicit IV len(if applicable), MAC digest len, padding len + padding
       * byte.
       */
  ...
  struct s2n_blob en = {
      ...
      .data = s2n_stuffer_raw_write(&conn->out, encrypted_length)};
  GUARD(s2n_record_encrypt(conn, cipher_suite, session_key, &iv, &aad, &en,
                           implicit_iv, block_size));
...
int s2n_record_write(struct s2n_connection *conn, uint8_t content_type,
                     struct s2n_blob *in) {
  struct iovec iov;
