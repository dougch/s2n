...
struct s2n_handshake_parameters {
  /* Signature/hash algorithm pairs offered by the client in the
   * signature_algorithms extension */
  struct s2n_sig_scheme_list client_sig_hash_algs;

  /* Signature/hash algorithm pairs offered by the server in the certificate
   * request */
  ...
  /* The subset of certificates that match the server_name presented in the
   * ClientHello. In the case of multiple certificates matching a server_name,
   * s2n will prefer certificates in FIFO order based on calls to
   * s2n_config_add_cert_chain_and_key_to_store
   *
   * Note that in addition to domain matching, the key type for the certificate
   * must also be suitable for a negotiation in order to be selected. The set of
   * matching certs here are indexed by s2n_authentication_method.
   *
   * Example:
   *    - Assume certA is added to s2n_config via
   * s2n_config_add_cert_chain_and_key_to_store
   *    - Next certB is added.
   *    - if certA matches www.foo.com and certB matches www.foo.com, s2n will
   * prefer certA
   *
   * Note that in addition to domain matching, the key type for the certificate
   * must also be suitable for a negotiation in order to be selected.
   *
   * Example:
...
struct s2n_handshake {
  ...
  /* A copy of the handshake messages hash used to validate the
  ...
  /* Hash algorithms required for this handshake. The set of required hashes can
   * be reduced as session parameters are negotiated, i.e. cipher suite and
   * protocol version.
   */
  ...
  /* Indicates the handshake blocked while trying to read data, and has been
   * paused */
  unsigned paused : 1;
...
#define IS_RESUMPTION_HANDSHAKE(type) \
  (!IS_FULL_HANDSHAKE((type)) && IS_NEGOTIATED((type)))
...
extern message_type_t s2n_conn_get_current_message_type(
...
extern uint8_t s2n_handshake_is_hash_required(struct s2n_handshake *handshake,
                                              s2n_hash_algorithm hash_alg);
extern int s2n_conn_update_required_handshake_hashes(
    struct s2n_connection *conn);
extern int s2n_handshake_get_hash_state(struct s2n_connection *conn,
                                        s2n_hash_algorithm hash_alg,
                                        struct s2n_hash_state *hash_state);
extern int s2n_conn_find_name_matching_certs(struct s2n_connection *conn);
extern int s2n_create_wildcard_hostname(struct s2n_stuffer *hostname,
