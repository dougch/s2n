...
struct s2n_security_policy_selection security_policy_selection[] = {
    ...
    { .version                   = "CloudFront-TLS-1-2-2020",
      .security_policy           = &security_policy_cloudfront_tls_1_2_2020,
      .ecc_extension_required    = 0,
      .pq_kem_extension_required = 0 },
...
{
    if (security_policy == NULL) { return false; }
...
bool s2n_security_policy_supports_tls13(const struct s2n_security_policy *security_policy)
{
    ...
    if (cipher_preferences == NULL) { return false; }

    for (uint8_t i = 0; i < cipher_preferences->count; i++) {
        if (s2n_is_valid_tls13_cipher(cipher_preferences->suites[ i ]->iana_value)) { return true; }
    }
...
int s2n_connection_is_valid_for_cipher_preferences(struct s2n_connection *conn, const char *version)
{
    ...
    if (s2n_connection_get_actual_protocol_version(conn) < security_policy->minimum_protocol_version) { return 0; }
