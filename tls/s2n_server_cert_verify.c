...
static int s2n_server_write_cert_verify_signature(struct s2n_connection *conn,
                                                  struct s2n_stuffer *out);
static int s2n_server_generate_unsigned_cert_verify_content(
    struct s2n_connection *conn, struct s2n_stuffer *unsigned_content);
...
int s2n_server_cert_read_and_verify_signature(struct s2n_connection *conn) {
  ...
  S2N_ERROR_IF(signature_size > s2n_stuffer_data_available(in),
               S2N_ERR_BAD_MESSAGE);
  ...
  /* Verify signature */
  GUARD(s2n_server_generate_unsigned_cert_verify_content(conn,
                                                         &unsigned_content));

  GUARD(s2n_hash_init(&message_hash, chosen_sig_scheme.hash_alg));
  GUARD(s2n_hash_update(&message_hash, unsigned_content.blob.data,
                        s2n_stuffer_data_available(&unsigned_content)));
  GUARD(s2n_pkey_verify(&conn->secure.server_public_key,
                        chosen_sig_scheme.sig_alg, &message_hash,
                        &signed_content));
...
int s2n_server_cert_verify_recv(struct s2n_connection *conn) {
  ...
  GUARD(s2n_get_and_validate_negotiated_signature_scheme(
      conn, &conn->handshake.io, &conn->secure.conn_sig_scheme));
...
int s2n_server_write_cert_verify_signature(struct s2n_connection *conn,
                                           struct s2n_stuffer *out) {
  notnull_check(conn->handshake_params.our_chain_and_key);
  const struct s2n_pkey *pkey =
      conn->handshake_params.our_chain_and_key->private_key;
  ...
  GUARD(s2n_hash_update(&message_hash, unsigned_content.blob.data,
                        s2n_stuffer_data_available(&unsigned_content)));
  GUARD(s2n_pkey_sign(pkey, conn->secure.conn_sig_scheme.sig_alg, &message_hash,
                      &signed_content));
...
/* Concatenates the handshake hash used for generating a Certificate Verify
 * Signature. */
int s2n_server_generate_unsigned_cert_verify_content(
    struct s2n_connection *conn, struct s2n_stuffer *unsigned_content) {
  ...
  GUARD(s2n_handshake_get_hash_state(conn, tls13_ctx.hash_algorithm,
                                     &handshake_hash));
  ...
  GUARD(s2n_stuffer_alloc(
      unsigned_content,
      hash_digest_length + s2n_server_cert_verify_header_length()));
  GUARD(s2n_stuffer_write_bytes(unsigned_content, S2N_CERT_VERIFY_PREFIX,
                                sizeof(S2N_CERT_VERIFY_PREFIX)));
  GUARD(s2n_stuffer_write_bytes(unsigned_content,
                                S2N_SERVER_CERT_VERIFY_CONTEXT,
                                sizeof(S2N_SERVER_CERT_VERIFY_CONTEXT)));
  GUARD(s2n_stuffer_write_bytes(unsigned_content, digest_out,
                                hash_digest_length));
...
uint8_t s2n_server_cert_verify_header_length() {
  return sizeof(S2N_CERT_VERIFY_PREFIX) +
         sizeof(S2N_SERVER_CERT_VERIFY_CONTEXT);
}
