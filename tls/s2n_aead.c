...
int s2n_aead_aad_init(const struct s2n_connection *conn,
...
int s2n_tls13_aead_aad_init(uint16_t record_length, uint8_t tag_length,
                            struct s2n_stuffer *additional_data) {
  ...
  GUARD(s2n_stuffer_write_uint8(additional_data,
                                TLS_APPLICATION_DATA)); /* fixed to 0x17 */
  GUARD(s2n_stuffer_write_uint8(additional_data, 3));   /* TLS record layer   */
  GUARD(s2n_stuffer_write_uint8(additional_data,
                                3)); /* version fixed at 1.2 (0x0303) */
