...
int s2n_process_alert_fragment(struct s2n_connection *conn) {
  ...
  while (s2n_stuffer_data_available(&conn->in)) {
    ...
    int bytes_to_read =
        MIN(bytes_required, s2n_stuffer_data_available(&conn->in));
...
int s2n_queue_writer_close_alert_warning(struct s2n_connection *conn) {
  ...
  /* If there is an alert pending or we've already sent a close_notify, do
   * nothing */
  if (s2n_stuffer_data_available(&conn->writer_alert_out) ||
      conn->close_notify_queued) {
    return 0;
  }
...
static int s2n_queue_reader_alert(struct s2n_connection *conn, uint8_t level,
                                  uint8_t error_code) {
  uint8_t alert[2];
  alert[0] = level;
  alert[1] = error_code;
...
int s2n_queue_reader_unsupported_protocol_version_alert(
  ...
  return s2n_queue_reader_alert(conn, S2N_TLS_ALERT_LEVEL_FATAL,
                                S2N_TLS_ALERT_PROTOCOL_VERSION);
}
