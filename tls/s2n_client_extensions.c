...
int s2n_client_extensions_send( struct s2n_connection *conn, struct s2n_stuffer *out )
{
    ...
    if ( server_name_len ) { total_size += 9 + server_name_len; }
    if ( application_protocols_len ) { total_size += 6 + application_protocols_len; }
    if ( conn->config->status_request_type != S2N_STATUS_REQUEST_NONE ) { total_size += 9; }
    if ( conn->config->ct_type != S2N_CT_SUPPORT_NONE ) { total_size += 4; }
    if ( conn->config->mfl_code != S2N_TLS_MAX_FRAG_LEN_EXT_NONE ) { total_size += 5; }
    if ( conn->config->use_tickets ) { total_size += 4 + client_ticket_len; }
    ...
    if ( pq_kem_extension_required ) {
        ...
        pq_kem_list_size = security_policy->kem_preferences->count * 2;
    ...
    if ( server_name_len ) { GUARD( s2n_extensions_client_server_name_send( conn, out ) ); }

    /* Write ALPN extension */
    if ( application_protocols_len ) { GUARD( s2n_extensions_client_alpn_send( conn, out ) ); }
    ...
    if ( conn->config->ct_type != S2N_CT_SUPPORT_NONE ) { GUARD( s2n_extensions_client_sct_list_send( conn, out ) ); }
    ...
    if ( conn->config->use_tickets ) { GUARD( s2n_extensions_client_session_ticket_send( conn, out ) ); }
     ...
     */
    if ( ecc_extension_required ) { GUARD( s2n_extensions_client_supported_groups_send( conn, out ) ); }

    if ( pq_kem_extension_required ) { GUARD( s2n_extensions_client_pq_kem_send( conn, out, pq_kem_list_size ) ); }
