...
int s2n_client_extensions_send(struct s2n_connection *conn,
  ...
  const uint8_t ecc_extension_required =
      s2n_ecc_extension_required(cipher_preferences);
  ...
  const uint8_t pq_kem_extension_required =
      s2n_pq_kem_extension_required(cipher_preferences);
  /* pq_kem_extension_required is true if and only if
   * cipher_preferences->kem_count > 0 */
  if (pq_kem_extension_required) {
    /* 2 for the extension id, 2 for overall length, 2 for length of the list,
     * and 2 for each kem ID*/
   ...
   * (renamed Supported Groups in TLS 1.3 RFC 8446) and the Supported Point
   * Formats Extension.
   */
  if (ecc_extension_required) {
...
int s2n_client_extensions_recv(struct s2n_connection *conn,
  ...
  for (int i = 0; i < parsed_extensions->num_of_elements; i++) {
    struct s2n_client_hello_parsed_extension *parsed_extension =
        s2n_array_get(parsed_extensions, i);
    ...
    switch (parsed_extension->extension_type) {
      ...
      case TLS_EXTENSION_SIGNATURE_ALGORITHMS:
        GUARD(
            s2n_extensions_client_signature_algorithms_recv(conn, &extension));
        break;
      ...
      case TLS_EXTENSION_SUPPORTED_VERSIONS:
        if (s2n_is_tls13_enabled()) {
          GUARD(
              s2n_extensions_client_supported_versions_recv(conn, &extension));
        }
        break;
