...
int s2n_connection_save_hmac_state( struct s2n_connection_hmac_handles *hmac_handles, struct s2n_connection *conn )
{
    ...
    GUARD(
        s2n_hmac_save_evp_hash_state( &hmac_handles->initial_client_copy, &conn->initial.record_mac_copy_workspace ) );
...
int s2n_connection_restore_hmac_state( struct s2n_connection *conn, struct s2n_connection_hmac_handles *hmac_handles )
{
    ...
    GUARD( s2n_hmac_restore_evp_hash_state( &hmac_handles->initial_client_copy,
    ...
    GUARD(
        s2n_hmac_restore_evp_hash_state( &hmac_handles->secure_client_copy, &conn->secure.record_mac_copy_workspace ) );
    return 0;
}
