...
int s2n_record_parse_cbc(const struct s2n_cipher_suite *cipher_suite,
  ...
  struct s2n_blob iv = {
      .data = implicit_iv,
      .size = cipher_suite->record_alg->cipher->io.cbc.record_iv_size};
  ...
  lte_check(cipher_suite->record_alg->cipher->io.cbc.record_iv_size,
            S2N_TLS_MAX_IV_LEN);
  ...
  GUARD(cipher_suite->record_alg->cipher->io.cbc.decrypt(session_key, &iv, &en,
                                                         &en));
