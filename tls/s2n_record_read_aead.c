...
int s2n_record_parse_aead(const struct s2n_cipher_suite *cipher_suite,
                          struct s2n_connection *conn, uint8_t content_type,
                          uint16_t encrypted_length, uint8_t *implicit_iv,
                          struct s2n_hmac_state *mac, uint8_t *sequence_number,
                          struct s2n_session_key *session_key) {
  const int is_tls13_record =
      cipher_suite->record_alg->flags & S2N_TLS13_RECORD_AEAD_NONCE;
  /* TLS 1.3 record protection uses a different 5 byte associated data than
   * TLS 1.2's */
  ...
  struct s2n_blob en = {
      ...
      .data = s2n_stuffer_raw_read(&conn->in, encrypted_length)};
  ...
  if (cipher_suite->record_alg->flags & S2N_TLS12_AES_GCM_AEAD_NONCE) {
     ...
     *
     * In these cipher modes, the sequence number (64 bits) is left padded by 4
     *bytes to align and xor-ed with the 96-bit IV.
     **/
    ...
    for (int i = 0; i < cipher_suite->record_alg->cipher->io.aead.fixed_iv_size;
         i++) {
      S2N_INVARIENT(i <=
                    cipher_suite->record_alg->cipher->io.aead.fixed_iv_size);
  ...
  gte_check(payload_length,
            cipher_suite->record_alg->cipher->io.aead.record_iv_size +
                cipher_suite->record_alg->cipher->io.aead.tag_size);
  ...
  if (is_tls13_record) {
    ...
    GUARD(s2n_aead_aad_init(conn, sequence_number, content_type, payload_length,
                            &ad_stuffer));
  }
  ...
  GUARD(cipher_suite->record_alg->cipher->io.aead.decrypt(session_key, &iv,
                                                          &aad, &en, &en));
  struct s2n_blob seq = {.data = sequence_number,
                         .size = S2N_TLS_SEQUENCE_NUM_LEN};
  GUARD(s2n_increment_sequence_number(&seq));

  /* O.k., we've successfully read and decrypted the record, now we need to
  ...
  if (conn->actual_protocol_version >= S2N_TLS12) {
    GUARD(s2n_stuffer_skip_read(
        &conn->in, cipher_suite->record_alg->cipher->io.aead.record_iv_size));
  }

  /* Truncate and wipe the MAC and any padding */
  GUARD(s2n_stuffer_wipe_n(
      &conn->in, s2n_stuffer_data_available(&conn->in) - payload_length));
