...
int s2n_record_parse_aead( const struct s2n_cipher_suite *cipher_suite, struct s2n_connection *conn,
                           uint8_t content_type, uint16_t encrypted_length, uint8_t *implicit_iv,
                           struct s2n_hmac_state *mac, uint8_t *sequence_number, struct s2n_session_key *session_key )
{
    ...
    gte_check( payload_length, cipher_suite->record_alg->cipher->io.aead.record_iv_size
    ...
    if ( is_tls13_record ) {
        GUARD( s2n_tls13_aead_aad_init( payload_length, cipher_suite->record_alg->cipher->io.aead.tag_size,
                                        &ad_stuffer ) );
    } else {
