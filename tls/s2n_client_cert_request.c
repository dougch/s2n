...
static uint8_t s2n_cert_type_preference_list[] = {S2N_CERT_TYPE_RSA_SIGN,
                                                  S2N_CERT_TYPE_ECDSA_SIGN};

static int s2n_cert_type_to_pkey_type(s2n_cert_type cert_type_in,
...
static int s2n_recv_client_cert_preferences(
    struct s2n_stuffer *in, s2n_cert_type *chosen_cert_type_out) {
  ...
  /* Iterate through our preference list from most to least preferred, and
   * return the first match that we find. */
  for (int our_cert_pref_idx = 0;
       our_cert_pref_idx < sizeof(s2n_cert_type_preference_list);
       our_cert_pref_idx++) {
    for (int their_cert_idx = 0; their_cert_idx < cert_types_len;
         their_cert_idx++) {
      if (their_cert_type_pref_list[their_cert_idx] ==
          s2n_cert_type_preference_list[our_cert_pref_idx]) {
        *chosen_cert_type_out =
            s2n_cert_type_preference_list[our_cert_pref_idx];
        return 0;
      }
    }
...
static int s2n_set_cert_chain_as_client(struct s2n_connection *conn) {
  if (s2n_config_get_num_default_certs(conn->config) > 0) {
    GUARD(s2n_choose_sig_scheme_from_peer_preference_list(
        conn, &conn->handshake_params.server_sig_hash_algs,
        &conn->secure.client_cert_sig_scheme));

    struct s2n_cert_chain_and_key *cert =
        s2n_config_get_single_default_cert(conn->config);
    notnull_check(cert);
...
int s2n_client_cert_req_recv(struct s2n_connection *conn) {
  ...
  GUARD(s2n_cert_type_to_pkey_type(cert_type,
                                   &conn->secure.client_cert_pkey_type));

  if (conn->actual_protocol_version == S2N_TLS12) {
    GUARD(s2n_recv_supported_sig_scheme_list(
        in, &conn->handshake_params.server_sig_hash_algs));
  }
  ...
  /* In the future we may have more advanced logic to match a set of configured
   * certificates against The cert authorities extension and the signature
   * algorithms advertised. For now, this will just set the only certificate
   * configured.
   */
...
int s2n_client_cert_req_send(struct s2n_connection *conn) {
  ...
  uint8_t client_cert_preference_list_size =
      sizeof(s2n_cert_type_preference_list);
