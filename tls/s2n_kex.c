...
static int s2n_get_server_ecc_extension_size(
    const struct s2n_connection *conn) {
 ...
 * RFC 4492 section 5.2 states that the absence of this extension in the Server
 * Hello is equivalent to allowing only the uncompressed point format. Let's
 * send the extension in case clients(Openssl 1.0.0) don't honor the implied
 * behavior.
 */
static int s2n_write_server_ecc_extension(const struct s2n_connection *conn,
  ...
  if (s2n_server_can_send_ec_point_formats(conn)) {
    ...
    /* Only uncompressed format is supported. Interoperability shouldn't be an
     * issue: RFC 4492 Section 5.1.2: Implementations must support it for all of
     * their curves.
     */
...
static int s2n_write_no_extension(const struct s2n_connection *conn,
                                  struct s2n_stuffer *out) {
  return 0;
}

static int s2n_check_rsa_key(const struct s2n_cipher_suite *cipher_suite,
                             struct s2n_connection *conn) {
  return s2n_get_compatible_cert_chain_and_key(conn, S2N_PKEY_TYPE_RSA) != NULL;
}

static int s2n_check_dhe(const struct s2n_cipher_suite *cipher_suite,
                         struct s2n_connection *conn) {
  return conn->config->dhparams != NULL;
}

static int s2n_check_ecdhe(const struct s2n_cipher_suite *cipher_suite,
                           struct s2n_connection *conn) {
  return conn->secure.server_ecc_evp_params.negotiated_curve != NULL;
}

static int s2n_check_kem(const struct s2n_cipher_suite *cipher_suite,
                         struct s2n_connection *conn) {
  ...
  if (s2n_connection_get_cipher_preferences(conn, &server_cipher_preferences) !=
      0) {
    return 0;
  }
  if (server_cipher_preferences->kem_count == 0) {
    return 0;
  }

  const struct s2n_iana_to_kem *supported_params = NULL;
  /* If the cipher suite has no supported KEMs return false */
  if (s2n_cipher_suite_to_kem(cipher_suite->iana_value, &supported_params) !=
      0) {
    return 0;
  }
  ...
  struct s2n_blob *client_kem_pref_list =
      &(conn->secure.client_pq_kem_extension);
  const struct s2n_kem *chosen_kem = NULL;
  if (client_kem_pref_list == NULL || client_kem_pref_list->data == NULL) {
    /* If the client did not send a PQ KEM extension, then the server can pick
     * its preferred parameter */
    if (s2n_choose_kem_without_peer_pref_list(
            cipher_suite->iana_value, server_cipher_preferences->kems,
            server_cipher_preferences->kem_count, &chosen_kem) != 0) {
      return 0;
    }
  } else {
    /* If the client did send a PQ KEM extension, then the server must find a
     * mutually supported parameter. */
    if (s2n_choose_kem_with_peer_pref_list(
            cipher_suite->iana_value, client_kem_pref_list,
            server_cipher_preferences->kems,
            server_cipher_preferences->kem_count, &chosen_kem) != 0) {
      return 0;
    }
  }
...
static int s2n_configure_kem(const struct s2n_cipher_suite *cipher_suite,
                             struct s2n_connection *conn) {
  /* There is no support for PQ KEMs while in FIPS mode */
  ...
  if (proposed_kems == NULL || proposed_kems->data == NULL) {
    /* If the client did not send a PQ KEM extension, then the server can pick
     * its preferred parameter */
    GUARD(s2n_choose_kem_without_peer_pref_list(
        cipher_suite->iana_value, cipher_preferences->kems,
        cipher_preferences->kem_count, &chosen_kem));
  } else {
    /* If the client did send a PQ KEM extension, then the server must find a
     * mutually supported parameter. */
    GUARD(s2n_choose_kem_with_peer_pref_list(
        cipher_suite->iana_value, proposed_kems, cipher_preferences->kems,
        cipher_preferences->kem_count, &chosen_kem));
  }
...
static int s2n_no_op_configure(const struct s2n_cipher_suite *cipher_suite,
                               struct s2n_connection *conn) {
  return 0;
}

static int s2n_check_hybrid_echde_kem(
    const struct s2n_cipher_suite *cipher_suite, struct s2n_connection *conn) {
  return s2n_check_ecdhe(cipher_suite, conn) &&
         s2n_check_kem(cipher_suite, conn);
}

static int s2n_write_server_hybrid_extensions(const struct s2n_connection *conn,
                                              struct s2n_stuffer *out) {
...
static int s2n_get_server_hybrid_extensions_size(
    const struct s2n_connection *conn) {
  const struct s2n_kex *kex = conn->secure.cipher_suite->key_exchange_alg;
  const struct s2n_kex *hybrid_kex_0 = kex->hybrid[0];
  const struct s2n_kex *hybrid_kex_1 = kex->hybrid[1];
  return s2n_kex_server_extension_size(hybrid_kex_0, conn) +
         s2n_kex_server_extension_size(hybrid_kex_1, conn);
}
...
int s2n_kex_write_server_extension(const struct s2n_kex *kex,
                                   const struct s2n_connection *conn,
                                   struct s2n_stuffer *out) {
  notnull_check(kex);
...
int s2n_kex_supported(const struct s2n_cipher_suite *cipher_suite,
                      struct s2n_connection *conn) {
  /* Don't return -1 from notnull_check because that might allow a improperly
   * configured kex to be marked as "supported" */
  return cipher_suite->key_exchange_alg->connection_supported != NULL &&
         cipher_suite->key_exchange_alg->connection_supported(cipher_suite,
                                                              conn);
}

int s2n_configure_kex(const struct s2n_cipher_suite *cipher_suite,
                      struct s2n_connection *conn) {
  notnull_check(cipher_suite->key_exchange_alg->configure_connection);
  return cipher_suite->key_exchange_alg->configure_connection(cipher_suite,
                                                              conn);
}
...
int s2n_kex_server_key_recv_parse_data(
    const struct s2n_kex *kex, struct s2n_connection *conn,
...
int s2n_kex_client_key_send(const struct s2n_kex *kex,
                            struct s2n_connection *conn,
                            struct s2n_blob *shared_key) {
...
int s2n_kex_tls_prf(const struct s2n_kex *kex, struct s2n_connection *conn,
