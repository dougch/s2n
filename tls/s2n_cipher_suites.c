...
const struct s2n_record_algorithm s2n_record_alg_chacha20_poly1305 = {
    ...
    .hmac_alg = S2N_HMAC_NONE,
    /* Per RFC 7905, ChaCha20-Poly1305 will use a nonce construction expected to
     * be used in TLS1.3. Give it a distinct 1.2 nonce value in case this
     * changes.
     */
...
const struct s2n_record_algorithm s2n_tls13_record_alg_aes128_gcm = {
    ...
    .hmac_alg =
        S2N_HMAC_NONE, /* previously used in 1.2 prf, we do not need this */
...
const struct s2n_record_algorithm s2n_tls13_record_alg_chacha20_poly1305 = {
    .cipher = &s2n_chacha20_poly1305,
    .hmac_alg = S2N_HMAC_NONE,
    /* this mirrors s2n_record_alg_chacha20_poly1305 with the exception of
       TLS 1.3 nonce flag */
    .flags = S2N_TLS13_RECORD_AEAD_NONCE,
};
...
struct s2n_cipher_suite s2n_rsa_with_aes_128_cbc_sha = /* 0x00,0x2F */ {
    ...
    .auth_method = S2N_AUTHENTICATION_RSA,
    .record_alg = NULL,
    .all_record_algs = {&s2n_record_alg_aes128_sha_composite,
                        &s2n_record_alg_aes128_sha},
    .num_record_algs = 2,
...
struct s2n_cipher_suite s2n_rsa_with_aes_256_cbc_sha = /* 0x00,0x35 */ {
    ...
    .auth_method = S2N_AUTHENTICATION_RSA,
    .record_alg = NULL,
    .all_record_algs = {&s2n_record_alg_aes256_sha_composite,
                        &s2n_record_alg_aes256_sha},
    .num_record_algs = 2,
...
struct s2n_cipher_suite s2n_rsa_with_aes_128_cbc_sha256 = /* 0x00,0x3C */ {
    ...
    .auth_method = S2N_AUTHENTICATION_RSA,
    .record_alg = NULL,
    .all_record_algs = {&s2n_record_alg_aes128_sha256_composite,
                        &s2n_record_alg_aes128_sha256},
    .num_record_algs = 2,
...
struct s2n_cipher_suite s2n_rsa_with_aes_256_cbc_sha256 = /* 0x00,0x3D */ {
    ...
    .auth_method = S2N_AUTHENTICATION_RSA,
    .record_alg = NULL,
    .all_record_algs = {&s2n_record_alg_aes256_sha256_composite,
                        &s2n_record_alg_aes256_sha256},
    .num_record_algs = 2,
    .sslv3_record_alg = NULL,
    .prf_alg = S2N_HMAC_SHA256,
    .minimum_required_tls_version = S2N_TLS12,
...
struct s2n_cipher_suite s2n_ecdhe_ecdsa_with_aes_128_cbc_sha256 =
    /* 0xC0,0x23 */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_ecdhe_ecdsa_with_aes_256_cbc_sha384 =
    /* 0xC0,0x24 */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_ecdhe_rsa_with_aes_128_cbc_sha256 =
    /* 0xC0,0x27 */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_ecdhe_rsa_with_aes_256_cbc_sha384 =
    /* 0xC0,0x28 */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_ecdhe_ecdsa_with_aes_128_gcm_sha256 =
    /* 0xC0,0x2B */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_ecdhe_ecdsa_with_aes_256_gcm_sha384 =
    /* 0xC0,0x2C */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_ecdhe_rsa_with_aes_128_gcm_sha256 =
    /* 0xC0,0x2F */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_ecdhe_rsa_with_aes_256_gcm_sha384 =
    /* 0xC0,0x30 */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_ecdhe_rsa_with_chacha20_poly1305_sha256 =
    /* 0xCC,0xA8 */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_ecdhe_ecdsa_with_chacha20_poly1305_sha256 =
    /* 0xCC,0xA9 */ {
        .available = 0,
...
struct s2n_cipher_suite s2n_dhe_rsa_with_chacha20_poly1305_sha256 =
    /* 0xCC,0xAA */ {
        .available = 0,
 ...
 * in order of IANA value. Exposed for the "test_all_fips" cipher preference
 * list.
 */
...
/* All ECDSA cipher suites first, then the rest of the supported ciphers that
 * s2n can negotiate. Exposed for the "test_ecdsa_priority" cipher preference
 * list.
 */
...
static struct s2n_cipher_suite
    *s2n_tls13_null_key_exchange_alg_cipher_suites[] = {
        &s2n_tls13_aes_128_gcm_sha256,       /* 0x13,0x01 */
        &s2n_tls13_aes_256_gcm_sha384,       /* 0x13,0x02 */
        &s2n_tls13_chacha20_poly1305_sha256, /* 0x13,0x03 */
};
...
const struct s2n_cipher_preferences
    cipher_preferences_test_tls13_null_key_exchange_alg = {
...
int s2n_cipher_suites_init(void) {
  ...
  for (int i = 0; i < num_cipher_suites; i++) {
    ...
    for (int j = 0; j < cur_suite->num_record_algs; j++) {
      /* Can we use the record algorithm's cipher? Won't be available if the
       * system CPU architecture doesn't support it or if the libcrypto lacks
       * the feature. All hmac_algs are supported.
       */
    ...
    /* Initialize SSLv3 cipher suite if SSLv3 utilizes a different record
     * algorithm */
    if (cur_suite->sslv3_record_alg &&
        cur_suite->sslv3_record_alg->cipher->is_available()) {
      struct s2n_blob cur_suite_mem = {.data = (uint8_t *)cur_suite,
                                       .size = sizeof(struct s2n_cipher_suite)};
      struct s2n_blob new_suite_mem = {0};
      GUARD(s2n_dup(&cur_suite_mem, &new_suite_mem));

      struct s2n_cipher_suite *new_suite =
          (struct s2n_cipher_suite *)(void *)new_suite_mem.data;
...
int s2n_cipher_suites_cleanup(void) {
  const int num_cipher_suites =
      sizeof(s2n_all_cipher_suites) / sizeof(struct s2n_cipher_suite *);
  for (int i = 0; i < num_cipher_suites; i++) {
    struct s2n_cipher_suite *cur_suite = s2n_all_cipher_suites[i];
    cur_suite->available = 0;
    cur_suite->record_alg = NULL;
    ...
    if (cur_suite->sslv3_cipher_suite != cur_suite) {
      GUARD(s2n_free_object((uint8_t **)&cur_suite->sslv3_cipher_suite,
                            sizeof(struct s2n_cipher_suite)));
    }
  ...
  /* per the reqs here
   * https://www.openssl.org/docs/man1.1.0/crypto/OPENSSL_init_crypto.html we
   * don't explicitly call cleanup in later versions */
#endif

  return 0;
}

struct s2n_cipher_suite *s2n_cipher_suite_from_wire(
    const uint8_t cipher_suite[S2N_TLS_CIPHER_SUITE_LEN]) {
  int low = 0;
  int top =
      (sizeof(s2n_all_cipher_suites) / sizeof(struct s2n_cipher_suite *)) - 1;
...
int s2n_set_cipher_as_client(struct s2n_connection *conn,
                             uint8_t wire[S2N_TLS_CIPHER_SUITE_LEN]) {
...
static int s2n_wire_ciphers_contain(const uint8_t *match, const uint8_t *wire,
                                    uint32_t count, uint32_t cipher_suite_len) {
  for (int i = 0; i < count; i++) {
    const uint8_t *theirs = wire + (i * cipher_suite_len) +
                            (cipher_suite_len - S2N_TLS_CIPHER_SUITE_LEN);
...
static int s2n_set_cipher_as_server(struct s2n_connection *conn, uint8_t *wire,
                                    uint32_t count, uint32_t cipher_suite_len) {
  uint8_t renegotiation_info_scsv[S2N_TLS_CIPHER_SUITE_LEN] = {
      TLS_EMPTY_RENEGOTIATION_INFO_SCSV};
  struct s2n_cipher_suite *higher_vers_match = NULL;

  /* RFC 7507 - If client is attempting to negotiate a TLS Version that is lower
   * than the highest supported server version, and the client cipher list
   * contains TLS_FALLBACK_SCSV, then the server must abort the connection since
   * TLS_FALLBACK_SCSV should only be present when the client previously failed
   * to negotiate a higher TLS version.
   */
  if (conn->client_protocol_version < s2n_highest_protocol_version) {
    ...
    if (s2n_wire_ciphers_contain(fallback_scsv, wire, count,
                                 cipher_suite_len)) {
      conn->closed = 1;
  ...
  /* RFC5746 Section 3.6: A server must check if
   * TLS_EMPTY_RENEGOTIATION_INFO_SCSV is included */
  if (s2n_wire_ciphers_contain(renegotiation_info_scsv, wire, count,
                               cipher_suite_len)) {
  ...
  for (int i = 0; i < cipher_preferences->count; i++) {
    ...
    if (s2n_wire_ciphers_contain(ours, wire, count, cipher_suite_len)) {
      ...
      /* Don't immediately choose a cipher the client shouldn't be able to
       * support */
...
int s2n_set_cipher_as_sslv2_server(struct s2n_connection *conn, uint8_t *wire,
                                   uint16_t count) {
  return s2n_set_cipher_as_server(conn, wire, count,
                                  S2N_SSLv2_CIPHER_SUITE_LEN);
}

int s2n_set_cipher_as_tls_server(struct s2n_connection *conn, uint8_t *wire,
                                 uint16_t count) {
