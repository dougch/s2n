...
static int s2n_wire_ciphers_contain( const uint8_t *match, const uint8_t *wire, uint32_t count,
                                     uint32_t cipher_suite_len )
{
    for ( int i = 0; i < count; i++ ) {
        const uint8_t *theirs = wire + ( i * cipher_suite_len ) + ( cipher_suite_len - S2N_TLS_CIPHER_SUITE_LEN );

        if ( !memcmp( match, theirs, S2N_TLS_CIPHER_SUITE_LEN ) ) { return 1; }
    }

    return 0;
}

static int s2n_set_cipher_as_server( struct s2n_connection *conn, uint8_t *wire, uint32_t count,
                                     uint32_t cipher_suite_len )
{
    ...
    for ( int i = 0; i < security_policy->cipher_preferences->count; i++ ) {
        ...
        if ( conn->actual_protocol_version >= S2N_TLS13 && !s2n_is_valid_tls13_cipher( ours ) ) { continue; }
        ...
        if ( s2n_wire_ciphers_contain( ours, wire, count, cipher_suite_len ) ) {
            ...
            if ( conn->client_protocol_version == S2N_SSLv3 ) { match = match->sslv3_cipher_suite; }
            ...
            if ( match->minimum_required_tls_version > conn->server_protocol_version ) { continue; }

            /* Make sure the cipher is valid for available certs */
            if ( s2n_is_cipher_suite_valid_for_auth( conn, match ) != S2N_SUCCESS ) { continue; }
            ...
            if ( match->minimum_required_tls_version < S2N_TLS13 ) {
                ...
                if ( !s2n_kex_supported( match, conn ) ) { continue; }

                /* If the kex is not configured correctly continue to the next candidate */
                if ( s2n_configure_kex( match, conn ) ) { continue; }
            }
            ...
            if ( conn->client_protocol_version < match->minimum_required_tls_version ) {
                if ( !higher_vers_match ) { higher_vers_match = match; }
                continue;
            }
