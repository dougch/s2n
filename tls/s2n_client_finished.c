...
int s2n_client_finished_recv(struct s2n_connection *conn) {
  ...
  uint8_t *their_version =
      s2n_stuffer_raw_read(&conn->handshake.io, S2N_TLS_FINISHED_LEN);
  notnull_check(their_version);

  S2N_ERROR_IF(!s2n_constant_time_equals(our_version, their_version,
                                         S2N_TLS_FINISHED_LEN) ||
...
int s2n_client_finished_send(struct s2n_connection *conn) {
  ...
  if (conn->actual_protocol_version == S2N_SSLv3) {
    GUARD(s2n_stuffer_write_bytes(&conn->handshake.io, our_version,
                                  S2N_SSL_FINISHED_LEN));
  } else {
    GUARD(s2n_stuffer_write_bytes(&conn->handshake.io, our_version,
                                  S2N_TLS_FINISHED_LEN));
  }
...
int s2n_tls13_client_finished_recv(struct s2n_connection *conn) {
  ...
  s2n_blob_init(&wire_finished_mac,
                s2n_stuffer_raw_read(&conn->handshake.io, length), length);
  ...
  GUARD(
      s2n_blob_init(&finished_key, conn->handshake.client_finished, keys.size));

  s2n_tls13_key_blob(client_finished_mac, keys.size);
  GUARD(s2n_tls13_calculate_finished_mac(&keys, &finished_key, &hash_state,
                                         &client_finished_mac));
