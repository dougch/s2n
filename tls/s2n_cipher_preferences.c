...
#define S2N_TLS13_CIPHER_SUITES_20190801                        \
  &s2n_tls13_aes_256_gcm_sha384, &s2n_tls13_aes_128_gcm_sha256, \
...
/* Extension list for SIKE P434 Round 2 and SIKE P503 Round 1 only (for
 * testing), in order of preference */
...
struct s2n_cipher_suite *cipher_suites_20150202[] = {
    &s2n_dhe_rsa_with_aes_128_cbc_sha256, &s2n_dhe_rsa_with_aes_128_cbc_sha,
...
struct s2n_cipher_suite *cipher_suites_20150214[] = {
    ...
    &s2n_dhe_rsa_with_aes_128_cbc_sha,    &s2n_dhe_rsa_with_3des_ede_cbc_sha,
...
/* Use ECDHE instead of plain DHE. Prioritize ECDHE in favour of non ECDHE; GCM
 * in favour of CBC; AES128 in favour of AES256. */
...
/* Preferences optimized for interop, includes ECDSA priortitized. DHE and 3DES
 * are added(at the lowest preference). */
...
/* Preferences optimized for interop. DHE and 3DES are added(at the lowest
 * preference). */
...
struct s2n_cipher_suite
    *cipher_suites_elb_security_policy_tls_1_2_ext_2018_06[] = {
        &s2n_ecdhe_ecdsa_with_aes_128_gcm_sha256,
        &s2n_ecdhe_rsa_with_aes_128_gcm_sha256,
        &s2n_ecdhe_ecdsa_with_aes_128_cbc_sha256,
        &s2n_ecdhe_rsa_with_aes_128_cbc_sha256,
...
const struct s2n_cipher_preferences elb_security_policy_tls_1_2_ext_2018_06 = {
    .count =
        s2n_array_len(cipher_suites_elb_security_policy_tls_1_2_ext_2018_06),
...
struct s2n_cipher_suite
    *cipher_suites_elb_security_policy_fs_1_2_Res_2019_08[] = {
        &s2n_ecdhe_ecdsa_with_aes_128_gcm_sha256,
        &s2n_ecdhe_rsa_with_aes_128_gcm_sha256,
        &s2n_ecdhe_ecdsa_with_aes_128_cbc_sha256,
        &s2n_ecdhe_rsa_with_aes_128_cbc_sha256,
...
const struct s2n_cipher_preferences elb_security_policy_fs_1_2_Res_2019_08 = {
    .count =
        s2n_array_len(cipher_suites_elb_security_policy_fs_1_2_Res_2019_08),
...
struct s2n_cipher_suite *cipher_suites_cloudfront_tls_1_2_2019[] = {
    &s2n_ecdhe_rsa_with_aes_128_gcm_sha256,
    &s2n_ecdhe_rsa_with_aes_256_gcm_sha384, &s2n_rsa_with_aes_128_gcm_sha256,
    &s2n_rsa_with_aes_256_gcm_sha384};
...
/* Includes only SIKE round 1 and round 2 (for integration tests). The cipher
 * suite list
 * is the same as in cipher_preferences_pq_sike_test_tls_1_0_2019_11. */
const struct s2n_cipher_preferences
    cipher_preferences_pq_sike_test_tls_1_0_2020_02 = {
        .count = s2n_array_len(cipher_suites_pq_sike_test_tls_1_0_2019_11),
        .suites = cipher_suites_pq_sike_test_tls_1_0_2019_11,
        .minimum_protocol_version = S2N_TLS10,
...
const struct s2n_cipher_preferences
    cipher_preferences_kms_fips_tls_1_2_2018_10 = {
...
struct {
  ...
  unsigned pq_kem_extension_required : 1;
    ...
    /* Zero init .ecc_extension_required and .pq_kem_extension_required, they'll
       be initialized later in s2n_cipher_preferences_init() */
    {.version = "default",
    ...
    {.version = "test_ecdsa_priority",
     ...
     .ecc_extension_required = 0,
     .pq_kem_extension_required = 0},
    {.version = "test_tls13_null_key_exchange_alg",
     .preferences = &cipher_preferences_test_tls13_null_key_exchange_alg,
     .ecc_extension_required = 0,
     .pq_kem_extension_required = 0},
    {.version = "null",
...
int s2n_find_cipher_pref_from_version(
    const char *version,
...
int s2n_config_set_cipher_preferences(struct s2n_config *config,
                                      const char *version) {
  GUARD(
      s2n_find_cipher_pref_from_version(version, &config->cipher_preferences));
  return 0;
}

int s2n_connection_set_cipher_preferences(struct s2n_connection *conn,
                                          const char *version) {
  GUARD(
      s2n_find_cipher_pref_from_version(version, &conn->cipher_pref_override));
  return 0;
}

int s2n_connection_is_valid_for_cipher_preferences(struct s2n_connection *conn,
                                                   const char *version) {
  notnull_check(conn);
  notnull_check(version);
  ...
  /* make sure we dont use a tls version lower than that configured by the
   * version */
  if (s2n_connection_get_actual_protocol_version(conn) <
      preferences->minimum_protocol_version) {
    return 0;
  }

  struct s2n_cipher_suite *cipher = conn->secure.cipher_suite;
  for (int i = 0; i < preferences->count; ++i) {
    if (0 == memcmp(preferences->suites[i]->iana_value, cipher->iana_value,
...
int s2n_cipher_preferences_init() {
  for (int i = 0; selection[i].version != NULL; i++) {
    ...
    for (int j = 0; j < preferences->count; j++) {
      ...
      if (cipher->key_exchange_alg == &s2n_ecdhe ||
          cipher->key_exchange_alg == &s2n_hybrid_ecdhe_kem) {
        selection[i].ecc_extension_required = 1;
      }
    ...
    if (selection[i].pq_kem_extension_required == 1) {
      S2N_ERROR_IF(preferences->kem_count == 0,
                   S2N_ERR_INVALID_CIPHER_PREFERENCES);
      S2N_ERROR_IF(preferences->kems == NULL,
                   S2N_ERR_INVALID_CIPHER_PREFERENCES);
    } else {
      S2N_ERROR_IF(preferences->kem_count != 0,
                   S2N_ERR_INVALID_CIPHER_PREFERENCES);
...
int s2n_ecc_extension_required(
    const struct s2n_cipher_preferences *preferences) {
...
int s2n_pq_kem_extension_required(
    const struct s2n_cipher_preferences *preferences) {
  notnull_check(preferences);
  for (int i = 0; selection[i].version != NULL; i++) {
    if (selection[i].preferences == preferences) {
