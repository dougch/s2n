...
typedef int s2n_kex_client_key_method(const struct s2n_kex *kex,
                                      struct s2n_connection *conn,
                                      struct s2n_blob *shared_key);
typedef void *s2n_stuffer_action(struct s2n_stuffer *stuffer,
                                 uint32_t data_len);
static int s2n_hybrid_client_action(struct s2n_connection *conn,
                                    struct s2n_blob *combined_shared_key,
                                    s2n_kex_client_key_method kex_method,
                                    uint32_t *cursor,
                                    s2n_stuffer_action stuffer_action) {
  ...
  /* Keep a copy to the start of the entire hybrid client key exchange message
   * for the hybrid PRF */
...
static int calculate_keys(struct s2n_connection *conn,
                          struct s2n_blob *shared_key) {
  /* Turn the pre-master secret into a master secret */
  GUARD(s2n_kex_tls_prf(conn->secure.cipher_suite->key_exchange_alg, conn,
                        shared_key));
...
int s2n_rsa_client_key_recv(struct s2n_connection *conn,
  ...
  struct s2n_blob encrypted = {.size = length,
                               .data = s2n_stuffer_raw_read(in, length)};
  ...
  conn->handshake.rsa_failed =
      !!s2n_pkey_decrypt(conn->handshake_params.our_chain_and_key->private_key,
                         &encrypted, shared_key);

  /* Set rsa_failed to 1, if it isn't already, if the protocol version isn't
   * what we expect */
  conn->handshake.rsa_failed |= !s2n_constant_time_equals(
      client_protocol_version, shared_key->data, S2N_TLS_PROTOCOL_VERSION_LEN);
  return 0;
}

int s2n_dhe_client_key_recv(struct s2n_connection *conn,
                            struct s2n_blob *shared_key) {
  struct s2n_stuffer *in = &conn->handshake.io;

  /* Get the shared key */
  GUARD(s2n_dh_compute_shared_secret_as_server(&conn->secure.server_dh_params,
                                               in, shared_key));
...
int s2n_ecdhe_client_key_recv(struct s2n_connection *conn,
                              struct s2n_blob *shared_key) {
  struct s2n_stuffer *in = &conn->handshake.io;

  /* Get the shared key */
  GUARD(s2n_ecc_evp_compute_shared_secret_as_server(
...
int s2n_kem_client_key_recv(struct s2n_connection *conn,
                            struct s2n_blob *shared_key) {
  struct s2n_stuffer *in = &conn->handshake.io;
  kem_ciphertext_key_size ciphertext_length;

  GUARD(s2n_stuffer_read_uint16(in, &ciphertext_length));
  S2N_ERROR_IF(ciphertext_length > s2n_stuffer_data_available(in),
               S2N_ERR_BAD_MESSAGE);

  const struct s2n_blob ciphertext = {
      .size = ciphertext_length,
      .data = s2n_stuffer_raw_read(in, ciphertext_length)};
  notnull_check(ciphertext.data);

  GUARD(
      s2n_kem_decapsulate(&conn->secure.s2n_kem_keys, shared_key, &ciphertext));

  GUARD(s2n_kem_free(&conn->secure.s2n_kem_keys));
  return 0;
}

int s2n_hybrid_client_key_recv(struct s2n_connection *conn,
                               struct s2n_blob *combined_shared_key) {
  return s2n_hybrid_client_action(
      conn, combined_shared_key, &s2n_kex_client_key_recv,
      &conn->handshake.io.read_cursor, &s2n_stuffer_raw_read);
}
...
int s2n_dhe_client_key_send(struct s2n_connection *conn,
                            struct s2n_blob *shared_key) {
  struct s2n_stuffer *out = &conn->handshake.io;
  GUARD(s2n_dh_compute_shared_secret_as_client(&conn->secure.server_dh_params,
                                               out, shared_key));
...
int s2n_ecdhe_client_key_send(struct s2n_connection *conn,
                              struct s2n_blob *shared_key) {
  struct s2n_stuffer *out = &conn->handshake.io;
  GUARD(s2n_ecc_evp_compute_shared_secret_as_client(
      &conn->secure.server_ecc_evp_params, out, shared_key));
...
int s2n_rsa_client_key_send(struct s2n_connection *conn,
                            struct s2n_blob *shared_key) {
  uint8_t client_protocol_version[S2N_TLS_PROTOCOL_VERSION_LEN];
  client_protocol_version[0] = conn->client_protocol_version / 10;
  client_protocol_version[1] = conn->client_protocol_version % 10;
  ...
  /* Over-write the first two bytes with the client protocol version, per
   * RFC2246 7.4.7.1 */
  memcpy_check(conn->secure.rsa_premaster_secret, client_protocol_version,
               S2N_TLS_PROTOCOL_VERSION_LEN);

  int encrypted_size = s2n_pkey_size(&conn->secure.server_public_key);
  S2N_ERROR_IF(encrypted_size < 0 || encrypted_size > 0xffff,
  ...
  GUARD(s2n_pkey_encrypt(&conn->secure.server_public_key, shared_key,
                         &encrypted));
...
int s2n_kem_client_key_send(struct s2n_connection *conn,
                            struct s2n_blob *shared_key) {
  struct s2n_stuffer *out = &conn->handshake.io;
  ...
  /* The ciphertext is not needed after this method, write it straight to the
   * stuffer */
  struct s2n_blob ciphertext = {
      .data = s2n_stuffer_raw_write(out, kem->ciphertext_length),
      .size = kem->ciphertext_length};
  notnull_check(ciphertext.data);

  GUARD(
      s2n_kem_encapsulate(&conn->secure.s2n_kem_keys, shared_key, &ciphertext));
  GUARD(s2n_kem_free(&conn->secure.s2n_kem_keys));
  return 0;
}

int s2n_hybrid_client_key_send(struct s2n_connection *conn,
                               struct s2n_blob *combined_shared_key) {
  return s2n_hybrid_client_action(
      conn, combined_shared_key, &s2n_kex_client_key_send,
      &conn->handshake.io.write_cursor, s2n_stuffer_raw_write);
}
