...
static int s2n_connection_init_hmacs(struct s2n_connection *conn) {
  ...
  GUARD(
      s2n_hmac_init(&conn->initial.client_record_mac, S2N_HMAC_NONE, NULL, 0));
  GUARD(
      s2n_hmac_init(&conn->initial.server_record_mac, S2N_HMAC_NONE, NULL, 0));
  GUARD(s2n_hmac_init(&conn->initial.record_mac_copy_workspace, S2N_HMAC_NONE,
                      NULL, 0));
  ...
  GUARD(s2n_hmac_init(&conn->secure.record_mac_copy_workspace, S2N_HMAC_NONE,
                      NULL, 0));
...
static int s2n_connection_wipe_keys(struct s2n_connection *conn) {
   ...
   * keys are allocated. */
  if (conn->secure.cipher_suite && conn->secure.cipher_suite->record_alg &&
      conn->secure.cipher_suite->record_alg->cipher &&
      conn->secure.cipher_suite->record_alg->cipher->destroy_key) {
    GUARD(conn->secure.cipher_suite->record_alg->cipher->destroy_key(
...
static int s2n_connection_free_io_contexts(struct s2n_connection *conn) {
  /* Free the I/O context if it was allocated by s2n. Don't touch
   * user-controlled contexts. */
  if (!conn->managed_io) {
    return 0;
  }

  GUARD(s2n_free_object((uint8_t **)&conn->send_io_context,
                        sizeof(struct s2n_socket_write_io_context)));
  GUARD(s2n_free_object((uint8_t **)&conn->recv_io_context,
                        sizeof(struct s2n_socket_read_io_context)));
...
static uint8_t s2n_default_verify_host(const char *host_name, size_t len,
                                       void *data) {
...
int s2n_connection_set_config(struct s2n_connection *conn,
  ...
  if (s2n_config_get_num_default_certs(config) > 1 &&
  ...
  int8_t dont_need_x509_validation =
      (conn->mode == S2N_SERVER) && (auth_type == S2N_CERT_AUTH_NONE);
  ...
  if (config->disable_x509_validation || dont_need_x509_validation) {
    ...
    GUARD(s2n_x509_validator_init(&conn->x509_validator, &config->trust_store,
                                  config->check_ocsp));
    ...
    if (config->max_verify_cert_chain_depth_set) {
      GUARD(s2n_x509_validator_set_max_chain_depth(
          &conn->x509_validator, config->max_verify_cert_chain_depth));
    }
  }
...
int s2n_connection_wipe(struct s2n_connection *conn) {
  ...
  /* Parts of the PRF working space, hash states, and hmac states  will be
   * wiped. Preserve structs to avoid reallocation */
  ...
  /* Truncate the message buffers to save memory, we will dynamically resize it
   * as needed */
  GUARD(s2n_stuffer_resize(&conn->client_hello.raw_message, 0));
...
#pragma GCC diagnostic ignored "-Waddress"
  ...
  memcpy_check(&reader_alert_out, &conn->reader_alert_out,
               sizeof(struct s2n_stuffer));
  memcpy_check(&writer_alert_out, &conn->writer_alert_out,
               sizeof(struct s2n_stuffer));
  memcpy_check(&client_ticket_to_decrypt, &conn->client_ticket_to_decrypt,
               sizeof(struct s2n_stuffer));
  memcpy_check(&handshake_io, &conn->handshake.io, sizeof(struct s2n_stuffer));
  memcpy_check(&client_hello_raw_message, &conn->client_hello.raw_message,
               sizeof(struct s2n_stuffer));
  ...
  memcpy_check(&initial_client_key, &conn->initial.client_key,
               sizeof(struct s2n_session_key));
  memcpy_check(&initial_server_key, &conn->initial.server_key,
               sizeof(struct s2n_session_key));
  memcpy_check(&secure_client_key, &conn->secure.client_key,
               sizeof(struct s2n_session_key));
  memcpy_check(&secure_server_key, &conn->secure.server_key,
               sizeof(struct s2n_session_key));
...
#pragma GCC diagnostic pop
  ...
  memcpy_check(&conn->reader_alert_out, &reader_alert_out,
               sizeof(struct s2n_stuffer));
  memcpy_check(&conn->writer_alert_out, &writer_alert_out,
               sizeof(struct s2n_stuffer));
  memcpy_check(&conn->client_ticket_to_decrypt, &client_ticket_to_decrypt,
               sizeof(struct s2n_stuffer));
  memcpy_check(&conn->handshake.io, &handshake_io, sizeof(struct s2n_stuffer));
  memcpy_check(&conn->client_hello.raw_message, &client_hello_raw_message,
               sizeof(struct s2n_stuffer));
  ...
  memcpy_check(&conn->initial.client_key, &initial_client_key,
               sizeof(struct s2n_session_key));
  memcpy_check(&conn->initial.server_key, &initial_server_key,
               sizeof(struct s2n_session_key));
  memcpy_check(&conn->secure.client_key, &secure_client_key,
               sizeof(struct s2n_session_key));
  memcpy_check(&conn->secure.server_key, &secure_server_key,
               sizeof(struct s2n_session_key));
  ...
  /* Require all handshakes hashes. This set can be reduced as the handshake
   * progresses. */
  GUARD(s2n_handshake_require_all_hashes(&conn->handshake));

  if (conn->mode == S2N_SERVER) {
    /* Start with the highest protocol version so that the highest common
     * protocol version can be selected */
    /* during handshake. */
    ...
    /* For clients, also set actual_protocol_version.  Record generation uses
     * that value for the initial */
    /* ClientHello record version. Not all servers ignore the record version in
     * ClientHello. */
...
int s2n_connection_get_client_cert_chain(struct s2n_connection *conn,
...
int s2n_connection_get_protocol_preferences(
    struct s2n_connection *conn, struct s2n_blob **protocol_preferences) {
  notnull_check(conn);
...
int s2n_connection_get_client_auth_type(
    struct s2n_connection *conn, s2n_cert_auth_type *client_cert_auth_type) {
  notnull_check(conn);
...
int s2n_connection_set_client_auth_type(
    struct s2n_connection *conn, s2n_cert_auth_type client_cert_auth_type) {
...
int s2n_connection_get_alert(struct s2n_connection *conn) {
  ...
  S2N_ERROR_IF(s2n_stuffer_data_available(&conn->alert_in) != 2,
               S2N_ERR_NO_ALERT);
...
const char *s2n_get_server_name(struct s2n_connection *conn) {
  ...
  GUARD_PTR(s2n_client_hello_get_parsed_extension(
      conn->client_hello.parsed_extensions, S2N_EXTENSION_SERVER_NAME,
      &parsed_extension));
...
int s2n_connection_get_session_id(struct s2n_connection *conn,
                                  uint8_t *session_id, size_t max_length) {
  notnull_check(conn);
...
int s2n_connection_set_blinding(struct s2n_connection *conn,
...
int s2n_connection_kill(struct s2n_connection *conn) {
  ...
  if (conn->blinding == S2N_BUILT_IN_BLINDING) {
    struct timespec sleep_time = {.tv_sec = conn->delay / ONE_S,
                                  .tv_nsec = conn->delay % ONE_S};
    int r;
...
int s2n_connection_set_dynamic_record_threshold(struct s2n_connection *conn,
                                                uint32_t resize_threshold,
                                                uint16_t timeout_threshold) {
  notnull_check(conn);
  S2N_ERROR_IF(resize_threshold > S2N_TLS_MAX_RESIZE_THRESHOLD,
...
int s2n_connection_recv_stuffer(struct s2n_stuffer *stuffer,
                                struct s2n_connection *conn, uint32_t len) {
  ...
  do {
    errno = 0;
    r = conn->recv(conn->recv_io_context,
                   stuffer->blob.data + stuffer->write_cursor, len);
...
int s2n_connection_send_stuffer(struct s2n_stuffer *stuffer,
                                struct s2n_connection *conn, uint32_t len) {
  notnull_check(conn);
  ...
  S2N_ERROR_IF(s2n_stuffer_data_available(stuffer) < len,
               S2N_ERR_STUFFER_OUT_OF_DATA);

  int w = 0;
  do {
    errno = 0;
    w = conn->send(conn->send_io_context,
                   stuffer->blob.data + stuffer->read_cursor, len);
    if (w < 0 && errno == EPIPE) {
...
int s2n_connection_is_managed_corked(
    const struct s2n_connection *s2n_connection) {
...
int s2n_connection_is_client_auth_enabled(
