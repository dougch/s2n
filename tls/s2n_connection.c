...
{
    if ( s2n_connection_is_managed_corked( conn ) && conn->recv ) { GUARD( s2n_socket_read_restore( conn ) ); }
    if ( s2n_connection_is_managed_corked( conn ) && conn->send ) { GUARD( s2n_socket_write_restore( conn ) ); }
...
static uint8_t s2n_default_verify_host( const char *host_name, size_t len, void *data )
{
    ...
    if ( conn->server_name[ 0 ] == '\0' ) { return 0; }

    /* complete match */
    if ( strlen( conn->server_name ) == len && strncasecmp( conn->server_name, host_name, len ) == 0 ) { return 1; }
    ...
    if ( len > 2 && host_name[ 0 ] == '*' && host_name[ 1 ] == '.' ) {
        ...
        if ( strlen( suffix ) == len - 1 && strncasecmp( suffix, host_name + 1, len - 1 ) == 0 ) { return 1; }
    }
...
int s2n_connection_set_config( struct s2n_connection *conn, struct s2n_config *config )
{
    ...
    if ( conn->config == config ) { return 0; }
    ...
    if ( conn->client_cert_auth_type_overridden ) { auth_type = conn->client_cert_auth_type; }
...
void *s2n_connection_get_ctx( struct s2n_connection *conn ) { return conn->context; }
...
int s2n_connection_get_security_policy( struct s2n_connection *            conn,
...
uint64_t s2n_connection_get_wire_bytes_in( struct s2n_connection *conn ) { return conn->wire_bytes_in; }

uint64_t s2n_connection_get_wire_bytes_out( struct s2n_connection *conn ) { return conn->wire_bytes_out; }
...
const char *s2n_connection_get_curve( struct s2n_connection *conn )
{
    ...
    if ( !conn->secure.server_ecc_evp_params.negotiated_curve ) { return "NONE"; }
...
const char *s2n_connection_get_kem_name( struct s2n_connection *conn )
{
    ...
    if ( !conn->secure.s2n_kem_keys.negotiated_kem ) { return "NONE"; }
...
const char *s2n_get_server_name( struct s2n_connection *conn )
{
    ...
    if ( conn->server_name[ 0 ] ) { return conn->server_name; }

    /* server name is not yet obtained from client hello, get it now */
    struct s2n_client_hello_parsed_extension parsed_extension = { 0 };

    GUARD_PTR( s2n_client_hello_get_parsed_extension( conn->client_hello.parsed_extensions, S2N_EXTENSION_SERVER_NAME,
                                                      &parsed_extension ) );
    ...
    if ( !conn->server_name[ 0 ] ) { return NULL; }
...
const char *s2n_get_application_protocol( struct s2n_connection *conn )
{
    ...
    if ( strlen( conn->application_protocol ) == 0 ) { return NULL; }
...
int s2n_connection_prefer_throughput( struct s2n_connection *conn )
{
    ...
    if ( !conn->mfl_code ) { conn->max_outgoing_fragment_length = S2N_LARGE_FRAGMENT_LENGTH; }
...
int s2n_connection_prefer_low_latency( struct s2n_connection *conn )
{
    ...
    if ( !conn->mfl_code ) { conn->max_outgoing_fragment_length = S2N_SMALL_FRAGMENT_LENGTH; }
...
int s2n_connection_set_verify_host_callback( struct s2n_connection *conn, s2n_verify_host_fn verify_host_fn,
                                             void *data )
{
    notnull_check( conn );
...
int s2n_connection_send_stuffer( struct s2n_stuffer *stuffer, struct s2n_connection *conn, uint32_t len )
{
    ...
    if ( conn->write_fd_broken ) { S2N_ERROR( S2N_ERR_SEND_STUFFER_TO_CONN ); }
    ...
    do {
        ...
        if ( w < 0 && errno == EPIPE ) { conn->write_fd_broken = 1; }
