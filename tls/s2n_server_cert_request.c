...
static int s2n_recv_client_cert_preferences( struct s2n_stuffer *in, s2n_cert_type *chosen_cert_type_out )
{
    ...
    for ( int our_cert_pref_idx = 0; our_cert_pref_idx < sizeof( s2n_cert_type_preference_list );
          our_cert_pref_idx++ ) {
...
int s2n_cert_req_send( struct s2n_connection *conn )
{
    ...
    if ( conn->actual_protocol_version == S2N_TLS12 ) { GUARD( s2n_send_supported_sig_scheme_list( conn, out ) ); }
