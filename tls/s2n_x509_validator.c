...
/* one day, boringssl, may add ocsp stapling support. Let's future proof this a
 * bit by grabbing a definition that would have to be there when they add
 * support */
...
/* Time used by default for nextUpdate if none provided in OCSP: 1 hour since
 * thisUpdate. */
...
int s2n_x509_trust_store_from_system_defaults(
...
int s2n_x509_trust_store_add_pem(struct s2n_x509_trust_store *store,
                                 const char *pem) {
  notnull_check(store);
  notnull_check(pem);
  ...
  do {
    ...
    DEFER_CLEANUP(X509 *ca_cert = d2i_X509(NULL, &data, next_cert.size),
                  X509_free_pointer);
...
int s2n_x509_trust_store_from_ca_file(struct s2n_x509_trust_store *store,
  ...
  int err_code =
      X509_STORE_load_locations(store->trust_store, ca_pem_filename, ca_dir);
  if (!err_code) {
    s2n_x509_trust_store_wipe(store);
    S2N_ERROR(S2N_ERR_X509_TRUST_STORE);
  }

  /* It's a likely scenario if this function is called, a self-signed
   * certificate is used, and that is was generated without a trust anchor.
   * However if you call this function, the assumption is you trust ca_file or
   * path and if a certificate is encountered that's in that path, it should be
   * trusted. The following flag tells libcrypto to not care that the cert is
   * missing a root anchor. */
...
int s2n_x509_validator_init_no_x509_validation(
...
int s2n_x509_validator_init(struct s2n_x509_validator *validator,
...
int s2n_x509_validator_set_max_chain_depth(struct s2n_x509_validator *validator,
                                           uint16_t max_depth) {
...
/*
 * For each name in the cert. Iterate them. Call the callback. If one returns
 * true, then consider it validated, if none of them return true, the cert is
 * considered invalid.
 */
static uint8_t s2n_verify_host_information(struct s2n_x509_validator *validator,
  ...
  STACK_OF(GENERAL_NAME) *names_list =
      X509_get_ext_d2i(public_cert, NID_subject_alt_name, NULL, NULL);
  ...
  for (int i = 0; i < n && !verified; i++) {
    ...
    if (current_name->type == GEN_DNS) {
      ...
      verified =
          conn->verify_host_fn(name, name_len, conn->data_for_verify_host);
      ...
      /* strlen should be safe here since we made sure we were null terminated
       * AND that inet_ntop succeeded */
      if (!parse_err) {
        verified = conn->verify_host_fn((const char *)address.data,
  ...
  if (!verified && !san_found) {
    ...
    if (subject_name) {
      ...
      while ((next_idx = X509_NAME_get_index_by_NID(
                  subject_name, NID_commonName, curr_idx)) >= 0) {
        curr_idx = next_idx;
      }

      if (curr_idx >= 0) {
        ASN1_STRING *common_name = X509_NAME_ENTRY_get_data(
            X509_NAME_get_entry(subject_name, curr_idx));
        ...
        if (common_name) {
          ...
          /* X520CommonName allows the following ANSI string types per RFC 5280
           * Appendix A.1 */
          if (ASN1_STRING_type(common_name) == V_ASN1_TELETEXSTRING ||
            ...
            verified =
                conn->verify_host_fn(peer_cn, len, conn->data_for_verify_host);
          }
        }
      }
    }
...
s2n_cert_validation_code s2n_x509_validator_validate_cert_chain(
  ...
  struct s2n_blob cert_chain_blob = {.data = cert_chain_in,
                                     .size = cert_chain_len};
  DEFER_CLEANUP(struct s2n_stuffer cert_chain_in_stuffer = {0},
                s2n_stuffer_free);
  ...
  while (s2n_stuffer_data_available(&cert_chain_in_stuffer) &&
         certificate_count < validator->max_chain_depth) {
    uint32_t certificate_size = 0;

    if (s2n_stuffer_read_uint24(&cert_chain_in_stuffer, &certificate_size) <
        0) {
      return S2N_CERT_ERR_INVALID;
    }

    if (certificate_size == 0 ||
        certificate_size > s2n_stuffer_data_available(&cert_chain_in_stuffer)) {
      return S2N_CERT_ERR_INVALID;
    }
    ...
    asn1cert.data =
        s2n_stuffer_raw_read(&cert_chain_in_stuffer, certificate_size);
    ...
    if (certificate_count == 0) {
      if (s2n_asn1der_to_public_key_and_type(&public_key, pkey_type,
                                             &asn1cert) < 0) {
        return S2N_CERT_ERR_INVALID;
      }
    }
    ...
    if (conn->actual_protocol_version == S2N_TLS13) {
      ...
      S2N_ERROR_IF(2 > s2n_stuffer_data_available(&cert_chain_in_stuffer),
                   S2N_ERR_BAD_MESSAGE);
      GUARD(s2n_stuffer_read_uint16(&cert_chain_in_stuffer,
                                    &certificate_extensions_length));
      S2N_ERROR_IF(certificate_extensions_length >
                       s2n_stuffer_data_available(&cert_chain_in_stuffer),
                   S2N_ERR_BAD_MESSAGE);
      ...
      if (certificate_extensions_length > 0) {
        ...
        extensions.data =
            s2n_stuffer_raw_read(&cert_chain_in_stuffer, extensions.size);
        notnull_check(extensions.data);

        /* RFC 8446: if an extension applies to the entire chain, it SHOULD be
         * included in the first CertificateEntry */
        if (certificate_count == 0) {
  ...
  if (!validator->skip_cert_validation) {
    ...
    if (conn->verify_host_fn &&
        !s2n_verify_host_information(validator, conn, leaf)) {
      return S2N_CERT_ERR_UNTRUSTED;
    }
    ...
    int op_code = X509_STORE_CTX_init(ctx, validator->trust_store->trust_store,
                                      leaf, validator->cert_chain);
...
s2n_cert_validation_code s2n_x509_validator_validate_cert_stapled_ocsp_response(
    struct s2n_x509_validator *validator, struct s2n_connection *conn,
    const uint8_t *ocsp_response_raw, uint32_t ocsp_response_length) {
  ...
  ocsp_response =
      d2i_OCSP_RESPONSE(NULL, &ocsp_response_raw, ocsp_response_length);
  ...
  if (certs_in_chain >= 2 && certs_in_ocsp >= 1) {
    X509 *responder =
        sk_X509_value(OCSP_GET_CERTS(basic_response), certs_in_ocsp - 1);

    /*check to see if one of the certs in the chain is an issuer of the cert in
     * the ocsp response.*/
  ...
  int ocsp_verify_err =
      OCSP_basic_verify(basic_response, validator->cert_chain,
                        validator->trust_store->trust_store, 0);
  ...
  for (i = 0; i < OCSP_resp_count(basic_response); i++) {
    ...
    ocsp_status = OCSP_single_get0_status(single_response, &status_reason,
                                          &revtime, &thisupd, &nextupd);

    uint64_t this_update = 0;
    int thisupd_err = s2n_asn1_time_to_nano_since_epoch_ticks(
        (const char *)thisupd->data, (uint32_t)thisupd->length, &this_update);

    uint64_t next_update = 0;
    int nextupd_err = 0;
    if (nextupd) {
      nextupd_err = s2n_asn1_time_to_nano_since_epoch_ticks(
    ...
    int current_time_err =
        conn->config->wall_clock(conn->config->sys_clock_ctx, &current_time);
