...
static int s2n_write_signature_blob(struct s2n_stuffer *out,
                                    const struct s2n_pkey *priv_key,
...
int s2n_server_key_recv(struct s2n_connection *conn) {
  ...
  const struct s2n_kex *key_exchange =
  ...
  GUARD(s2n_kex_server_key_recv_read_data(key_exchange, conn, &data_to_verify,
                                          &kex_data));

  /* Add common signature data */
  struct s2n_signature_scheme active_sig_scheme;
  if (conn->actual_protocol_version == S2N_TLS12) {
    /* Verify the SigScheme picked by the Server was actually in the list we
     * sent */
    GUARD(s2n_get_and_validate_negotiated_signature_scheme(conn, in,
                                                           &active_sig_scheme));
  } else {
    active_sig_scheme = conn->secure.conn_sig_scheme;
  }
  GUARD(s2n_hash_init(signature_hash, active_sig_scheme.hash_alg));
  GUARD(s2n_hash_update(signature_hash, conn->secure.client_random,
                        S2N_TLS_RANDOM_DATA_LEN));
  GUARD(s2n_hash_update(signature_hash, conn->secure.server_random,
                        S2N_TLS_RANDOM_DATA_LEN));

  /* Add KEX specific data */
  GUARD(s2n_hash_update(signature_hash, data_to_verify.data,
                        data_to_verify.size));
  ...
  struct s2n_blob signature = {
      ...
      .data = s2n_stuffer_raw_read(in, signature_length)};
  notnull_check(signature.data);
  gt_check(signature_length, 0);

  S2N_ERROR_IF(s2n_pkey_verify(&conn->secure.server_public_key,
                               active_sig_scheme.sig_alg, signature_hash,
                               &signature) < 0,
               S2N_ERR_BAD_MESSAGE);
  ...
  /* Parse the KEX data into whatever form needed and save it to the connection
   * object */
...
int s2n_ecdhe_server_key_recv_read_data(
    struct s2n_connection *conn, struct s2n_blob *data_to_verify,
    struct s2n_kex_raw_server_data *raw_server_data) {
  struct s2n_stuffer *in = &conn->handshake.io;

  GUARD(s2n_ecc_evp_read_params(in, data_to_verify,
                                &raw_server_data->ecdhe_data));
  return 0;
}

int s2n_ecdhe_server_key_recv_parse_data(
    struct s2n_connection *conn,
    struct s2n_kex_raw_server_data *raw_server_data) {
  GUARD(s2n_ecc_evp_parse_params(&raw_server_data->ecdhe_data,
                                 &conn->secure.server_ecc_evp_params));
...
int s2n_dhe_server_key_recv_parse_data(
    struct s2n_connection *conn,
    struct s2n_kex_raw_server_data *raw_server_data) {
  struct s2n_dhe_raw_server_points dhe_data = raw_server_data->dhe_data;

  /* Copy the DH details */
  GUARD(s2n_dh_p_g_Ys_to_dh_params(&conn->secure.server_dh_params, &dhe_data.p,
                                   &dhe_data.g, &dhe_data.Ys));
  return 0;
}
...
int s2n_kem_server_key_recv_read_data(
  ...
  S2N_ERROR_IF(key_length > s2n_stuffer_data_available(in),
               S2N_ERR_BAD_MESSAGE);
  ...
  data_to_verify->size =
      sizeof(kem_extension_size) + sizeof(kem_public_key_size) + key_length;
  return 0;
}

int s2n_kem_server_key_recv_parse_data(
    struct s2n_connection *conn,
    struct s2n_kex_raw_server_data *raw_server_data) {
  struct s2n_kem_raw_server_params *kem_data = &raw_server_data->kem_data;
  ...
  S2N_ERROR_IF(
      s2n_choose_kem_with_peer_pref_list(
          cipher_suite->iana_value, &kem_data->kem_name,
          cipher_preferences->kems, cipher_preferences->kem_count, &match) != 0,
      S2N_ERR_KEM_UNSUPPORTED_PARAMS);
  conn->secure.s2n_kem_keys.negotiated_kem = match;

  S2N_ERROR_IF(kem_data->raw_public_key.size !=
                   conn->secure.s2n_kem_keys.negotiated_kem->public_key_length,
               S2N_ERR_BAD_MESSAGE);
...
int s2n_hybrid_server_key_recv_read_data(
    struct s2n_connection *conn, struct s2n_blob *total_data_to_verify,
    struct s2n_kex_raw_server_data *raw_server_data) {
  notnull_check(conn);
  ...
  GUARD(s2n_kex_server_key_recv_read_data(hybrid_kex_0, conn, &data_to_verify_0,
                                          raw_server_data));

  struct s2n_blob data_to_verify_1 = {0};
  GUARD(s2n_kex_server_key_recv_read_data(hybrid_kex_1, conn, &data_to_verify_1,
                                          raw_server_data));
...
int s2n_hybrid_server_key_recv_parse_data(
    struct s2n_connection *conn,
    struct s2n_kex_raw_server_data *raw_server_data) {
  notnull_check(conn);
  notnull_check(conn->secure.cipher_suite);
  ...
  GUARD(
      s2n_kex_server_key_recv_parse_data(hybrid_kex_0, conn, raw_server_data));
  GUARD(
      s2n_kex_server_key_recv_parse_data(hybrid_kex_1, conn, raw_server_data));
  return 0;
}
...
int s2n_server_key_send(struct s2n_connection *conn) {
  ...
  if (conn->actual_protocol_version == S2N_TLS12) {
    GUARD(
        s2n_stuffer_write_uint16(out, conn->secure.conn_sig_scheme.iana_value));
  }
  ...
  GUARD(s2n_write_signature_blob(
      out, conn->handshake_params.our_chain_and_key->private_key,
      conn->secure.conn_sig_scheme.sig_alg, signature_hash));
  return 0;
}

int s2n_ecdhe_server_key_send(struct s2n_connection *conn,
                              struct s2n_blob *data_to_sign) {
  struct s2n_stuffer *out = &conn->handshake.io;

  /* Generate an ephemeral key and  */
  GUARD(
      s2n_ecc_evp_generate_ephemeral_key(&conn->secure.server_ecc_evp_params));

  /* Write it out and calculate the data to sign later */
  GUARD(s2n_ecc_evp_write_params(&conn->secure.server_ecc_evp_params, out,
                                 data_to_sign));
  return 0;
}

int s2n_dhe_server_key_send(struct s2n_connection *conn,
                            struct s2n_blob *data_to_sign) {
  struct s2n_stuffer *out = &conn->handshake.io;

  /* Duplicate the DH key from the config */
  GUARD(s2n_dh_params_copy(conn->config->dhparams,
                           &conn->secure.server_dh_params));

  /* Generate an ephemeral key */
  GUARD(s2n_dh_generate_ephemeral_key(&conn->secure.server_dh_params));

  /* Write it out and calculate the data to sign later */
  GUARD(s2n_dh_params_to_p_g_Ys(&conn->secure.server_dh_params, out,
                                data_to_sign));
  return 0;
}

int s2n_kem_server_key_send(struct s2n_connection *conn,
                            struct s2n_blob *data_to_sign) {
  struct s2n_stuffer *out = &conn->handshake.io;
  ...
  /* The public key is not needed after this method, write it straight to the
   * stuffer */
  ...
  data_to_sign->size = sizeof(kem_extension_size) +
                       sizeof(kem_public_key_size) + public_key->size;
  return 0;
}

int s2n_hybrid_server_key_send(struct s2n_connection *conn,
                               struct s2n_blob *total_data_to_sign) {
  notnull_check(conn);
  notnull_check(conn->secure.cipher_suite);
  const struct s2n_kex *kex = conn->secure.cipher_suite->key_exchange_alg;
  const struct s2n_kex *hybrid_kex_0 = kex->hybrid[0];
...
static int s2n_write_signature_blob(struct s2n_stuffer *out,
  ...
  S2N_ERROR_IF(s2n_pkey_sign(priv_key, sig_alg, digest, &signature) < 0,
               S2N_ERR_DH_FAILED_SIGNING);

  /* Now that the signature has been created, write the actual size that was
   * stored in the signature blob */
