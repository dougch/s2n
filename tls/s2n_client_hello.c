...
struct s2n_client_hello *s2n_connection_get_client_hello(
...
ssize_t s2n_client_hello_get_raw_message(struct s2n_client_hello *ch,
                                         uint8_t *out, uint32_t max_length) {
  notnull_check(ch);
  notnull_check(out);
...
ssize_t s2n_client_hello_get_cipher_suites(struct s2n_client_hello *ch,
                                           uint8_t *out, uint32_t max_length) {
  notnull_check(ch);
  notnull_check(out);
...
ssize_t s2n_client_hello_get_extensions(struct s2n_client_hello *ch,
                                        uint8_t *out, uint32_t max_length) {
  notnull_check(ch);
  notnull_check(out);
...
int s2n_client_hello_free_parsed_extensions(
...
int s2n_collect_client_hello(struct s2n_connection *conn,
                             struct s2n_stuffer *source) {
  notnull_check(conn);
...
static int s2n_parse_client_hello(struct s2n_connection *conn) {
  ...
  GUARD(s2n_stuffer_read_bytes(in, client_protocol_version,
                               S2N_TLS_PROTOCOL_VERSION_LEN));
  GUARD(s2n_stuffer_erase_and_read_bytes(in, conn->secure.client_random,
  ...
  conn->client_protocol_version =
      MIN((client_protocol_version[0] * 10) + client_protocol_version[1],
          S2N_TLS12);
  conn->client_hello_version = conn->client_protocol_version;

  S2N_ERROR_IF(conn->session_id_len > S2N_TLS_SESSION_ID_MAX_LEN ||
                   conn->session_id_len > s2n_stuffer_data_available(in),
               S2N_ERR_BAD_MESSAGE);
  ...
  S2N_ERROR_IF(cipher_suites_length % S2N_TLS_CIPHER_SUITE_LEN,
               S2N_ERR_BAD_MESSAGE);

  client_hello->cipher_suites.size = cipher_suites_length;
  client_hello->cipher_suites.data =
      s2n_stuffer_raw_read(in, cipher_suites_length);
  ...
  conn->secure.server_ecc_evp_params.negotiated_curve =
      s2n_ecc_evp_supported_curves_list[0];
  ...
  if (s2n_stuffer_data_available(in) >= 2) {
    ...
    S2N_ERROR_IF(extensions_length > s2n_stuffer_data_available(in),
                 S2N_ERR_BAD_MESSAGE);
...
static int s2n_parsed_extensions_compare(const void *p, const void *q) {
  const struct s2n_client_hello_parsed_extension *left =
      (const struct s2n_client_hello_parsed_extension *)p;
  const struct s2n_client_hello_parsed_extension *right =
      (const struct s2n_client_hello_parsed_extension *)q;
...
static int s2n_populate_client_hello_extensions(struct s2n_client_hello *ch) {
  ...
  if (ch->parsed_extensions == NULL) {
    notnull_check(ch->parsed_extensions = s2n_array_new(
                      sizeof(struct s2n_client_hello_parsed_extension)));
  }
  ...
  while (s2n_stuffer_data_available(&in)) {
    ...
    S2N_ERROR_IF(S2N_CBIT_TEST(parsed_extensions_mask, ext_type),
                 S2N_ERR_BAD_MESSAGE);
    ...
    struct s2n_client_hello_parsed_extension *parsed_extension =
        s2n_array_pushback(ch->parsed_extensions);
  ...
  qsort(ch->parsed_extensions->mem.data, ch->parsed_extensions->num_of_elements,
        ch->parsed_extensions->element_size, s2n_parsed_extensions_compare);
...
int s2n_process_client_hello(struct s2n_connection *conn) {
  ...
  GUARD(s2n_set_cipher_as_tls_server(conn, client_hello->cipher_suites.data,
  ...
  GUARD(s2n_choose_sig_scheme_from_peer_preference_list(
      conn, &conn->handshake_params.client_sig_hash_algs,
      &conn->secure.conn_sig_scheme));

  /* And finally, set the certs specified by the final auth + sig_alg combo. */
  GUARD(s2n_select_certs_for_server_auth(
      conn, &conn->handshake_params.our_chain_and_key));
...
int s2n_client_hello_recv(struct s2n_connection *conn) {
  ...
  /* Mark the collected client hello as available when parsing is done and
   * before the client hello callback */
  conn->client_hello.parsed = 1;

  /* Call client_hello_cb if exists, letting application to modify
   * s2n_connection or swap s2n_config */
  if (conn->config->client_hello_cb) {
    int rc =
        conn->config->client_hello_cb(conn, conn->config->client_hello_cb_ctx);
    if (rc < 0) {
...
int s2n_client_hello_send(struct s2n_connection *conn) {
  ...
  uint8_t reported_protocol_version =
      MIN(conn->client_protocol_version, S2N_TLS12);
  ...
  /* Find the number of available suites in the preference list. Some ciphers
   * may be unavailable if s2n is built with an older libcrypto
   */
  ...
  GUARD(s2n_stuffer_write_uint16(
      out, num_available_suites * S2N_TLS_CIPHER_SUITE_LEN));

  /* Now, write the IANA values every available cipher suite in our list */
  for (int i = 0; i < cipher_preferences->count; i++) {
    if (cipher_preferences->suites[i]->available) {
      GUARD(s2n_stuffer_write_bytes(out,
                                    cipher_preferences->suites[i]->iana_value,
                                    S2N_TLS_CIPHER_SUITE_LEN));
    }
  }
  /* Lastly, write TLS_EMPTY_RENEGOTIATION_INFO_SCSV so that server knows it's
   * an initial handshake (RFC5746 Section 3.4) */
...
/* See
 * http://www-archive.mozilla.org/projects/security/pki/nss/ssl/draft02.html 2.5
 */
int s2n_sslv2_client_hello_recv(struct s2n_connection *conn) {
  ...
  conn->actual_protocol_version =
      MIN(conn->client_protocol_version, conn->server_protocol_version);

  /* We start 5 bytes into the record */
  uint16_t cipher_suites_length;
  GUARD(s2n_stuffer_read_uint16(in, &cipher_suites_length));
  S2N_ERROR_IF(cipher_suites_length % S2N_SSLv2_CIPHER_SUITE_LEN,
               S2N_ERR_BAD_MESSAGE);
  ...
  GUARD(s2n_set_cipher_as_sslv2_server(
      conn, client_hello->cipher_suites.data,
  ...
  S2N_ERROR_IF(session_id_length > s2n_stuffer_data_available(in),
               S2N_ERR_BAD_MESSAGE);
  if (session_id_length > 0 &&
      session_id_length <= S2N_TLS_SESSION_ID_MAX_LEN) {
...
int s2n_client_hello_get_parsed_extension(
    struct s2n_array *parsed_extensions, s2n_tls_extension_type extension_type,
    struct s2n_client_hello_parsed_extension *parsed_extension) {
  ...
  struct s2n_client_hello_parsed_extension *result_extension = bsearch(
      &search, parsed_extensions->mem.data, parsed_extensions->num_of_elements,
...
ssize_t s2n_client_hello_get_extension_length(
  ...
  if (s2n_client_hello_get_parsed_extension(
          ch->parsed_extensions, extension_type, &parsed_extension)) {
    return 0;
  }
