...
WRITE:
    while (s2n_stuffer_data_available(&conn->out)) {
        ...
        if (w < 0) {
            if (errno == EWOULDBLOCK || errno == EAGAIN) { S2N_ERROR(S2N_ERR_IO_BLOCKED); }
            S2N_ERROR(S2N_ERR_IO);
        }
        conn->wire_bytes_out += w;
    }

    if (conn->closing) { conn->closed = 1; }
                              ...
                              s2n_blocked_status *blocked)
{
    ...
    *blocked = S2N_BLOCKED_ON_WRITE;
    ...
    if (conn->mode == S2N_CLIENT) { writer = conn->client; }
    ...
    for (int i = 0; i < count; i++) { total_size += bufs[ i ].iov_len; }
    total_size -= offs;
    ...
    while (total_size - conn->current_user_data_consumed) {
        ...
        if (conn->active_application_bytes_consumed < ( uint64_t )conn->dynamic_record_resize_threshold) {
            ...
            if (min_payload_size < to_write) { to_write = min_payload_size; }
        }
