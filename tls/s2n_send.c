...
WRITE:
  while (s2n_stuffer_data_available(&conn->out)) {
    errno = 0;
    w = s2n_connection_send_stuffer(&conn->out, conn,
                                    s2n_stuffer_data_available(&conn->out));
    if (w < 0) {
...
ssize_t s2n_sendv_with_offset(struct s2n_connection *conn,
                              const struct iovec *bufs, ssize_t count,
  ...
  *blocked = S2N_BLOCKED_ON_WRITE;
  ...
  total_size -= offs;
  S2N_ERROR_IF(conn->current_user_data_consumed > total_size,
               S2N_ERR_SEND_SIZE);
  ...
  if (conn->dynamic_record_timeout_threshold > 0) {
     ...
     * inactivity */
    if (elapsed - conn->last_write_elapsed >
        (uint64_t)conn->dynamic_record_timeout_threshold * 1000000000) {
  ...
  while (total_size - conn->current_user_data_consumed) {
    ssize_t to_write =
        MIN(total_size - conn->current_user_data_consumed, max_payload_size);

    /* If dynamic record size is enabled,
     * use small TLS records that fit into a single TCP segment for the
     * threshold bytes of data
     */
    if (conn->active_application_bytes_consumed <
        (uint64_t)conn->dynamic_record_resize_threshold) {
    ...
    /* Don't split messages in server mode for interoperability with naive
     * clients. Some clients may have expectations based on the amount of
     * content in the first record.
     */
    if (conn->actual_protocol_version < S2N_TLS11 &&
        writer->cipher_suite->record_alg->cipher->type == S2N_CBC &&
    ...
    if (s2n_flush(conn, blocked) < 0) {
      if (s2n_errno == S2N_ERR_BLOCKED && user_data_sent > 0) {
        /* We successfully sent >0 user bytes on the wire, but not the full
         * requested payload because we became blocked on I/O. Acknowledge the
         * data sent. */
...
ssize_t s2n_send(struct s2n_connection *conn, const void *buf, ssize_t size,
