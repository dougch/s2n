...
int s2n_extensions_client_supported_groups_send(struct s2n_connection *conn,
                                                struct s2n_stuffer *out) {
  GUARD(s2n_stuffer_write_uint16(out, TLS_EXTENSION_SUPPORTED_GROUPS));
  GUARD(s2n_stuffer_write_uint16(
      out, 2 + s2n_ecc_evp_supported_curves_list_len * 2));
  /* Curve list len */
  GUARD(
      s2n_stuffer_write_uint16(out, s2n_ecc_evp_supported_curves_list_len * 2));
  /* Curve list */
  for (int i = 0; i < s2n_ecc_evp_supported_curves_list_len; i++) {
    GUARD(s2n_stuffer_write_uint16(
        out, s2n_ecc_evp_supported_curves_list[i]->iana_id));
  }
...
int s2n_recv_client_supported_groups(struct s2n_connection *conn,
  ...
  if (s2n_ecc_evp_find_supported_curve(
          &proposed_curves,
          &conn->secure.server_ecc_evp_params.negotiated_curve) != 0) {
    /* Can't agree on a curve, ECC is not allowed. Return success to proceed
     * with the handshake. */
