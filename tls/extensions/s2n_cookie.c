...
int s2n_extensions_cookie_size(struct s2n_connection *conn) {
  ...
  const int cookie_extension_size =
...
int s2n_extensions_cookie_send(struct s2n_connection *conn,
  ...
  if (s2n_extensions_cookie_size(conn) > 0) {
    const int extension_data_size = s2n_extensions_cookie_size(conn) -
                                    S2N_SIZE_OF_EXTENSION_TYPE -
                                    S2N_SIZE_OF_EXTENSION_DATA_SIZE;
    const int cookie_data_size =
        extension_data_size - S2N_SIZE_OF_COOKIE_DATA_SIZE;
