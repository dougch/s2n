...
int s2n_extensions_client_signature_algorithms_send(struct s2n_connection *conn,
  ...
  const uint16_t total_size =
      s2n_extensions_client_signature_algorithms_size(conn);
...
int s2n_extensions_client_signature_algorithms_recv(
    struct s2n_connection *conn, struct s2n_stuffer *extension) {
  return s2n_recv_supported_sig_scheme_list(
      extension, &conn->handshake_params.client_sig_hash_algs);
}

int s2n_extensions_client_signature_algorithms_size(
