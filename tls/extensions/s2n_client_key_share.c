 ...
 *
 * The server will NOT alert when processing a client extension that violates
 *the RFC. So the server will accept:
 * - Multiple key shares for the same named group. The server will accept the
 *first key share for the group and ignore any duplicates.
 * - Key shares for named groups not in the client's supported_groups extension.
 **/

uint32_t s2n_client_key_share_extension_size;

static int s2n_ecdhe_supported_curves_send(struct s2n_connection *conn,
...
int s2n_client_key_share_init() {
  ...
  for (uint32_t i = 0; i < s2n_ecc_evp_supported_curves_list_len; i++) {
    s2n_client_key_share_extension_size +=
        S2N_SIZE_OF_KEY_SHARE_SIZE + S2N_SIZE_OF_NAMED_GROUP;
    s2n_client_key_share_extension_size +=
        s2n_ecc_evp_supported_curves_list[i]->share_size;
  }

  return 0;
}

int s2n_extensions_client_key_share_recv(struct s2n_connection *conn,
  ...
  S2N_ERROR_IF(s2n_stuffer_data_available(extension) < key_shares_size,
               S2N_ERR_BAD_MESSAGE);
  ...
  /* bytes_processed is declared as a uint32_t to avoid integer overflow in
   * later calculations */
  ...
  while (bytes_processed < key_shares_size) {
    ...
    S2N_ERROR_IF(s2n_stuffer_data_available(extension) < share_size,
                 S2N_ERR_BAD_MESSAGE);
    bytes_processed +=
        share_size + S2N_SIZE_OF_NAMED_GROUP + S2N_SIZE_OF_KEY_SHARE_SIZE;
    ...
    if (conn->secure.client_ecc_evp_params[supported_curve_index]
            .negotiated_curve) {
      GUARD(s2n_stuffer_skip_read(extension, share_size));
      continue;
    }
    ...
    conn->secure.client_ecc_evp_params[supported_curve_index].negotiated_curve =
        supported_curve;
    if (s2n_ecc_evp_parse_params_point(
            &point_blob,
            &conn->secure.client_ecc_evp_params[supported_curve_index]) < 0) {
      /* Ignore curves with points we can't parse */
      conn->secure.client_ecc_evp_params[supported_curve_index]
          .negotiated_curve = NULL;
      GUARD(s2n_ecc_evp_params_free(
          &conn->secure.client_ecc_evp_params[supported_curve_index]));
    }
  }
...
int s2n_extensions_client_key_share_send(struct s2n_connection *conn,
                                         struct s2n_stuffer *out) {
  notnull_check(out);

  const uint16_t extension_type = TLS_EXTENSION_KEY_SHARE;
  const uint16_t extension_data_size = s2n_client_key_share_extension_size -
