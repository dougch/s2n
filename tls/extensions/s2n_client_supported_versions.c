...
int s2n_extensions_client_supported_versions_size(struct s2n_connection *conn) {
  ...
  GUARD(s2n_connection_get_minimum_supported_version(
      conn, &minimum_supported_version));
  uint8_t highest_supported_version = conn->client_protocol_version;

  uint8_t version_list_length =
      highest_supported_version - minimum_supported_version + 1;

  return version_list_length * S2N_TLS_PROTOCOL_VERSION_LEN + 5;
}

int s2n_extensions_client_supported_versions_process(
    struct s2n_connection *conn, struct s2n_stuffer *extension) {
  ...
  S2N_ERROR_IF(size_of_version_list != s2n_stuffer_data_available(extension),
               S2N_ERR_BAD_MESSAGE);
  S2N_ERROR_IF(size_of_version_list % S2N_TLS_PROTOCOL_VERSION_LEN != 0,
               S2N_ERR_BAD_MESSAGE);
  ...
  for (int i = 0; i < size_of_version_list; i += S2N_TLS_PROTOCOL_VERSION_LEN) {
    ...
    GUARD(s2n_stuffer_read_bytes(extension, client_version_parts,
                                 S2N_TLS_PROTOCOL_VERSION_LEN));

    uint16_t client_version =
        (client_version_parts[0] * 10) + client_version_parts[1];
    ...
    conn->actual_protocol_version =
        MAX(client_version, conn->actual_protocol_version);
  }

  S2N_ERROR_IF(conn->actual_protocol_version == s2n_unknown_protocol_version,
...
int s2n_extensions_client_supported_versions_send(struct s2n_connection *conn,
  ...
  for (uint8_t i = highest_supported_version; i >= minimum_supported_version;
       i--) {
