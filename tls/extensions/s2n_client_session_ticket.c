...
int s2n_extensions_client_session_ticket_send(struct s2n_connection *conn,
...
int s2n_recv_client_session_ticket_ext(struct s2n_connection *conn,
  ...
  if (s2n_stuffer_data_available(extension) == S2N_TICKET_SIZE_IN_BYTES) {
    ...
    GUARD(s2n_stuffer_copy(extension, &conn->client_ticket_to_decrypt,
                           S2N_TICKET_SIZE_IN_BYTES));
