...
static int s2n_get_number_certs_in_chain(struct s2n_cert *head,
                                         uint8_t *chain_length);

int s2n_certificate_extensions_parse(struct s2n_connection *conn,
  ...
  while (s2n_stuffer_data_available(&extensions_in)) {
    ...
    S2N_ERROR_IF(s2n_stuffer_data_available(&extensions_in) < 4,
                 S2N_ERR_BAD_MESSAGE);
     ...
     */
    S2N_ERROR_IF(S2N_CBIT_TEST(parsed_extensions_mask, extension_type),
                 S2N_ERR_BAD_MESSAGE);
    S2N_CBIT_SET(parsed_extensions_mask, extension_type);

    S2N_ERROR_IF(extension_size > s2n_stuffer_data_available(&extensions_in),
                 S2N_ERR_BAD_MESSAGE);
...
int s2n_certificate_extensions_send(struct s2n_stuffer *out) {
  /* For minimal TLS 1.3 implementation, we are sending no certificate
   * extensions. We only send the length field with a value of 0.
   */
...
int s2n_certificate_extensions_size(struct s2n_cert *head) {
  /* For minimal TLS 1.3 implementation, we are sending no certificate
   * extensions. For now, size is hardcoded to 2 * num_certs in order to send
   * extensions_length field with value 0 for each cert.
   */
  uint8_t num_certs;
...
int s2n_get_number_certs_in_chain(struct s2n_cert *head,
                                  uint8_t *chain_length) {
  notnull_check(head);
