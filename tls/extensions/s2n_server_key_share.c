...
static int s2n_server_key_share_recv(struct s2n_connection *conn, struct s2n_stuffer *extension)
{
    ...
    ENSURE_POSIX(has_supported_curve, S2N_ERR_ECDHE_UNSUPPORTED_CURVE);
     ...
     */
    if (s2n_is_hello_retry_message(conn)) { return S2N_SUCCESS; }
...
int s2n_extensions_server_key_share_send_size(struct s2n_connection *conn)
{
    ...
    if (s2n_is_hello_retry_message(conn)) { return key_share_size; }
