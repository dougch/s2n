...
int s2n_extensions_server_key_share_send_check(struct s2n_connection *conn) {
  ...
  const struct s2n_ecc_evp_params client_ecc_evp =
      conn->secure.client_ecc_evp_params[curve_index];
 ...
 *
 * This functions does not error, but s2n_extensions_server_key_share_send()
 * would
 */
int s2n_extensions_server_key_share_send_size(struct s2n_connection *conn) {
  ...
  const int key_share_size =
      S2N_SIZE_OF_EXTENSION_TYPE + S2N_SIZE_OF_EXTENSION_DATA_SIZE +
      S2N_SIZE_OF_NAMED_GROUP + S2N_SIZE_OF_KEY_SHARE_SIZE + curve->share_size;
 ...
 *
 * Expects negotiated_curve to be set and generates a ephemeral key for key
 * sharing
 */
int s2n_extensions_server_key_share_send(struct s2n_connection *conn,
  ...
  GUARD(s2n_stuffer_write_uint16(
      out, s2n_extensions_server_key_share_send_size(conn) -
               S2N_SIZE_OF_EXTENSION_TYPE - S2N_SIZE_OF_EXTENSION_DATA_SIZE));
 ...
 */
int s2n_extensions_server_key_share_recv(struct s2n_connection *conn,
  ...
  S2N_ERROR_IF(s2n_stuffer_data_available(extension) < 4,
  ...
  S2N_ERROR_IF(
      conn->secure.client_ecc_evp_params[supported_curve_index].evp_pkey ==
          NULL,
      S2N_ERR_BAD_KEY_SHARE);
  ...
  S2N_ERROR_IF(
      s2n_ecc_evp_read_params_point(extension, share_size, &point_blob) < 0,
      S2N_ERR_BAD_KEY_SHARE);
  S2N_ERROR_IF(
      s2n_ecc_evp_parse_params_point(&point_blob, server_ecc_evp_params) < 0,
      S2N_ERR_BAD_KEY_SHARE);
