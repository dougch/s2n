...
int s2n_extensions_server_supported_versions_size() { return 6; }

int s2n_extensions_server_supported_versions_process(
  ...
  GUARD(s2n_stuffer_read_bytes(extension, server_version_parts,
                               S2N_TLS_PROTOCOL_VERSION_LEN));

  uint16_t server_version =
      (server_version_parts[0] * 10) + server_version_parts[1];
...
int s2n_extensions_server_supported_versions_recv(
    struct s2n_connection *conn, struct s2n_stuffer *extension) {
  S2N_ERROR_IF(
      s2n_extensions_server_supported_versions_process(conn, extension) < 0,
      S2N_ERR_BAD_MESSAGE);

  return 0;
}

int s2n_extensions_server_supported_versions_send(struct s2n_connection *conn,
