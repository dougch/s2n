...
int s2n_extensions_client_alpn_send(struct s2n_connection *conn,
...
int s2n_recv_client_alpn(struct s2n_connection *conn,
  ...
  while (s2n_stuffer_data_available(&server_protos)) {
    ...
    while (s2n_stuffer_data_available(&client_protos)) {
      ...
      S2N_ERROR_IF(client_length > s2n_stuffer_data_available(&client_protos),
                   S2N_ERR_BAD_MESSAGE);
      if (client_length != length) {
        GUARD(s2n_stuffer_skip_read(&client_protos, client_length));
      } else {
        uint8_t client_protocol[255];
        GUARD(s2n_stuffer_read_bytes(&client_protos, client_protocol,
                                     client_length));
        if (memcmp(client_protocol, server_protocol, client_length) == 0) {
          memcpy_check(conn->application_protocol, client_protocol,
                       client_length);
