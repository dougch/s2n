...
int s2n_server_cert_recv(struct s2n_connection *conn) {
  if (conn->actual_protocol_version == S2N_TLS13) {
    ...
    GUARD(s2n_stuffer_read_uint8(&conn->handshake.io,
                                 &certificate_request_context_len));
  }

  uint32_t size_of_all_certificates;
  GUARD(
      s2n_stuffer_read_uint24(&conn->handshake.io, &size_of_all_certificates));

  S2N_ERROR_IF(size_of_all_certificates >
                       s2n_stuffer_data_available(&conn->handshake.io) ||
                   size_of_all_certificates < 3,
               S2N_ERR_BAD_MESSAGE);
  ...
  cert_chain.data =
      s2n_stuffer_raw_read(&conn->handshake.io, size_of_all_certificates);
  notnull_check(cert_chain.data);

  S2N_ERROR_IF(
      s2n_x509_validator_validate_cert_chain(
          &conn->x509_validator, conn, cert_chain.data, cert_chain.size,
...
int s2n_server_cert_send(struct s2n_connection *conn) {
  S2N_ERROR_IF(conn->handshake_params.our_chain_and_key == NULL,
  ...
  if (conn->actual_protocol_version == S2N_TLS13) {
    ...
    GUARD(s2n_stuffer_write_uint8(&conn->handshake.io,
                                  certificate_request_context_len));
  }

  GUARD(s2n_send_cert_chain(
      &conn->handshake.io, conn->handshake_params.our_chain_and_key->cert_chain,
