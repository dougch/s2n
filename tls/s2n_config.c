...
{
    if ( s2n_is_tls13_enabled() ) { return &s2n_default_tls13_config; }
...
{
    if ( config->ticket_keys != NULL ) { GUARD( s2n_set_free_p( &config->ticket_keys ) ); }

    if ( config->ticket_key_hashes != NULL ) { GUARD( s2n_set_free_p( &config->ticket_key_hashes ) ); }
...
{
    if ( config->dhparams ) { GUARD( s2n_dh_params_free( config->dhparams ) ); }
...
int s2n_config_set_verification_ca_location( struct s2n_config *config, const char *ca_pem_filename,
                                             const char *ca_dir )
{
    notnull_check( config );
                                   ...
                                   uint32_t length )
{
    ...
    if ( s2n_config_get_num_default_certs( config ) == 0 ) { S2N_ERROR( S2N_ERR_UPDATING_EXTENSION ); }
...
int s2n_config_set_session_cache_onoff( struct s2n_config *config, uint8_t enabled )
{
    ...
    if ( enabled && config->cache_store && config->cache_retrieve && config->cache_delete ) {
        ...
        if ( !config->use_tickets ) { GUARD( s2n_config_free_session_ticket_keys( config ) ); }
        config->use_session_cache = 0;
    }
    return 0;
}
