...
/* This config should only used by the s2n_client for unit/integration testing
 * purposes. */
...
static int s2n_config_init(struct s2n_config *config) {
  ...
  memset(&config->application_protocols, 0,
         sizeof(config->application_protocols));
  ...
  /* By default, only the client will authenticate the Server's Certificate. The
   * Server does not request or authenticate any client certificates. */
  ...
  notnull_check(config->domain_name_to_cert_map =
                    s2n_map_new_with_initial_capacity(1));
...
static int s2n_config_update_domain_name_to_cert_map(
    struct s2n_config *config, struct s2n_blob *name,
  ...
  if (s2n_map_lookup(domain_name_to_cert_map, name, &s2n_map_value) == 0) {
    ...
    if (value->certs[cert_type] == NULL) {
       ...
       * Run the application's tiebreaking callback to decide which cert should
       * be used. An application may have some context specific logic to resolve
       * ties that are based on factors like trust, expiry, etc.
       */
      struct s2n_cert_chain_and_key *winner = config->cert_tiebreak_cb(
          value->certs[cert_type], cert_key_pair, name->data, name->size);
      if (winner) {
...
static int s2n_config_build_domain_name_to_cert_map(
  ...
  if (s2n_array_num_elements(cert_key_pair->san_names) == 0) {
    for (int i = 0; i < s2n_array_num_elements(cert_key_pair->cn_names); i++) {
      ...
      GUARD(s2n_config_update_domain_name_to_cert_map(config, cn_name,
                                                      cert_key_pair));
    }
  } else {
    for (int i = 0; i < s2n_array_num_elements(cert_key_pair->san_names); i++) {
      ...
      GUARD(s2n_config_update_domain_name_to_cert_map(config, san_name,
                                                      cert_key_pair));
    }
  }
...
struct s2n_config *s2n_fetch_default_config(void) {
  if (!default_config_init) {
    ...
    s2n_default_config.client_cert_auth_type =
        S2N_CERT_AUTH_NONE; /* Do not require the client to provide a Cert to
                               the Server */
...
struct s2n_config *s2n_fetch_unsafe_client_testing_config(void) {
  if (!unsafe_client_testing_config_init) {
    ...
    s2n_config_set_cipher_preferences(&s2n_unsafe_client_testing_config,
                                      "default");
...
struct s2n_config *s2n_fetch_unsafe_client_ecdsa_testing_config(void) {
  if (!unsafe_client_ecdsa_testing_config_init) {
    ...
    s2n_config_set_cipher_preferences(&s2n_unsafe_client_ecdsa_testing_config,
                                      "test_all_ecdsa");
    s2n_unsafe_client_ecdsa_testing_config.client_cert_auth_type =
        S2N_CERT_AUTH_NONE;
...
static int s2n_config_store_ticket_key_comparator(const void *a,
                                                  const void *b) {
...
static int s2n_verify_unique_ticket_key_comparator(const void *a,
                                                   const void *b) {
...
int s2n_config_free_cert_chain_and_key(struct s2n_config *config) {
   ...
   * to it. This is necessary until s2n_config_add_cert_chain_and_key is
   * deprecated. */
...
int s2n_config_free_dhparams(struct s2n_config *config) {
  ...
  GUARD(s2n_free_object((uint8_t **)&config->dhparams,
                        sizeof(struct s2n_dh_params)));
  return 0;
}
...
int s2n_config_set_alert_behavior(struct s2n_config *config,
...
int s2n_config_set_max_cert_chain_depth(struct s2n_config *config,
                                        uint16_t max_depth) {
  notnull_check(config);
...
int s2n_config_set_status_request_type(struct s2n_config *config,
  ...
  S2N_ERROR_IF(
      type == S2N_STATUS_REQUEST_OCSP && !s2n_x509_ocsp_stapling_supported(),
...
int s2n_config_set_verification_ca_location(struct s2n_config *config,
                                            ...
                                            const char *ca_dir) {
  notnull_check(config);
  int err_code = s2n_x509_trust_store_from_ca_file(&config->trust_store,
                                                   ca_pem_filename, ca_dir);

  if (!err_code) {
    config->status_request_type = s2n_x509_ocsp_stapling_supported()
...
int s2n_config_add_cert_chain_and_key(struct s2n_config *config,
  ...
  GUARD(s2n_cert_chain_and_key_load_pem(chain_and_key, cert_chain_pem,
                                        private_key_pem));
...
int s2n_config_add_cert_chain_and_key_to_store(
    struct s2n_config *config, struct s2n_cert_chain_and_key *cert_key_pair) {
  notnull_check(config->domain_name_to_cert_map);
  ...
  if (!config->default_certs_are_explicit) {
    /* Attempt to auto set default based on ordering. ie: first RSA cert is the
     * default, first ECDSA cert is the default, etc. */
    s2n_pkey_type cert_type =
        s2n_cert_chain_and_key_get_pkey_type(cert_key_pair);
...
int s2n_config_set_cert_chain_and_key_defaults(
    struct s2n_config *config, struct s2n_cert_chain_and_key **cert_key_pairs,
    uint32_t num_cert_key_pairs) {
  notnull_check(config);
  notnull_check(cert_key_pairs);
  S2N_ERROR_IF(
      num_cert_key_pairs < 1 || num_cert_key_pairs > S2N_CERT_TYPE_COUNT,
      S2N_ERR_NUM_DEFAULT_CERTIFICATES);

  /* Validate certs being set before clearing auto-chosen defaults or previously
   * set defaults */
  struct certs_by_type new_defaults = {{0}};
  for (int i = 0; i < num_cert_key_pairs; i++) {
    notnull_check(cert_key_pairs[i]);
    s2n_pkey_type cert_type =
        s2n_cert_chain_and_key_get_pkey_type(cert_key_pairs[i]);
    S2N_ERROR_IF(new_defaults.certs[cert_type] != NULL,
...
int s2n_config_add_dhparams(struct s2n_config *config,
  ...
  DEFER_CLEANUP(struct s2n_stuffer dhparams_out_stuffer = {0},
                s2n_stuffer_free);
  ...
  GUARD(
      s2n_stuffer_growable_alloc(&dhparams_out_stuffer, strlen(dhparams_pem)));

  /* Convert pem to asn1 and asn1 to the private key */
  GUARD(s2n_stuffer_dhparams_from_pem(&dhparams_in_stuffer,
                                      &dhparams_out_stuffer));

  dhparams_blob.size = s2n_stuffer_data_available(&dhparams_out_stuffer);
  dhparams_blob.data =
      s2n_stuffer_raw_read(&dhparams_out_stuffer, dhparams_blob.size);
...
extern int s2n_config_set_wall_clock(struct s2n_config *config,
...
extern int s2n_config_set_monotonic_clock(struct s2n_config *config,
                                          s2n_clock_time_nanoseconds clock_fn,
                                          void *ctx) {
  notnull_check(clock_fn);
...
int s2n_config_set_cache_store_callback(
    struct s2n_config *config, s2n_cache_store_callback cache_store_callback,
    void *data) {
...
int s2n_config_set_cache_delete_callback(
    struct s2n_config *config, s2n_cache_delete_callback cache_delete_callback,
    void *data) {
...
int s2n_config_set_extension_data(struct s2n_config *config,
  ...
  struct s2n_cert_chain_and_key *config_chain_and_key =
      s2n_config_get_single_default_cert(config);
  notnull_check(config_chain_and_key);

  switch (type) {
    case S2N_EXTENSION_CERTIFICATE_TRANSPARENCY: {
      GUARD(s2n_cert_chain_and_key_set_sct_list(config_chain_and_key, data,
                                                length));
    } break;
    case S2N_EXTENSION_OCSP_STAPLING: {
      GUARD(s2n_cert_chain_and_key_set_ocsp_data(config_chain_and_key, data,
                                                 length));
    } break;
    default:
...
int s2n_config_send_max_fragment_length(struct s2n_config *config,
                                        s2n_max_frag_len mfl_code) {
  notnull_check(config);

  S2N_ERROR_IF(mfl_code > S2N_TLS_MAX_FRAG_LEN_4096,
...
int s2n_config_set_session_state_lifetime(struct s2n_config *config,
  ...
  config->session_state_lifetime_in_nanos =
      (lifetime_in_secs * ONE_SEC_IN_NANOS);
  return 0;
}

int s2n_config_set_session_tickets_onoff(struct s2n_config *config,
                                         uint8_t enabled) {
  notnull_check(config);
...
int s2n_config_set_session_cache_onoff(struct s2n_config *config,
                                       uint8_t enabled) {
  notnull_check(config);
  if (enabled && config->cache_store && config->cache_retrieve &&
      config->cache_delete) {
    GUARD(s2n_config_init_session_ticket_keys(config));
...
int s2n_config_set_ticket_encrypt_decrypt_key_lifetime(
...
int s2n_config_add_ticket_crypto_key(
    struct s2n_config *config, const uint8_t *name, uint32_t name_len,
    uint8_t *key, uint32_t key_len, uint64_t intro_time_in_seconds_from_epoch) {
  notnull_check(config);
  notnull_check(name);
  notnull_check(key);

  /* both session ticket and session cache encryption/decryption can use the
   * same key mechanism */
  ...
  S2N_ERROR_IF(s2n_set_size(config->ticket_keys) >= S2N_MAX_TICKET_KEYS,
               S2N_ERR_TICKET_KEY_LIMIT);

  S2N_ERROR_IF(name_len == 0 || name_len > S2N_TICKET_KEY_NAME_LEN ||
                   s2n_find_ticket_key(config, name),
  ...
  memcpy_check(session_ticket_key->implicit_aad, out_key.data,
               S2N_TICKET_AAD_IMPLICIT_LEN);
  ...
  if (intro_time_in_seconds_from_epoch == 0) {
    ...
    session_ticket_key->intro_timestamp =
        (intro_time_in_seconds_from_epoch * ONE_SEC_IN_NANOS);
  }
