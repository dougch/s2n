...
{
    if (config->ticket_keys != NULL) { GUARD_AS_POSIX(s2n_set_free_p(&config->ticket_keys)); }

    if (config->ticket_key_hashes != NULL) { GUARD_AS_POSIX(s2n_set_free_p(&config->ticket_key_hashes)); }
