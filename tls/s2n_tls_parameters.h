...
/* TLS Hybrid post-quantum definitions from
 * https://tools.ietf.org/html/draft-campagna-tls-bike-sike-hybrid-02 */
...
/* TLS SignatureScheme (Backwards compatible with SigHash and SigAlg values
 * above) */
/* Defined here:
...
/* In TLS 1.0 and 1.1 the hard-coded default scheme was RSA_PKCS1_MD5_SHA1, but
 * there's no IANA defined backwards compatible value for that Scheme for
 * TLS 1.2 and 1.3. So we define an internal value in the private range that
 * won't match anything in the valid range so that all TLS Versions can use the
 * same SignatureScheme negotiation abstraction
 * layer. This scheme isn't in any preference list, so it can't be negotiated
 * even if a client sent it in its pref list. */
...
/* Elliptic curve formats from
 * http://www.iana.org/assignments/tls-parameters/tls-parameters.xhtml#tls-parameters-9
...
#define S2N_TLS_MAXIMUM_RECORD_LENGTH \
  (S2N_TLS_MAXIMUM_FRAGMENT_LENGTH + S2N_TLS_RECORD_HEADER_LENGTH)
...
#define S2N_SSL2_MAXIMUM_RECORD_LENGTH \
  (S2N_SSL2_MAXIMUM_MESSAGE_LENGTH + S2N_SSL2_RECORD_HEADER_LENGTH)

/* s2n can use a "small" record length that is aligned to the dominant internet
 * MTU; 1500 bytes, minus 20 bytes for an IP header, minus 20 bytes for a tcp
 * header and 20 bytes for tcp/ip options (timestamp, sack etc) and a "large"
 * record length that is designed to maximize throughput (fewer MACs per byte
 * transferred and better efficiency of crypto engines).
 */
#define S2N_SMALL_RECORD_LENGTH (1500 - 20 - 20 - 20)
#define S2N_SMALL_FRAGMENT_LENGTH \
  (S2N_SMALL_RECORD_LENGTH - S2N_TLS_RECORD_HEADER_LENGTH)

/* Testing in the wild has found 8k max record sizes give a good balance of low
 * latency and throughput.
 */
#define S2N_DEFAULT_RECORD_LENGTH 8092
#define S2N_DEFAULT_FRAGMENT_LENGTH \
  (S2N_DEFAULT_RECORD_LENGTH - S2N_TLS_RECORD_HEADER_LENGTH)
