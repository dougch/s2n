...
struct s2n_connection {
  ...
  /* Is this connection using CORK/SO_RCVLOWAT optimizations? Only valid when
   * the connection is using managed_io
   */
  ...
  /* Whether server_name extension was used to make a decision on cert
   * selection. RFC6066 Section 3 states that server which used server_name to
   * make a decision on certificate or security settings has to send an empty
   * server_name.
   */
  ...
  /* Whether to use client_cert_auth_type stored in s2n_config or in this
   * s2n_connection.
   *
   * By default the s2n_connection will defer to
   * s2n_config->client_cert_auth_type on whether or not to use Client Auth. But
   * users can override Client Auth at the connection level using
   * s2n_connection_set_client_auth_type() without mutating s2n_config since
   * s2n_config can be shared between multiple s2n_connections. */
  uint8_t client_cert_auth_type_overridden;

  /* Whether or not the s2n_connection should require the Client to authenticate
   * itself to the server. Only used if client_cert_auth_type_overridden is
   * non-zero. */
   ...
   * If this value > 0 then dynamic TLS record size is enabled. Otherwise, the
   * feature is disabled (default).
   */
  uint32_t dynamic_record_resize_threshold;

  /* Reset record size back to a single segment after threshold seconds of
   * inactivity */
   ...
   * In client mode, this will be set after is_handshake_complete(connection) is
   * true.
   */
  ...
  /* After a connection is created this is the verification function that should
   * always be used. At init time, the config should be checked for a verify
   * callback and each connection should default to that. However, from the
   * user's perspective, it's sometimes simpler to manage state by attaching
   * each validation function/data to the connection, instead of globally to a
   * single config.*/
...
extern int s2n_connection_get_cipher_preferences(
    struct s2n_connection *conn,
    const struct s2n_cipher_preferences **cipher_preferences);
extern int s2n_connection_get_protocol_preferences(
    struct s2n_connection *conn, struct s2n_blob **protocol_preferences);
extern int s2n_connection_set_client_auth_type(
    struct s2n_connection *conn, s2n_cert_auth_type cert_auth_type);
extern int s2n_connection_get_client_auth_type(
    struct s2n_connection *conn, s2n_cert_auth_type *client_cert_auth_type);
extern int s2n_connection_get_client_cert_chain(struct s2n_connection *conn,
