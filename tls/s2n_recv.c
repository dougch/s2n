...
int s2n_read_full_record(struct s2n_connection *conn, uint8_t *record_type,
                         int *isSSLv2) {
  int r;
  ...
  *isSSLv2 = 0;
  ...
  while (s2n_stuffer_data_available(&conn->header_in) <
         S2N_TLS_RECORD_HEADER_LENGTH) {
    int remaining = S2N_TLS_RECORD_HEADER_LENGTH -
  ...
  if (conn->header_in.blob.data[0] & 0x80) {
    ...
    *isSSLv2 = 1;
    ...
    if (s2n_sslv2_record_header_parse(conn, record_type,
   ...
   * TLS_APPLICATION_DATA. The actual record content type is found after the
...
ssize_t s2n_recv(struct s2n_connection *conn, void *buf, ssize_t size,
  ...
  while (size && !conn->closed) {
    ...
    if (r < 0) {
      ...
      if (s2n_errno != S2N_ERR_BLOCKED &&
          s2n_allowed_to_cache_connection(conn) && conn->session_id_len) {
...
int s2n_recv_close_notify(struct s2n_connection *conn,
                          s2n_blocked_status *blocked) {
  uint8_t record_type;
  int isSSLv2;
  *blocked = S2N_BLOCKED_ON_READ;
