...
static struct s2n_handshake_action state_machine[] = {
    /* message_type_t           = {Record type   Message type     Writer
       S2N_SERVER                S2N_CLIENT }  */
    [CLIENT_HELLO] = {TLS_HANDSHAKE,
                      TLS_CLIENT_HELLO,
                      'C',
                      {s2n_establish_session, s2n_client_hello_send}},
    [SERVER_HELLO] = {TLS_HANDSHAKE,
                      TLS_SERVER_HELLO,
                      'S',
                      {s2n_server_hello_send, s2n_server_hello_recv}},
    [SERVER_NEW_SESSION_TICKET] = {TLS_HANDSHAKE,
                                   TLS_SERVER_NEW_SESSION_TICKET,
                                   'S',
                                   {s2n_server_nst_send, s2n_server_nst_recv}},
    [SERVER_CERT] = {TLS_HANDSHAKE,
                     TLS_CERTIFICATE,
                     'S',
                     {s2n_server_cert_send, s2n_server_cert_recv}},
    [SERVER_CERT_STATUS] = {TLS_HANDSHAKE,
                            TLS_SERVER_CERT_STATUS,
                            'S',
                            {s2n_server_status_send, s2n_server_status_recv}},
    [SERVER_KEY] = {TLS_HANDSHAKE,
                    TLS_SERVER_KEY,
                    'S',
                    {s2n_server_key_send, s2n_server_key_recv}},
    [SERVER_CERT_REQ] = {TLS_HANDSHAKE,
                         TLS_CERT_REQ,
                         'S',
                         {s2n_client_cert_req_send, s2n_client_cert_req_recv}},
    [SERVER_HELLO_DONE] = {TLS_HANDSHAKE,
                           TLS_SERVER_HELLO_DONE,
                           'S',
                           {s2n_server_done_send, s2n_server_done_recv}},
    [CLIENT_CERT] = {TLS_HANDSHAKE,
                     TLS_CERTIFICATE,
                     'C',
                     {s2n_client_cert_recv, s2n_client_cert_send}},
    [CLIENT_KEY] = {TLS_HANDSHAKE,
                    TLS_CLIENT_KEY,
                    'C',
                    {s2n_client_key_recv, s2n_client_key_send}},
    [CLIENT_CERT_VERIFY] = {TLS_HANDSHAKE,
                            TLS_CERT_VERIFY,
                            'C',
                            {s2n_client_cert_verify_recv,
                             s2n_client_cert_verify_send}},
    [CLIENT_CHANGE_CIPHER_SPEC] = {TLS_CHANGE_CIPHER_SPEC,
                                   0,
                                   'C',
                                   {s2n_client_ccs_recv, s2n_ccs_send}},
    [CLIENT_FINISHED] = {TLS_HANDSHAKE,
                         TLS_FINISHED,
                         'C',
                         {s2n_client_finished_recv, s2n_client_finished_send}},
    [SERVER_CHANGE_CIPHER_SPEC] = {TLS_CHANGE_CIPHER_SPEC,
                                   0,
                                   'S',
                                   {s2n_ccs_send, s2n_server_ccs_recv}},
    [SERVER_FINISHED] = {TLS_HANDSHAKE,
                         TLS_FINISHED,
                         'S',
                         {s2n_server_finished_send, s2n_server_finished_recv}},
...
static struct s2n_handshake_action tls13_state_machine[] = {
    /* message_type_t           = {Record type, Message type, Writer, {Server
    ...
    [SERVER_HELLO] = {TLS_HANDSHAKE,
                      ...
                      'S',
                      {s2n_server_hello_send, s2n_server_hello_recv}},
    [ENCRYPTED_EXTENSIONS] = {TLS_HANDSHAKE,
                              TLS_ENCRYPTED_EXTENSIONS,
                              'S',
                              {s2n_encrypted_extensions_send,
                               s2n_encrypted_extensions_recv}},
    [SERVER_CERT_REQ] = {TLS_HANDSHAKE,
                         TLS_CERT_REQ,
                         'S',
                         {s2n_client_cert_req_send, s2n_client_cert_req_recv}},
    [SERVER_CERT] = {TLS_HANDSHAKE,
                     ...
                     'S',
                     {s2n_server_cert_send, s2n_server_cert_recv}},
    [SERVER_CERT_VERIFY] = {TLS_HANDSHAKE,
                            TLS_CERT_VERIFY,
                            'S',
                            {s2n_server_cert_verify_send,
                             s2n_server_cert_verify_recv}},
    [SERVER_FINISHED] = {TLS_HANDSHAKE,
                         TLS_FINISHED,
                         'S',
                         {s2n_tls13_server_finished_send,
                          s2n_tls13_server_finished_recv}},
    ...
    [CLIENT_FINISHED] = {TLS_HANDSHAKE,
                         ...
                         {s2n_tls13_client_finished_recv,
                          s2n_tls13_client_finished_send}},

    /* Not used by TLS1.3, except to maintain middlebox compatibility */
    [CLIENT_CHANGE_CIPHER_SPEC] = {TLS_CHANGE_CIPHER_SPEC,
                                   0,
                                   'C',
                                   {s2n_basic_ccs_recv, s2n_ccs_send}},
    [SERVER_CHANGE_CIPHER_SPEC] = {TLS_CHANGE_CIPHER_SPEC,
                                   0,
                                   'S',
                                   {s2n_ccs_send, s2n_basic_ccs_recv}},
...
/* We support different ordering of TLS Handshake messages, depending on what is
 * being negotiated. There's also a dummy "INITIAL" handshake that everything
 * starts out as until we know better.
 */

static message_type_t
    handshakes[S2N_HANDSHAKES_COUNT][S2N_MAX_HANDSHAKE_LENGTH] = {
        [INITIAL] = {CLIENT_HELLO, SERVER_HELLO},

        [NEGOTIATED] = {CLIENT_HELLO, SERVER_HELLO, SERVER_CHANGE_CIPHER_SPEC,
                        SERVER_FINISHED, CLIENT_CHANGE_CIPHER_SPEC,
                        CLIENT_FINISHED, APPLICATION_DATA},

        [NEGOTIATED |
            WITH_SESSION_TICKET] = {CLIENT_HELLO, SERVER_HELLO,
        ...
        [NEGOTIATED | FULL_HANDSHAKE |
            WITH_SESSION_TICKET] = {CLIENT_HELLO, SERVER_HELLO, SERVER_CERT,
        ...
        [NEGOTIATED | FULL_HANDSHAKE |
            OCSP_STATUS] = {CLIENT_HELLO, SERVER_HELLO, SERVER_CERT,
                            SERVER_CERT_STATUS, SERVER_HELLO_DONE, CLIENT_KEY,
                            CLIENT_CHANGE_CIPHER_SPEC, CLIENT_FINISHED,
                            SERVER_CHANGE_CIPHER_SPEC, SERVER_FINISHED,
                            APPLICATION_DATA},
        ...
        [NEGOTIATED | FULL_HANDSHAKE | TLS12_PERFECT_FORWARD_SECRECY |
            OCSP_STATUS] = {CLIENT_HELLO, SERVER_HELLO, SERVER_CERT,
                            SERVER_CERT_STATUS, SERVER_KEY, SERVER_HELLO_DONE,
                            CLIENT_KEY, CLIENT_CHANGE_CIPHER_SPEC,
        ...
        [NEGOTIATED | FULL_HANDSHAKE |
            CLIENT_AUTH] = {CLIENT_HELLO, SERVER_HELLO, SERVER_CERT,
        ...
        [NEGOTIATED | FULL_HANDSHAKE | CLIENT_AUTH |
            NO_CLIENT_CERT] = {CLIENT_HELLO, SERVER_HELLO, SERVER_CERT,
                               SERVER_CERT_REQ, SERVER_HELLO_DONE, CLIENT_CERT,
                               CLIENT_KEY, CLIENT_CHANGE_CIPHER_SPEC,
                               CLIENT_FINISHED, SERVER_CHANGE_CIPHER_SPEC,
                               SERVER_FINISHED, APPLICATION_DATA},
        ...
        [NEGOTIATED | FULL_HANDSHAKE | CLIENT_AUTH |
            WITH_SESSION_TICKET] = {CLIENT_HELLO, SERVER_HELLO, SERVER_CERT,
                                    ...
                                    CLIENT_CERT, CLIENT_KEY, CLIENT_CERT_VERIFY,
                                    CLIENT_CHANGE_CIPHER_SPEC, CLIENT_FINISHED,
                                    SERVER_NEW_SESSION_TICKET,
                                    SERVER_CHANGE_CIPHER_SPEC, SERVER_FINISHED,
                                    APPLICATION_DATA},
        ...
        [NEGOTIATED | FULL_HANDSHAKE | TLS12_PERFECT_FORWARD_SECRECY |
            CLIENT_AUTH] = {CLIENT_HELLO, SERVER_HELLO, SERVER_CERT, SERVER_KEY,
                            SERVER_CERT_REQ, SERVER_HELLO_DONE, CLIENT_CERT,
                            CLIENT_KEY, CLIENT_CERT_VERIFY,
                            CLIENT_CHANGE_CIPHER_SPEC, CLIENT_FINISHED,
                            SERVER_CHANGE_CIPHER_SPEC, SERVER_FINISHED,
        ...
        [NEGOTIATED | FULL_HANDSHAKE | TLS12_PERFECT_FORWARD_SECRECY |
            OCSP_STATUS |
            CLIENT_AUTH] = {CLIENT_HELLO, SERVER_HELLO, SERVER_CERT,
                            SERVER_CERT_STATUS, SERVER_KEY, SERVER_CERT_REQ,
                            SERVER_HELLO_DONE, CLIENT_CERT, CLIENT_KEY,
                            CLIENT_CERT_VERIFY, CLIENT_CHANGE_CIPHER_SPEC,
                            CLIENT_FINISHED, SERVER_CHANGE_CIPHER_SPEC,
                            SERVER_FINISHED, APPLICATION_DATA},
...
/*
 * This selection of handshakes resembles the standard set, but with changes
 * made to support tls1.3.
 *
 * These are just the basic handshakes. At the moment hello retries, session
 * resumption, and early data are not supported.
 *
 * The CHANGE_CIPHER_SPEC messages are included only for middlebox
 * compatibility. See https://tools.ietf.org/html/rfc8446#appendix-D.4
 */
static message_type_t
    tls13_handshakes[S2N_HANDSHAKES_COUNT][S2N_MAX_HANDSHAKE_LENGTH] = {
        [INITIAL] = {CLIENT_HELLO, SERVER_HELLO},
...
static char handshake_type_str[S2N_HANDSHAKES_COUNT][MAX_HANDSHAKE_TYPE_LEN] = {
    0};
...
#define ACTIVE_STATE_MACHINE(conn) \
  (IS_TLS13_HANDSHAKE(conn) ? tls13_state_machine : state_machine)
#define ACTIVE_HANDSHAKES(conn) \
  (IS_TLS13_HANDSHAKE(conn) ? tls13_handshakes : handshakes)

#define ACTIVE_MESSAGE(conn) \
  ACTIVE_HANDSHAKES(conn)    \
  [(conn)->handshake.handshake_type][(conn)->handshake.message_number]
...
static int s2n_advance_message(struct s2n_connection *conn) {
  ...
  /* If optimized io hasn't been enabled or if the caller started out with a
   * corked socket, we don't mess with it
   */
  ...
  if (ACTIVE_STATE(conn).writer == previous_writer ||
...
int s2n_generate_new_client_session_id(struct s2n_connection *conn) {
  if (conn->mode == S2N_SERVER) {
    struct s2n_blob session_id = {.data = conn->session_id,
                                  .size = S2N_TLS_SESSION_ID_MAX_LEN};
...
int s2n_conn_set_handshake_type(struct s2n_connection *conn) {
  ...
  if (conn->config->use_tickets) {
    if (conn->session_ticket_status == S2N_DECRYPT_TICKET) {
      ...
      /* If a session ticket is presented by the client, then skip lookup in
       * Session ID server cache */
  ...
  /* If a TLS session is resumed, the Server should respond in its ServerHello
   * with the same SessionId the Client sent in the ClientHello. */
...
skip_cache_lookup:
  ...
  if (conn->mode == S2N_CLIENT &&
    ...
    /* If we're a client, and Client Auth is REQUIRED, then the Client must
     * expect the CLIENT_CERT_REQ Message */
    conn->handshake.handshake_type |= CLIENT_AUTH;
  } else if (conn->mode == S2N_SERVER &&
             client_cert_auth_type != S2N_CERT_AUTH_NONE) {
    /* If we're a server, and Client Auth is REQUIRED or OPTIONAL, then the
     * server must send the CLIENT_CERT_REQ Message*/
    conn->handshake.handshake_type |= CLIENT_AUTH;
  }
...
int s2n_conn_set_handshake_no_client_cert(struct s2n_connection *conn) {
  s2n_cert_auth_type client_cert_auth_type;
  GUARD(s2n_connection_get_client_auth_type(conn, &client_cert_auth_type));
  S2N_ERROR_IF(client_cert_auth_type != S2N_CERT_AUTH_OPTIONAL,
               S2N_ERR_BAD_MESSAGE);
...
const char *s2n_connection_get_handshake_type_name(
...
static int s2n_conn_update_handshake_hashes(struct s2n_connection *conn,
                                            struct s2n_blob *data) {
  if (s2n_handshake_is_hash_required(&conn->handshake, S2N_HASH_MD5)) {
     ...
     * the handshake MD5 hash state is specifically used by the TLS 1.0 and
     * TLS 1.1 PRF, which is required to comply with the TLS 1.0 and 1.1 RFCs
     * and is approved as per NIST Special Publication 800-52 Revision 1.
     */
  ...
  if (md5_sha1_required) {
     ...
     * the handshake hashes. This will only be used for the signature check in
     * the CertificateVerify message and the PRF. NIST SP 800-52r1 approves use
...
static int s2n_conn_post_handshake_hashes_update(struct s2n_connection *conn) {
  ...
  struct s2n_blob client_seq = {
      .data = conn->secure.client_sequence_number,
      .size = sizeof(conn->secure.client_sequence_number)};
  struct s2n_blob server_seq = {
      .data = conn->secure.server_sequence_number,
...
/* Writing is relatively straight forward, simply write each message out as a
 * record, we may fragment a message across multiple records, but we never
 * coalesce multiple messages into single records. Precondition: secure outbound
 * I/O has already been flushed
 */
static int s2n_handshake_write_io(struct s2n_connection *conn) {
   ...
   * Check wiped instead of s2n_stuffer_data_available to differentiate between
   * the initial call to s2n_handshake_write_io and a repeated call after an
   * EWOULDBLOCK.
   */
  ...
  while (s2n_stuffer_data_available(&conn->handshake.io) > 0) {
    ...
    out.size =
        MIN(s2n_stuffer_data_available(&conn->handshake.io), max_payload_size);
    ...
    /* Actually send the record. We could block here. Assume the caller will
     * call flush before coming back. */
 ...
 */
static int s2n_read_full_handshake_message(struct s2n_connection *conn,
                                           uint8_t *message_type) {
  uint32_t current_handshake_data =
      s2n_stuffer_data_available(&conn->handshake.io);
  if (current_handshake_data < TLS_HANDSHAKE_HEADER_LENGTH) {
    /* The message may be so badly fragmented that we don't even read the full
     * header, take what we can and then continue to the next record read
     * iteration.
     */
    if (s2n_stuffer_data_available(&conn->in) <
        (TLS_HANDSHAKE_HEADER_LENGTH - current_handshake_data)) {
      GUARD(s2n_stuffer_copy(&conn->in, &conn->handshake.io,
                             s2n_stuffer_data_available(&conn->in)));
      return 1;
    }

    /* Get the remainder of the header */
    GUARD(s2n_stuffer_copy(
        &conn->in, &conn->handshake.io,
        (TLS_HANDSHAKE_HEADER_LENGTH - current_handshake_data)));
  }

  uint32_t handshake_message_length;
  GUARD(s2n_handshake_parse_header(conn, message_type,
                                   &handshake_message_length));

  S2N_ERROR_IF(handshake_message_length > S2N_MAXIMUM_HANDSHAKE_MESSAGE_LENGTH,
               S2N_ERR_BAD_MESSAGE);

  uint32_t bytes_to_take = handshake_message_length -
                           s2n_stuffer_data_available(&conn->handshake.io);
...
static int s2n_handshake_conn_update_hashes(struct s2n_connection *conn) {
  ...
  handshake_record.size =
      TLS_HANDSHAKE_HEADER_LENGTH + handshake_message_length;
  ...
  if (r < 0) {
    ...
    switch (s2n_errno) {
      ...
      case S2N_ERR_BLOCKED:
        /* A blocking condition is retryable, so we should return without
...
static int s2n_try_delete_session_cache(struct s2n_connection *conn) {
  ...
  if (s2n_allowed_to_cache_connection(conn) > 0) {
    conn->config->cache_delete(conn, conn->config->cache_delete_data,
...
/* Reading is a little more complicated than writing as the TLS RFCs allow
 * content types to be interleaved at the record layer. We may get an alert
 * message during the handshake phase, or messages of types that we don't
 * support (e.g. HEARTBEAT messages), or during renegotiations we may even get
 * application data messages that need to be handled by the application. The
 * latter is punted for now (s2n does not support renegotiations).
 */
static int s2n_handshake_read_io(struct s2n_connection *conn) {
  ...
  /* Now we have a record, but it could be a partial fragment of a message, or
   * it might contain several messages.
   */
  S2N_ERROR_IF(record_type == TLS_APPLICATION_DATA, S2N_ERR_BAD_MESSAGE);
  if (record_type == TLS_CHANGE_CIPHER_SPEC) {
    /* TLS1.2 should not receive unexpected change cipher spec messages, but
     * TLS1.3 might. */
    if (!IS_TLS13_HANDSHAKE(conn)) {
      S2N_ERROR_IF(EXPECTED_RECORD_TYPE(conn) != TLS_CHANGE_CIPHER_SPEC,
                   S2N_ERR_BAD_MESSAGE);
    }

    S2N_ERROR_IF(s2n_stuffer_data_available(&conn->in) != 1,
                 S2N_ERR_BAD_MESSAGE);
  ...
  while (s2n_stuffer_data_available(&conn->in)) {
    ...
    GUARD((r = s2n_read_full_handshake_message(
               conn, &actual_handshake_message_type)));

    /* Do we need more data? This happens for message fragmentation */
    if (r == 1) {
      /* Break out of this inner loop, but since we're not changing the state,
       * the outer loop in s2n_handshake_io() will read another record.
       */
      GUARD(s2n_stuffer_wipe(&conn->header_in));
      GUARD(s2n_stuffer_wipe(&conn->in));
      conn->in_status = ENCRYPTED;
    ...
    /* If we're a Client, and received a ClientCertRequest message, and
     * ClientAuth is set to optional, then switch the State Machine that we're
     * using to expect the ClientCertRequest. */
    if (conn->mode == S2N_CLIENT &&
    ...
    S2N_ERROR_IF(actual_handshake_message_type != EXPECTED_MESSAGE_TYPE(conn),
                 S2N_ERR_BAD_MESSAGE);

    /* Call the relevant handler */
    r = ACTIVE_STATE(conn).handler[conn->mode](conn);

    /* Don't update handshake hashes until after the handler has executed since
     * some handlers need to read the hash values before they are updated. */
...
static int s2n_handle_retry_state(struct s2n_connection *conn) {
  /* If we were blocked reading a record, then the handler is waiting on
   * external data. The handler will know how to continue, so we should call the
   * handler right away. We aren't going to read more handshake data yet because
   * the current message has not finished processing. */
    ...
    if (ACTIVE_STATE(conn).writer == this) {
      *blocked = S2N_BLOCKED_ON_WRITE;
      if (s2n_handshake_write_io(conn) < 0 && s2n_errno != S2N_ERR_BLOCKED) {
        /* Non-retryable write error. The peer might have sent an alert. Try and
         * read it. */
