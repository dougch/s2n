...
/** Initialize trust store from a CA file. This will allocate memory, and load each cert in the file into the trust
 * store Returns 0 on success, or S2N error codes on failure. */
...
/** Initialize the validator in safe mode. Will use trust store to validate x.509 cerficiates, ocsp responses, and will
 * call the verify host callback to determine if a subject name or alternative name from the cert should be trusted.
...
/**
 * Validates a certificate chain against the configured trust store in safe mode. In unsafe mode, it will find the
 * public key and return it but not validate the certificates. Alternative Names and Subject Name will be passed to the
 * host verification callback. The verification callback will be possibly called multiple times depending on how many
 * names are found. If any of those calls return TRUE, that stage of the validation will continue, otherwise once all
 * names are tried and none matched as trusted, the chain will be considered UNTRUSTED
 */
