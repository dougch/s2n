...
/* The names below come from
 * https://tools.ietf.org/html/draft-campagna-tls-bike-sike-hybrid-02#section-5.1.6
 */
 ...
 * support for the KEM extension - that must be added via the cipher
 * preferences.*/
const struct s2n_kem *bike_kems[] = {&s2n_bike1_l1_r1, &s2n_bike1_l1_r2};
...
int s2n_kem_encapsulate(const struct s2n_kem_keypair *kem_keys,
  ...
  GUARD(kem->encapsulate(ciphertext->data, shared_secret->data,
                         kem_keys->public_key.data));
  return 0;
}

int s2n_kem_decapsulate(const struct s2n_kem_keypair *kem_keys,
                        struct s2n_blob *shared_secret,
  ...
  GUARD(s2n_alloc(shared_secret,
                  kem_keys->negotiated_kem->shared_secret_key_length));

  GUARD(kem->decapsulate(shared_secret->data, ciphertext->data,
                         kem_keys->private_key.data));
  return 0;
}
...
static int s2n_kem_check_kem_compatibility(
    ...
    const struct s2n_kem *candidate_kem, uint8_t *kem_is_compatible) {
  ...
  for (uint8_t i = 0; i < compatible_kems->kem_count; i++) {
    if (candidate_kem->kem_extension_id ==
        compatible_kems->kems[i]->kem_extension_id) {
      *kem_is_compatible = 1;
      return 0;
    }
  }
...
int s2n_choose_kem_with_peer_pref_list(
    const uint8_t iana_value[S2N_TLS_CIPHER_SUITE_LEN],
    struct s2n_blob *client_kem_ids,
  ...
  for (uint8_t i = 0; i < num_server_supported_kems; i++) {
    ...
    GUARD(s2n_kem_check_kem_compatibility(iana_value, candidate_server_kem,
                                          &server_kem_is_compatible));
    ...
    for (uint8_t j = 0; j < num_client_candidate_kems; j++) {
      ...
      GUARD(s2n_stuffer_read_uint16(&client_kem_ids_stuffer,
  ...
  /* Client and server did not propose any mutually supported KEMs compatible
   * with the ciphersuite */
  S2N_ERROR(S2N_ERR_KEM_UNSUPPORTED_PARAMS);
}

int s2n_choose_kem_without_peer_pref_list(
    const uint8_t iana_value[S2N_TLS_CIPHER_SUITE_LEN],
    const struct s2n_kem *server_kem_pref_list[],
    const uint8_t num_server_supported_kems,
    const struct s2n_kem **chosen_kem) {
  for (uint8_t i = 0; i < num_server_supported_kems; i++) {
    ...
    GUARD(s2n_kem_check_kem_compatibility(iana_value, server_kem_pref_list[i],
                                          &kem_is_compatible));
  ...
  /* The server preference list did not contain any KEM extensions compatible
   * with the ciphersuite */
  S2N_ERROR(S2N_ERR_KEM_UNSUPPORTED_PARAMS);
}
...
int s2n_cipher_suite_to_kem(const uint8_t iana_value[S2N_TLS_CIPHER_SUITE_LEN],
                            const struct s2n_iana_to_kem **compatible_params) {
  for (int i = 0; i < s2n_array_len(kem_mapping); i++) {
    const struct s2n_iana_to_kem *candidate = &kem_mapping[i];
    if (memcmp(iana_value, candidate->iana_value, S2N_TLS_CIPHER_SUITE_LEN) ==
        0) {
