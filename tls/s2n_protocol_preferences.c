...
int s2n_blob_set_protocol_preferences(struct s2n_blob *application_protocols,
  ...
  for (int i = 0; i < protocol_count; i++) {
    ...
    S2N_ERROR_IF(
        length > 255 || (s2n_stuffer_data_available(&protocol_stuffer) +
                         length + 1) > 65535,
...
int s2n_config_set_protocol_preferences(struct s2n_config *config,
                                        const char *const *protocols,
                                        int protocol_count) {
  return s2n_blob_set_protocol_preferences(&config->application_protocols,
                                           protocols, protocol_count);
}

int s2n_connection_set_protocol_preferences(struct s2n_connection *conn,
                                            const char *const *protocols,
                                            int protocol_count) {
  return s2n_blob_set_protocol_preferences(
      &conn->application_protocols_overridden, protocols, protocol_count);
}
