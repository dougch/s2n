...
int s2n_allowed_to_cache_connection(struct s2n_connection *conn) {
  /* We're unable to cache connections with a Client Cert since we currently
   * don't serialize the Client Cert,
   * which means that callers won't have access to the Client's Cert if the
...
static int s2n_serialize_resumption_state(struct s2n_connection *conn,
                                          struct s2n_stuffer *to) {
  uint64_t now;

  S2N_ERROR_IF(s2n_stuffer_space_remaining(to) < S2N_STATE_SIZE_IN_BYTES,
  ...
  GUARD(s2n_stuffer_write_bytes(to, conn->secure.cipher_suite->iana_value,
                                S2N_TLS_CIPHER_SUITE_LEN));
  GUARD(s2n_stuffer_write_uint64(to, now));
  GUARD(s2n_stuffer_write_bytes(to, conn->secure.master_secret,
                                S2N_TLS_SECRET_LEN));

  return 0;
}

static int s2n_deserialize_resumption_state(struct s2n_connection *conn,
                                            struct s2n_stuffer *from) {
  uint8_t format;
  uint8_t protocol_version;
  uint8_t cipher_suite[S2N_TLS_CIPHER_SUITE_LEN];

  S2N_ERROR_IF(s2n_stuffer_data_available(from) < S2N_STATE_SIZE_IN_BYTES,
               S2N_ERR_STUFFER_OUT_OF_DATA);

  GUARD(s2n_stuffer_read_uint8(from, &format));
  S2N_ERROR_IF(format != S2N_SERIALIZED_FORMAT_VERSION,
               S2N_ERR_INVALID_SERIALIZED_SESSION_STATE);

  GUARD(s2n_stuffer_read_uint8(from, &protocol_version));
  S2N_ERROR_IF(protocol_version != conn->actual_protocol_version,
               S2N_ERR_INVALID_SERIALIZED_SESSION_STATE);

  GUARD(s2n_stuffer_read_bytes(from, cipher_suite, S2N_TLS_CIPHER_SUITE_LEN));
  S2N_ERROR_IF(memcmp(conn->secure.cipher_suite->iana_value, cipher_suite,
                      S2N_TLS_CIPHER_SUITE_LEN),
               S2N_ERR_INVALID_SERIALIZED_SESSION_STATE);
  ...
  S2N_ERROR_IF(now - then > conn->config->session_state_lifetime_in_nanos,
               S2N_ERR_INVALID_SERIALIZED_SESSION_STATE);

  /* Last but not least, put the master secret in place */
  GUARD(s2n_stuffer_read_bytes(from, conn->secure.master_secret,
                               S2N_TLS_SECRET_LEN));

  return 0;
}

static int s2n_client_serialize_resumption_state(struct s2n_connection *conn,
                                                 struct s2n_stuffer *to) {
...
static int s2n_client_deserialize_session_state(struct s2n_connection *conn,
                                                struct s2n_stuffer *from) {
  ...
  uint8_t *cipher_suite_wire =
      s2n_stuffer_raw_read(from, S2N_TLS_CIPHER_SUITE_LEN);
...
static int s2n_client_deserialize_with_session_id(struct s2n_connection *conn,
                                                  struct s2n_stuffer *from) {
...
static int s2n_client_deserialize_with_session_ticket(
    struct s2n_connection *conn, struct s2n_stuffer *from) {
  uint16_t session_ticket_len;
  GUARD(s2n_stuffer_read_uint16(from, &session_ticket_len));

  if (session_ticket_len == 0 ||
      session_ticket_len > s2n_stuffer_data_available(from)) {
    S2N_ERROR(S2N_ERR_INVALID_SERIALIZED_SESSION_STATE);
  }
...
static int s2n_client_deserialize_resumption_state(struct s2n_connection *conn,
                                                   struct s2n_stuffer *from) {
  uint8_t format;
  GUARD(s2n_stuffer_read_uint8(from, &format));
...
int s2n_resume_from_cache(struct s2n_connection *conn) {
  ...
  S2N_ERROR_IF(conn->session_id_len > S2N_TLS_SESSION_ID_MAX_LEN,
               S2N_ERR_SESSION_ID_TOO_LONG);
  size = entry.size;

  GUARD_NONBLOCKING(conn->config->cache_retrieve(
      conn, conn->config->cache_retrieve_data, conn->session_id,
      conn->session_id_len, entry.data, &size));
...
int s2n_store_to_cache(struct s2n_connection *conn) {
  ...
  /* session_id_len should always be >0 since either the Client provided a
   * SessionId or the Server generated a new one for the Client */
  S2N_ERROR_IF(conn->session_id_len == 0, S2N_ERR_SESSION_ID_TOO_SHORT);
  S2N_ERROR_IF(conn->session_id_len > S2N_TLS_SESSION_ID_MAX_LEN,
               S2N_ERR_SESSION_ID_TOO_LONG);
  ...
  /* Store to the cache */
  conn->config->cache_store(conn, conn->config->cache_store_data,
                            S2N_TLS_SESSION_CACHE_TTL, conn->session_id,
                            conn->session_id_len, entry.data, entry.size);

  return 0;
}

int s2n_connection_set_session(struct s2n_connection *conn,
                               const uint8_t *session, size_t length) {
  notnull_check(conn);
...
int s2n_connection_get_session(struct s2n_connection *conn, uint8_t *session,
                               size_t max_length) {
  notnull_check(conn);
  notnull_check(session);
...
int s2n_connection_get_session_ticket_lifetime_hint(
    struct s2n_connection *conn) {
  notnull_check(conn);
  S2N_ERROR_IF(!(conn->config->use_tickets && conn->client_ticket.size > 0),
...
int s2n_connection_get_session_length(struct s2n_connection *conn) {
  /* Session resumption using session ticket "format (1) + session_ticket_len +
   * session_ticket + session state" */
  if (conn->config->use_tickets && conn->client_ticket.size > 0) {
    return S2N_STATE_FORMAT_LEN + S2N_SESSION_TICKET_SIZE_LEN +
           conn->client_ticket.size + S2N_STATE_SIZE_IN_BYTES;
  } else if (conn->session_id_len > 0) {
    /* Session resumption using session id: "format (0) + session_id_len +
     * session_id + session state" */
    return S2N_STATE_FORMAT_LEN + 1 + conn->session_id_len +
...
int s2n_config_is_encrypt_decrypt_key_available(struct s2n_config *config) {
  ...
  for (int i = s2n_set_size(config->ticket_keys) - 1; i >= 0; i--) {
    uint64_t key_intro_time =
        ((struct s2n_ticket_key *)s2n_set_get(config->ticket_keys, i))
            ->intro_timestamp;
...
/* This function is used in s2n_get_ticket_encrypt_decrypt_key to compute the
 * weight of the keys and to choose a single key from all of the encrypt-decrypt
 * keys. Higher the weight of the key, higher the probability of being picked.
 */
int s2n_compute_weight_of_encrypt_decrypt_keys(
    struct s2n_config *config, uint8_t *encrypt_decrypt_keys_index,
    uint8_t num_encrypt_decrypt_keys, uint64_t now) {
  ...
  for (int i = 0; i < num_encrypt_decrypt_keys; i++) {
    uint64_t key_intro_time =
        ((struct s2n_ticket_key *)s2n_set_get(config->ticket_keys,
                                              encrypt_decrypt_keys_index[i]))
            ->intro_timestamp;
    uint64_t key_encryption_peak_time =
        key_intro_time + (config->encrypt_decrypt_key_lifetime_in_nanos / 2);
    ...
    if (now < key_encryption_peak_time) {
      ...
      ticket_keys_weight[i].key_weight =
          (config->encrypt_decrypt_key_lifetime_in_nanos / 2) -
          (now - key_encryption_peak_time);
    }
  ...
  /* Pick a random number in [0, 1). Using 53 bits (IEEE 754 double-precision
   * floats). */
  ...
  for (int i = 0; i < num_encrypt_decrypt_keys; i++) {
    ticket_keys_weight[i].key_weight =
        ticket_keys_weight[i].key_weight / total_weight;
 ...
 */
struct s2n_ticket_key *s2n_get_ticket_encrypt_decrypt_key(
  ...
  int8_t idx;
  GUARD_PTR(
      idx = s2n_compute_weight_of_encrypt_decrypt_keys(
 ...
 */
struct s2n_ticket_key *s2n_find_ticket_key(struct s2n_config *config,
                                           const uint8_t *name) {
  uint64_t now;
  GUARD_PTR(config->wall_clock(config->sys_clock_ctx, &now));
  notnull_check_ptr(config->ticket_keys);
  ...
  for (int i = 0; i < s2n_set_size(config->ticket_keys); i++) {
    if (memcmp(((struct s2n_ticket_key *)s2n_set_get(config->ticket_keys, i))
                   ->key_name,
               name, S2N_TICKET_KEY_NAME_LEN) == 0) {
      /* Check to see if the key has expired */
      if (now >= ((struct s2n_ticket_key *)s2n_set_get(config->ticket_keys, i))
                         ->intro_timestamp +
...
int s2n_encrypt_session_ticket(struct s2n_connection *conn,
                               struct s2n_stuffer *to) {
  ...
  /* No keys loaded by the user or the keys are either in decrypt-only or
   * expired state */
  ...
  GUARD(s2n_stuffer_write_bytes(&aad, key->implicit_aad,
  ...
  GUARD(s2n_aes256_gcm.io.aead.encrypt(&aes_ticket_key, &iv, &aad_blob,
...
int s2n_decrypt_session_ticket(struct s2n_connection *conn) {
  ...
  GUARD(s2n_aes256_gcm.io.aead.decrypt(&aes_ticket_key, &iv, &aad_blob,
                                       &en_blob, &en_blob));
   ...
   * for the ticket.
   */
  if (now >= key->intro_timestamp +
                 conn->config->encrypt_decrypt_key_lifetime_in_nanos) {
...
int s2n_encrypt_session_cache(struct s2n_connection *conn,
                              struct s2n_stuffer *to) {
  return s2n_encrypt_session_ticket(conn, to);
}

int s2n_decrypt_session_cache(struct s2n_connection *conn,
                              struct s2n_stuffer *from) {
  struct s2n_ticket_key *key;
  struct s2n_session_key aes_ticket_key;
  struct s2n_blob aes_key_blob;
...
/* This function is used to remove all or just one expired key from server
 * config */
int s2n_config_wipe_expired_ticket_crypto_keys(struct s2n_config *config,
                                               int8_t expired_key_index) {
...
int s2n_config_store_ticket_key(struct s2n_config *config,
                                struct s2n_ticket_key *key) {
