...
const uint8_t tls12_downgrade_protection_bytes[] = {0x44, 0x4F, 0x57, 0x4E,
                                                    0x47, 0x52, 0x44, 0x01};

const uint8_t tls11_downgrade_protection_bytes[] = {0x44, 0x4F, 0x57, 0x4E,
                                                    0x47, 0x52, 0x44, 0x00};
...
static int s2n_client_detect_downgrade_mechanism(struct s2n_connection *conn) {
  ...
  uint8_t *downgrade_bytes =
      &conn->secure.server_random[S2N_TLS_RANDOM_DATA_LEN -
  ...
  if (conn->client_protocol_version == S2N_TLS13 &&
    ...
    if (s2n_constant_time_equals(downgrade_bytes,
      ...
      S2N_ERROR(S2N_ERR_PROTOCOL_DOWNGRADE_DETECTED);
             ...
             conn->server_protocol_version <= S2N_TLS11) {
    if (s2n_constant_time_equals(downgrade_bytes,
                                 tls11_downgrade_protection_bytes,
                                 S2N_DOWNGRADE_PROTECTION_SIZE)) {
      S2N_ERROR(S2N_ERR_PROTOCOL_DOWNGRADE_DETECTED);
    }
  }
...
static int s2n_server_add_downgrade_mechanism(struct s2n_connection *conn) {
  ...
  if (conn->server_protocol_version >= S2N_TLS13 &&
    ...
    memcpy_check(downgrade_bytes, tls12_downgrade_protection_bytes,
                 S2N_DOWNGRADE_PROTECTION_SIZE);
  } else if (conn->server_protocol_version >= S2N_TLS13 &&
             conn->actual_protocol_version <= S2N_TLS11) {
    /* TLS1.3 servers MUST, use a special random value when negotiating TLS1.1
     * or below */
    memcpy_check(downgrade_bytes, tls11_downgrade_protection_bytes,
                 S2N_DOWNGRADE_PROTECTION_SIZE);
  }
...
int s2n_server_hello_recv(struct s2n_connection *conn) {
  ...
  GUARD(s2n_stuffer_read_bytes(in, protocol_version,
                               S2N_TLS_PROTOCOL_VERSION_LEN));
  GUARD(s2n_stuffer_read_bytes(in, conn->secure.server_random,
                               S2N_TLS_RANDOM_DATA_LEN));

  GUARD(s2n_stuffer_read_uint8(in, &session_id_len));
  S2N_ERROR_IF(session_id_len > S2N_TLS_SESSION_ID_MAX_LEN,
               S2N_ERR_BAD_MESSAGE);
  GUARD(s2n_stuffer_read_bytes(in, session_id, session_id_len));

  uint8_t *cipher_suite_wire =
      s2n_stuffer_raw_read(in, S2N_TLS_CIPHER_SUITE_LEN);
  notnull_check(cipher_suite_wire);

  GUARD(s2n_stuffer_read_uint8(in, &compression_method));
  S2N_ERROR_IF(compression_method != S2N_TLS_COMPRESSION_METHOD_NULL,
               S2N_ERR_BAD_MESSAGE);

  if (s2n_stuffer_data_available(in) >= 2) {
    GUARD(s2n_stuffer_read_uint16(in, &extensions_size));

    S2N_ERROR_IF(extensions_size > s2n_stuffer_data_available(in),
                 S2N_ERR_BAD_MESSAGE);
  ...
  if (conn->server_protocol_version >= S2N_TLS13) {
    ...
    S2N_ERROR_IF(session_id_len != conn->session_id_len ||
    ...
    conn->server_protocol_version =
        (uint8_t)(protocol_version[0] * 10) + protocol_version[1];

    S2N_ERROR_IF(s2n_client_detect_downgrade_mechanism(conn),
    ...
    /* Use the session state if server sent same session id as client sent in
     * client hello */
    if (session_id_len != 0 && session_id_len == conn->session_id_len &&
        !memcmp(session_id, conn->session_id, session_id_len)) {
      /* check if the resumed session state is valid */
      S2N_ERROR_IF(conn->actual_protocol_version != actual_protocol_version,
                   S2N_ERR_BAD_MESSAGE);
      S2N_ERROR_IF(memcmp(conn->secure.cipher_suite->iana_value,
                          cipher_suite_wire, S2N_TLS_CIPHER_SUITE_LEN) != 0,
                   S2N_ERR_BAD_MESSAGE);
      ...
      memset_check((uint8_t *)conn->secure.master_secret, 0,
                   S2N_TLS_SECRET_LEN);

      /* Erase client session ticket which might have been set for session
       * resumption */
...
int s2n_server_hello_send(struct s2n_connection *conn) {
  ...
  rand_data.data =
      s2n_stuffer_raw_write(&server_random, S2N_TLS_RANDOM_DATA_LEN);
  ...
  GUARD(s2n_stuffer_write_bytes(out, protocol_version,
                                S2N_TLS_PROTOCOL_VERSION_LEN));
  GUARD(s2n_stuffer_write_bytes(out, conn->secure.server_random,
                                S2N_TLS_RANDOM_DATA_LEN));
  ...
  GUARD(s2n_stuffer_write_bytes(out, conn->secure.cipher_suite->iana_value,
