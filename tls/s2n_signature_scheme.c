...
const struct s2n_signature_scheme s2n_ecdsa_sha1 = {
    ...
    .signature_curve =
        NULL, /* Decided by supported_groups Extension in TLS 1.2 and before */
};
...
static int s2n_find_signature_pref_from_version(
    const char *version,
...
int s2n_config_set_signature_preferences(struct s2n_config *config,
                                         const char *version) {
  GUARD(s2n_find_signature_pref_from_version(version,
                                             &config->signature_preferences));
  return 0;
}
