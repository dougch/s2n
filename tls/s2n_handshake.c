...
{
    S2N_ERROR_IF( s2n_stuffer_data_available( &conn->handshake.io ) < TLS_HANDSHAKE_HEADER_LENGTH,
...
static int s2n_handshake_get_hash_state_ptr( struct s2n_connection *conn, s2n_hash_algorithm hash_alg,
 ...
 */
int s2n_handshake_get_hash_state( struct s2n_connection *conn, s2n_hash_algorithm hash_alg,
                                  struct s2n_hash_state *hash_state )
{
...
int s2n_create_wildcard_hostname( struct s2n_stuffer *hostname_stuffer, struct s2n_stuffer *output )
{
    ...
    if ( s2n_stuffer_data_available( hostname_stuffer ) == 0 ) { return 0; }
                                  ...
                                  struct s2n_cert_chain_and_key *matches[ S2N_CERT_TYPE_COUNT ], uint8_t *match_exists )
{
    ...
    if ( s2n_map_lookup( domain_name_to_cert_map, dns_name, &map_value ) == 1 ) {
        ...
        for ( int i = 0; i < S2N_CERT_TYPE_COUNT; i++ ) { matches[ i ] = value->certs[ i ]; }
        *match_exists = 1;
    }
...
{
    if ( !s2n_server_received_server_name( conn ) ) { return 0; }
    ...
    GUARD( s2n_find_cert_matches( conn->config->domain_name_to_cert_map, &normalized_name,
 ...
 */
struct s2n_cert_chain_and_key *s2n_get_compatible_cert_chain_and_key( struct s2n_connection *conn,
                                                                      const s2n_pkey_type    cert_type )
{
    if ( conn->handshake_params.exact_sni_match_exists ) {
