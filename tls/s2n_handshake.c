...
int s2n_handshake_write_header(struct s2n_connection *conn,
...
int s2n_handshake_parse_header(struct s2n_connection *conn,
...
int s2n_handshake_get_hash_state(struct s2n_connection *conn,
...
int s2n_handshake_require_all_hashes(struct s2n_handshake *handshake) {
  memset(handshake->required_hash_algs, 1,
         sizeof(handshake->required_hash_algs));
  return 0;
}

static int s2n_handshake_require_hash(struct s2n_handshake *handshake,
                                      s2n_hash_algorithm hash_alg) {
  handshake->required_hash_algs[hash_alg] = 1;
  return 0;
}

uint8_t s2n_handshake_is_hash_required(struct s2n_handshake *handshake,
                                       s2n_hash_algorithm hash_alg) {
  return handshake->required_hash_algs[hash_alg];
}

/* Update the required handshake hash algs depending on current handshake
 * session state. This function must called at the end of a handshake message
 * handler. Additionally it must be called after the ClientHello or ServerHello
 * is processed in client and server mode respectively. The relevant handshake
 * parameters are not available until those messages are processed.
 */
int s2n_conn_update_required_handshake_hashes(struct s2n_connection *conn) {
  /* Clear all of the required hashes */
  memset(conn->handshake.required_hash_algs, 0,
         sizeof(conn->handshake.required_hash_algs));

  message_type_t handshake_message = s2n_conn_get_current_message_type(conn);
  const uint8_t client_cert_verify_done =
      (handshake_message >= CLIENT_CERT_VERIFY) ? 1 : 0;
  s2n_cert_auth_type client_cert_auth_type;
  GUARD(s2n_connection_get_client_auth_type(conn, &client_cert_auth_type));

  /* If client authentication is possible, all hashes are needed until we're
   * past CLIENT_CERT_VERIFY. */
  if ((client_cert_auth_type != S2N_CERT_AUTH_NONE) &&
      !client_cert_verify_done) {
...
/*
 * Take a hostname and return a single "simple" wildcard domain name that
 * matches it. The output wildcard representation is meant to be compared
 * directly against a wildcard domain in a certificate. We take a restrictive
 * definition of wildcard here to achieve a single unique wildcard
 * representation given any input hostname. No embedded or trailing wildcards
 * are supported. Additionally, we only support one level of wildcard matching.
 * Thus the output should be a single wildcard character in the first(left-most)
 * DNS label.
 *
 * Example:
 ...
 * - Certificate selection can be implemented with a constant number of
 * lookups(two).
 */
int s2n_create_wildcard_hostname(struct s2n_stuffer *hostname_stuffer,
  ...
  GUARD(s2n_stuffer_copy(hostname_stuffer, output,
                         s2n_stuffer_data_available(hostname_stuffer)));

  return 0;
}

static int s2n_find_cert_matches(
    struct s2n_map *domain_name_to_cert_map, struct s2n_blob *dns_name,
 ...
 * For a given ServerName there can be multiple matching certificates based on
 * the type of key in the certificate.
 *
...
int s2n_conn_find_name_matching_certs(struct s2n_connection *conn) {
  ...
  struct s2n_blob hostname_blob = {.data = (uint8_t *)(uintptr_t)name,
                                   .size = strlen(name)};
  ...
  struct s2n_blob normalized_name = {.data = (uint8_t *)normalized_hostname,
                                     .size = hostname_blob.size};
  ...
  GUARD(s2n_stuffer_skip_write(&normalized_hostname_stuffer,
                               normalized_name.size));

  /* Find the exact matches for the ServerName */
  GUARD(s2n_find_cert_matches(
      conn->config->domain_name_to_cert_map, &normalized_name,
  ...
  if (!conn->handshake_params.exact_sni_match_exists) {
    /* We have not yet found an exact domain match. Try to find wildcard
     * matches. */
    char wildcard_hostname[S2N_MAX_SERVER_NAME + 1] = {0};
    struct s2n_blob wildcard_blob = {.data = (uint8_t *)wildcard_hostname,
                                     .size = sizeof(wildcard_hostname)};
    struct s2n_stuffer wildcard_stuffer;
    GUARD(s2n_stuffer_init(&wildcard_stuffer, &wildcard_blob));
    GUARD(s2n_create_wildcard_hostname(&normalized_hostname_stuffer,
                                       &wildcard_stuffer));
    ...
    /* The client's SNI is wildcardified, do an exact match against the set of
     * server certs. */
   ...
   * Note that this may have already been set by the client hello callback, so
   * we won't override its value
   */
...
struct s2n_cert_chain_and_key *s2n_get_compatible_cert_chain_and_key(
  ...
  if (conn->handshake_params.exact_sni_match_exists) {
    /* This may return NULL if there was an SNI match, but not a match the
     * cipher_suite's authentication type. */
  ...
  if (conn->handshake_params.wc_sni_match_exists) {
    ...
    /* We don't have any name matches. Use the default certificate that works
     * with the key type. */
