...
__inline void fpcopy(const felm_t a, felm_t c)
{
    ...
    for (i = 0; i < NWORDS_FIELD; i++) c[i] = a[i];
}
...
unsigned int mp_sub(const digit_t *a, const digit_t *b, digit_t *c, const unsigned int nwords)
{
    ...
    for (i = 0; i < nwords; i++) SUBC(borrow, a[i], b[i], borrow, c[i]);

    return borrow;
}

// Multiprecision subtraction followed by addition with p*2^MAXBITS_FIELD, c = a-b+(p*2^MAXBITS_FIELD) if a-b < 0,
// otherwise c=a-b.
__inline static void mp_subaddfast(const digit_t *a, const digit_t *b, digit_t *c)
{
    ...
    for (int i = 0; i < NWORDS_FIELD; i++) t1[i] = ((const digit_t *)PRIME)[i] & mask;
