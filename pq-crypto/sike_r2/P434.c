...
// obtained by approximating the number of bits to the immediately greater multiple of 32. For example, a 434-bit field
// element is represented with Ceil(434 / 64) = 7 64-bit digits or Ceil(434 / 32) = 14 32-bit digits.

//
// Curve isogeny system "SIDHp434". Base curve: Montgomery curve By^2 = Cx^3 + Ax^2 + Cx defined over GF(p434^2), where
// A=6, B=1, C=1 and p434 = 2^216*3^137-1
//
...
const uint64_t Bob_order[NWORDS64_ORDER] = {0x58AEA3FDC1767AE3, 0xC520567BC65C7831, 0x1773446CFC5FD681,
                                            0x0000000002341F27};
// Alice's generator values {XPA0 + XPA1*i, XQA0 + xQA1*i, XRA0 + XRA1*i} in GF(p434^2), expressed in Montgomery
// representation
...
int s2n_check_sike434_r2_asm_compatibility()
{
     ...
     *
     * In our function definition, we are explicitly separating leaf from eax; leaf is the input
     * and eax will contain the output. We need the returned eax >= 7 to perform the extended features check below.
     */
