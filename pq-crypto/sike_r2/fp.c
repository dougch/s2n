...
void fpadd434( const digit_t *a, const digit_t *b, digit_t *c )
{
    ...
    carry = 0;
    for ( i = 0; i < NWORDS_FIELD; i++ ) { SUBC( carry, c[ i ], ( ( const digit_t * )p434x2 )[ i ], carry, c[ i ] ); }
...
void fpneg434( digit_t *a )
{
    ...
    for ( i = 0; i < NWORDS_FIELD; i++ ) { SUBC( borrow, ( ( const digit_t * )p434x2 )[ i ], a[ i ], borrow, a[ i ] ); }
}
...
void fpcorrection434( digit_t *a )
{
    ...
    for ( i = 0; i < NWORDS_FIELD; i++ ) { SUBC( borrow, a[ i ], ( ( const digit_t * )p434 )[ i ], borrow, a[ i ] ); }
    mask = 0 - ( digit_t )borrow;
