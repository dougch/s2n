...
{  // Computes [2^e](X:Z) on Montgomery curve with projective constant via e repeated doublings.
   // Input: projective Montgomery x-coordinates P = (XP:ZP), such that xP=XP/ZP and Montgomery curve constants A+2C and
   // 4C. Output: projective Montgomery x-coordinates Q <- (2^e)*P.
    int i;
...
{  // Tripling of a Montgomery point in projective coordinates (X:Z).
   // Input: projective Montgomery x-coordinates P = (X:Z), where x=X/Z and Montgomery curve constants A24plus = A+2C
   // and A24minus = A-2C. Output: projective Montgomery x-coordinates Q = 3*P = (X3:Z3).
    ...
    fp2add(t3, t1, t2);   // t2 = [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2] + A24minus*(X-Z)^3 -
                          // coeff*(X+Z)^3
    fp2sqr_mont(t2, t2);  // t2 = t2^2
    fp2mul_mont(t4, t2, &Q->X);  // X3 = 2*X*t2
    fp2sub(t3, t1, t1);   // t1 = A24minus*(X-Z)^3 - A24plus*(X+Z)^3 - [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 -
                          // A24minus*(X-Z)^2]
...
{  // Computes [3^e](X:Z) on Montgomery curve with projective constant via e repeated triplings.
   // Input: projective Montgomery x-coordinates P = (XP:ZP), such that xP=XP/ZP and Montgomery curve constants A24plus
   // = A+2C and A24minus = A-2C. Output: projective Montgomery x-coordinates Q <- (3^e)*P.
    int i;
...
{  // Computes the corresponding 3-isogeny of a projective Montgomery point (X3:Z3) of order 3.
   ...
   // Output: the 3-isogenous Montgomery curve with projective coefficient A/C.
    ...
    fp2sub(
        t4, A24minus,
        t0);  // t0 = [4*X^2 - (X+Z)^2]*[8*X^2 + 2*(X+Z)^2 - (X-Z)^2] - [4*X^2 - (X-Z)^2]*[8*X^2 - (X+Z)^2 + 2*(X-Z)^2]
...
{  // Given the x-coordinates of P, Q, and R, returns the value A corresponding to the Montgomery curve E_A:
   // y^2=x^3+A*x^2+x such that R=Q-P on E_A. Input:  the x-coordinates xP, xQ, and xR of the points P, Q and R. Output:
...
{  // Simultaneous doubling and differential addition.
   // Input: projective Montgomery points P=(XP:ZP) and Q=(XQ:ZQ) such that xP=XP/ZP and xQ=XQ/ZQ, affine difference
   // xPQ=x(P-Q) and Montgomery curve constant A24=(A+2)/4. Output: projective Montgomery points P <- 2*P = (X2P:Z2P)
   // such that x(2P)=X2P/Z2P, and Q <- P+Q = (XQP:ZQP) such that = x(Q+P)=XQP/ZQP.
    f2elm_t _t0, _t1, _t2;
    f2elm_t *t0 = &_t0, *t1 = &_t1, *t2 = &_t2;
...
static void LADDER3PT(const f2elm_t *xP, const f2elm_t *xQ, const f2elm_t *xPQ, const digit_t *m,
