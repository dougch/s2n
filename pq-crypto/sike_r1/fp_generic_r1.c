...
{   // Modular addition, c = a+b mod p503.
    ...
    carry = 0;
    for ( i = 0; i < NWORDS_FIELD; i++ ) { SUBC( carry, c[ i ], ( ( const digit_t * )p503x2 )[ i ], carry, c[ i ] ); }
...
{   // Modular negation, a = -a mod p503.
    ...
    for ( i = 0; i < NWORDS_FIELD; i++ ) { SUBC( borrow, ( ( const digit_t * )p503x2 )[ i ], a[ i ], borrow, a[ i ] ); }
}
...
{  // Modular correction to reduce field element a in [0, 2*p503-1] to [0, p503-1].
    ...
    for ( i = 0; i < NWORDS_FIELD; i++ ) { SUBC( borrow, a[ i ], ( ( const digit_t * )p503 )[ i ], borrow, a[ i ] ); }
    mask = 0 - ( digit_t )borrow;
