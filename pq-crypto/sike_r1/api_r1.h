...
// ----------------------------------------------------------------------
// Elements over GF(p503) are encoded in 63 octets in little endian format (i.e., the least significant octet is located
// in the lowest memory address). Elements (a+b*i) over GF(p503^2), where a and b are defined over GF(p503), are encoded
// as {a, b}, with a in the lowest memory portion.
//
// Private keys sk consist of the concatenation of a 24-byte random value, a value in the range [0, 2^252-1] and the
// public key pk. In the SIKE API, private keys are encoded in 434 octets in little endian format. Public keys pk
// consist of 3 elements in GF(p503^2). In the SIKE API, pk is encoded in 378 octets. Ciphertexts ct consist of the
// concatenation of a public key value and a 24-byte value. In the SIKE API, ct is encoded in 378 + 24 = 402 octets.
...
// See "On the Security of Supersingular Isogeny Cryptosystems", S.D. Galbraith, C. Petit, B. Shani and Y.B. Ti, in
// ASIACRYPT 2016, 2016. Extended version available at: http://eprint.iacr.org/2016/859
...
int EphemeralSecretAgreement_A(const digit_t *PrivateKeyA, const unsigned char *PublicKeyB,
...
int EphemeralSecretAgreement_B(const digit_t *PrivateKeyB, const unsigned char *PublicKeyA,
...
// as {a, b}, with a in the lowest memory portion.
//
// Private keys PrivateKeyA and PrivateKeyB can have values in the range [0, 2^250-1] and [0, 2^252-1], resp. In the
// SIDH API, private keys are encoded in 32 octets in little endian format. Public keys PublicKeyA and PublicKeyB
// consist of 3 elements in GF(p503^2). In the SIDH API, they are encoded in 378 octets. Shared keys SharedSecretA and
// SharedSecretB consist of one element in GF(p503^2). In the SIDH API, they are encoded in 126 octets.
