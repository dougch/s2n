...
_INLINE_ uint32_t secure_cmp( IN const uint8_t *a, IN const uint8_t *b, IN const uint32_t size )
{
    volatile uint8_t res = 0;

    for ( uint32_t i = 0; i < size; ++i ) { res |= ( a[ i ] ^ b[ i ] ); }
...
_INLINE_ uint32_t iszero( IN const uint8_t *s, IN const uint32_t len )
{
    ...
    for ( uint64_t i = 0; i < len; i++ ) { res |= s[ i ]; }
    return ( 0 == res );
}
