...
_INLINE_ void rotr_big(OUT syndrome_t *out, IN const syndrome_t *in, IN size_t qw_num)
{
    // For preventing overflows (comparison in bytes)
    bike_static_assert(sizeof(*out) > 8 * (R_QW + (2 * R_QW_HALF_LOG2)), rotr_big_err);
...
_INLINE_ void rotr_small(OUT syndrome_t *out, IN const syndrome_t *in, IN const size_t bits)
{
...
void rotate_right(OUT syndrome_t *out, IN const syndrome_t *in, IN const uint32_t bitscount)
{
