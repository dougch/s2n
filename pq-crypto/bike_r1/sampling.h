...
typedef enum { NO_RESTRICTION = 0, MUST_BE_ODD = 1 } must_be_odd_t;
...
// No restrictions exist for the top or bottom bits -
// in case an odd number is  requried then set must_be_odd=1
_INLINE_ ret_t sample_uniform_r_bits( OUT r_t *r, IN const seed_t *seed, IN const must_be_odd_t must_be_odd )
{
...
ret_t generate_sparse_rep( OUT uint64_t *a, OUT idx_t wlist[], IN uint32_t weight, IN uint32_t len,
                           IN uint32_t padded_len, IN OUT aes_ctr_prf_state_t *prf_state );

EXTERNC void secure_set_bits( IN OUT uint64_t *a, IN const idx_t wlist[], IN uint32_t a_len, IN uint32_t weight );
