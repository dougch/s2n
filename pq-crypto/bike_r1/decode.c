...
_INLINE_ ret_t recompute_syndrome( OUT syndrome_t *syndrome, IN const ct_t *ct, IN const sk_t *sk,
...
_INLINE_ uint8_t get_threshold( IN const syndrome_t *s )
{
    ...
    const uint8_t threshold = THRESHOLD_COEFF0 + ( THRESHOLD_COEFF1 * syndrome_weight );
...
_INLINE_ void bit_sliced_adder( OUT upc_t *upc, IN OUT syndrome_t *rotated_syndrome, IN const size_t num_of_slices )
{
...
_INLINE_ void find_err1( OUT split_e_t *e, OUT split_e_t *black_e, OUT split_e_t *gray_e, IN const syndrome_t *syndrome,
                         IN const compressed_idx_dv_ar_t wlist, IN const uint8_t threshold )
{
...
_INLINE_ void find_err2( OUT split_e_t *e, IN split_e_t *pos_e, IN const syndrome_t *syndrome,
                         IN const compressed_idx_dv_ar_t wlist, IN const uint8_t threshold )
{
    DEFER_CLEANUP( syndrome_t rotated_syndrome = { 0 }, syndrome_cleanup );
    DEFER_CLEANUP( upc_t upc, upc_cleanup );
...
ret_t decode( OUT split_e_t *e, IN const syndrome_t *original_s, IN const ct_t *ct, IN const sk_t *sk )
{
    split_e_t  black_e = { 0 };
    split_e_t  gray_e  = { 0 };
    syndrome_t s;
    ...
    if ( r_bits_vector_weight( ( r_t * )s.qw ) > 0 ) { BIKE_ERROR( E_DECODING_FAILURE ); }
