...
ret_t init_aes_ctr_prf_state( OUT aes_ctr_prf_state_t *s, IN const uint32_t max_invokations, IN const seed_t *seed )
{
    if ( 0 == max_invokations ) { BIKE_ERROR( E_AES_CTR_PRF_INIT_FAIL ); }
...
_INLINE_ ret_t perform_aes( OUT uint8_t *ct, IN OUT aes_ctr_prf_state_t *s )
{
    ...
    if ( 0 == s->rem_invokations ) { BIKE_ERROR( E_AES_OVER_USED ); }
