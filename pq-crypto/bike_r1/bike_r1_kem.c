...
_INLINE_ void merge_e(OUT e_t *e, IN const split_e_t *splitted_e)
{
    memcpy(e->raw, splitted_e->val[0].raw, R_SIZE);

    e->raw[R_SIZE - 1] = ((splitted_e->val[1].raw[0] << LAST_R_BYTE_LEAD) | (e->raw[R_SIZE - 1] & LAST_R_BYTE_MASK));

    // Now load second value
    for (uint32_t i = 1; i < R_SIZE; ++i) {
        e->raw[R_SIZE + i - 1] =
            ((splitted_e->val[1].raw[i] << LAST_R_BYTE_LEAD) | (splitted_e->val[1].raw[i - 1] >> LAST_R_BYTE_TRAIL));
    }
...
_INLINE_ ret_t encrypt(OUT ct_t *ct, IN const pk_t *pk, IN const seed_t *seed, IN const split_e_t *splitted_e)
{
    ...
    GUARD(gf2x_add(p_ct[0].val.raw, p_ct[0].val.raw, splitted_e->val[0].raw, R_SIZE));
    GUARD(gf2x_add(p_ct[1].val.raw, p_ct[1].val.raw, splitted_e->val[1].raw, R_SIZE));
