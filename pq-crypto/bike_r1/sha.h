...
_INLINE_ void sha_mb( OUT sha_hash_t *hash_out, IN const uint8_t *msg, IN const uint32_t byte_len,
                      IN const uint32_t num )
{
    const uint32_t ls = ( byte_len / NUM_OF_BLOCKS_IN_MB );

    // Hash each block (X[i])
    for ( uint32_t i = 0; i < num; i++ ) { SHA384( &msg[ i * ls ], ls, hash_out[ i ].u.raw ); }
}
