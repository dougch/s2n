...
_INLINE_ void split_e(OUT split_e_t *splitted_e, IN const e_t *e)
{
    // Copy lower bytes (e0)
    ...
    for (uint32_t i = R_SIZE; i < N_SIZE; ++i) {
        splitted_e->val[1].raw[i - R_SIZE] = ((e->raw[i] << LAST_R_BYTE_TRAIL) | (e->raw[i - 1] >> LAST_R_BYTE_LEAD));
    }
...
_INLINE_ void translate_hash_to_ss(OUT ss_t *ss, IN sha_hash_t *hash)
{
    bike_static_assert(sizeof(*hash) >= sizeof(*ss), hash_size_lt_ss_size);
    memcpy(ss->raw, hash->u.raw, sizeof(*ss));
}

_INLINE_ void translate_hash_to_seed(OUT seed_t *seed, IN sha_hash_t *hash)
{
    bike_static_assert(sizeof(*hash) >= sizeof(*seed), hash_size_lt_seed_size);
    memcpy(seed->raw, hash->u.raw, sizeof(*seed));
}

_INLINE_ ret_t calc_pk(OUT pk_t *pk, IN const seed_t *g_seed, IN const pad_sk_t p_sk)
{
...
// e from (m*f0, m*f1):
_INLINE_ ret_t function_h(OUT split_e_t *splitted_e, IN const r_t *in0, IN const r_t *in1)
{
...
_INLINE_ ret_t encrypt(OUT ct_t *ct, OUT split_e_t *mf, IN const pk_t *pk, IN const seed_t *seed)
{
...
_INLINE_ ret_t reencrypt(OUT pad_ct_t ce, OUT split_e_t *e2, IN const split_e_t *e, IN const ct_t *l_ct)
{
...
_INLINE_ void get_ss(OUT ss_t *out, IN const r_t *in0, IN const r_t *in1, IN const ct_t *ct)
{
    DMSG("    Enter get_ss.\n");
...
int BIKE1_L1_R2_crypto_kem_keypair(OUT unsigned char *pk, OUT unsigned char *sk)
{
    ...
    GUARD(generate_sparse_rep((uint64_t *)&p_sk[0], l_sk.wlist[0].val, DV, R_BITS, sizeof(p_sk[0]), &h_prf_state));

    // Sample the sigmas
    GUARD(sample_uniform_r_bits_with_fixed_prf_context(&l_sk.sigma0, &s_prf_state, NO_RESTRICTION));
    GUARD(sample_uniform_r_bits_with_fixed_prf_context(&l_sk.sigma1, &s_prf_state, NO_RESTRICTION));

    GUARD(generate_sparse_rep((uint64_t *)&p_sk[1], l_sk.wlist[1].val, DV, R_BITS, sizeof(p_sk[1]), &h_prf_state));
...
int BIKE1_L1_R2_crypto_kem_dec(OUT unsigned char *ss, IN const unsigned char *ct, IN const unsigned char *sk)
{
    ...
    success_cond &= secure_cmp32(T1, r_bits_vector_weight(&e.val[0]) + r_bits_vector_weight(&e.val[1]));
