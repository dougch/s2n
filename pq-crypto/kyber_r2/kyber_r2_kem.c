...
int kyber_512_r2_crypto_kem_keypair(uint8_t *pk, uint8_t *sk)
{
    ...
    for (i = 0; i < KYBER_INDCPA_PUBLICKEYBYTES; i++) { sk[ i + KYBER_INDCPA_SECRETKEYBYTES ] = pk[ i ]; }
    hash_h(sk + KYBER_SECRETKEYBYTES - 2 * KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES);
    GUARD_AS_POSIX(get_random_bytes(sk + KYBER_SECRETKEYBYTES - KYBER_SYMBYTES,
...
int kyber_512_r2_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk)
{
    ...
    hash_h(buf + KYBER_SYMBYTES, pk,
           KYBER_PUBLICKEYBYTES); /* Multitarget countermeasure for coins + contributory KEM */
...
int kyber_512_r2_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk)
{
    ...
    for (i = 0; i < KYBER_SYMBYTES; i++) { /* Multitarget countermeasure for coins + contributory KEM */
        buf[ KYBER_SYMBYTES + i ] =
            sk[ KYBER_SECRETKEYBYTES - 2 * KYBER_SYMBYTES + i ]; /* Save hash by storing H(pk) in sk */
    }
    hash_g(kr, buf, 2 * KYBER_SYMBYTES);
    ...
    PQCLEAN_KYBER512_CLEAN_cmov(kr, sk + KYBER_SECRETKEYBYTES - KYBER_SYMBYTES, KYBER_SYMBYTES,
