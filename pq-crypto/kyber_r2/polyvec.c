    ...
    for (size_t i = 0; i < KYBER_K; i++) {
        for (size_t j = 0; j < KYBER_N / 4; j++) {
            r->vec[ i ].coeffs[ 4 * j + 0 ] =
                (int16_t)((((a[ 5 * j + 0 ] | ((( uint32_t )a[ 5 * j + 1 ] & 0x03) << 8)) * KYBER_Q) + 512) >> 10);
            r->vec[ i ].coeffs[ 4 * j + 1 ] = (int16_t)(
                ((((a[ 5 * j + 1 ] >> 2) | ((( uint32_t )a[ 5 * j + 2 ] & 0x0f) << 6)) * KYBER_Q) + 512) >> 10);
            r->vec[ i ].coeffs[ 4 * j + 2 ] = (int16_t)(
                ((((a[ 5 * j + 2 ] >> 4) | ((( uint32_t )a[ 5 * j + 3 ] & 0x3f) << 4)) * KYBER_Q) + 512) >> 10);
            r->vec[ i ].coeffs[ 4 * j + 3 ] = (int16_t)(
                ((((a[ 5 * j + 3 ] >> 6) | ((( uint32_t )a[ 5 * j + 4 ] & 0xff) << 2)) * KYBER_Q) + 512) >> 10);
        }
        a += 320;
    }
}
...
{
    for (size_t i = 0; i < KYBER_K; i++) { PQCLEAN_KYBER512_CLEAN_poly_tobytes(r + i * KYBER_POLYBYTES, &a->vec[ i ]); }
}
...
{
    for (size_t i = 0; i < KYBER_K; i++) { PQCLEAN_KYBER512_CLEAN_poly_ntt(&r->vec[ i ]); }
}
...
{
    for (size_t i = 0; i < KYBER_K; i++) { PQCLEAN_KYBER512_CLEAN_poly_invntt(&r->vec[ i ]); }
}
...
{
    for (size_t i = 0; i < KYBER_K; i++) { PQCLEAN_KYBER512_CLEAN_poly_reduce(&r->vec[ i ]); }
}
...
{
    for (size_t i = 0; i < KYBER_K; i++) { PQCLEAN_KYBER512_CLEAN_poly_csubq(&r->vec[ i ]); }
}
...
{
    for (size_t i = 0; i < KYBER_K; i++) { PQCLEAN_KYBER512_CLEAN_poly_add(&r->vec[ i ], &a->vec[ i ], &b->vec[ i ]); }
}
