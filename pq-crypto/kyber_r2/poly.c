...
**************************************************/
void PQCLEAN_KYBER512_CLEAN_poly_invntt(poly *r) { PQCLEAN_KYBER512_CLEAN_invntt(r->coeffs); }
...
void PQCLEAN_KYBER512_CLEAN_poly_basemul(poly *r, const poly *a, const poly *b)
{
    for (size_t i = 0; i < KYBER_N / 4; ++i) {
        PQCLEAN_KYBER512_CLEAN_basemul(r->coeffs + 4 * i, a->coeffs + 4 * i, b->coeffs + 4 * i,
...
{
    for (size_t i = 0; i < KYBER_N; i++) { r->coeffs[ i ] = PQCLEAN_KYBER512_CLEAN_barrett_reduce(r->coeffs[ i ]); }
}
...
{
    for (size_t i = 0; i < KYBER_N; i++) { r->coeffs[ i ] = PQCLEAN_KYBER512_CLEAN_csubq(r->coeffs[ i ]); }
}
...
{
    for (size_t i = 0; i < KYBER_N; i++) { r->coeffs[ i ] = a->coeffs[ i ] + b->coeffs[ i ]; }
}
