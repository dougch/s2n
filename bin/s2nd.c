...
uint8_t default_ticket_key[ 32 ] = { 0x07, 0x77, 0x09, 0x36, 0x2c, 0x2e, 0x32, 0xdf, 0x0d, 0xdc, 0x3f,
                             ...
                             uint64_t *value_size )
{
    ...
    *value_size = cache[ index ].value_len;
    ...
    for ( int i = 0; i < key_size; i++ ) { printf( "%02x", ( ( const uint8_t * )key )[ i ] ); }
    printf( "\n" );
...
int cache_delete_callback( struct s2n_connection *conn, void *ctx, const void *key, uint64_t key_size )
{
    ...
    S2N_ERROR_IF( cache[ index ].key_len != 0 && memcmp( cache[ index ].key, key, key_size ),
 ...
 */
static uint8_t unsafe_verify_host_fn( const char *host_name, size_t host_name_len, void *data ) { return 1; }
...
void usage()
{
    ...
    fprintf( stderr, "  --cert\n" );
    fprintf( stderr,
             "    Path to a PEM encoded certificate [chain]. Option can be repeated to load multiple certs.\n" );
    fprintf( stderr, "  --key\n" );
    fprintf(
        stderr,
    ...
    fprintf( stderr,
             "    Location of trust store CA file (PEM format). If neither -t or -d are specified. System defaults "
             "will be used." );
    fprintf( stderr, "    This option is only used if mutual auth is enabled.\n" );
    fprintf( stderr, "  -d,--ca-dir [directory path]\n" );
    fprintf( stderr,
             "    Directory containing hashed trusted certs. If neither -t or -d are specified. System defaults will "
             "be used." );
    fprintf( stderr, "    This option is only used if mutual auth is enabled.\n" );
...
int handle_connection( int fd, struct s2n_config *config, struct conn_settings settings )
{
    ...
    if ( settings.self_service_blinding ) { s2n_connection_set_blinding( conn, S2N_SELF_SERVICE_BLINDING ); }
    ...
    if ( settings.use_corked_io ) { GUARD_RETURN( s2n_connection_use_corked_io( conn ), "Error setting corked io" ); }
    ...
    if ( !settings.only_negotiate ) { echo( conn, fd ); }
