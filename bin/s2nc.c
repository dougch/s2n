...
void usage()
{
    ...
    fprintf(stderr,
            ...
            "    The client will generate keyshares only for the curve names present in the ecc_preferences list "
            "configured in the security_policy.\n"
            "    The curves currently supported by s2n are: `x25519`, `secp256r1` and `secp384r1`. Note that `none` "
            "represents a list of empty keyshares.\n"
...
int main(int argc, char *const *argv)
{
        ...
        switch (c) {
            ...
            case 'K':
                ...
                while (token != NULL) {
                    ...
                    if (++keyshares_count == S2N_ECC_EVP_SUPPORTED_CURVES_COUNT) { break; }
                    token = strtok(NULL, ":");
                }
                break;
            case 'n':
    ...
    do {
        ...
        if (client_cert_input != client_key_input) { print_s2n_error("Client cert/key pair must be given."); }
        ...
        for (size_t i = 0; i < keyshares_count; i++) {
            if (keyshares[ i ]) {
                GUARD_EXIT(s2n_connection_set_keyshare_by_name_for_testing(conn, keyshares[ i ]),
                           "Error setting keyshares to generate");
            }
        }
