...
const struct s2n_ecc_named_curve s2n_ecc_curve_secp256r1 = { .iana_id       = TLS_EC_CURVE_SECP_256_R1,
                                                             .libcrypto_nid = NID_X9_62_prime256v1,
                                                             .name          = "secp256r1",
                                                             .share_size    = (32 * 2) + 1 };

const struct s2n_ecc_named_curve s2n_ecc_curve_secp384r1 = { .iana_id       = TLS_EC_CURVE_SECP_384_R1,
                                                             .libcrypto_nid = NID_secp384r1,
                                                             .name          = "secp384r1",
                                                             .share_size    = (48 * 2) + 1 };

#if EVP_APIS_SUPPORTED
const struct s2n_ecc_named_curve s2n_ecc_curve_x25519 = { .iana_id       = TLS_EC_CURVE_ECDH_X25519,
...
int s2n_is_evp_apis_supported() { return EVP_APIS_SUPPORTED; }
...
static int s2n_ecc_evp_compute_shared_secret(EVP_PKEY *own_key, EVP_PKEY *peer_public, uint16_t iana_id,
...
{
#if EVP_APIS_SUPPORTED
    if (named_curve->libcrypto_nid == NID_X25519) { return s2n_ecc_evp_generate_key_x25519(named_curve, evp_pkey); }
#endif
                                                ...
                                                struct s2n_blob *shared_key)
{
    ...
    if (ecc_evp_params->negotiated_curve->libcrypto_nid == NID_X25519) {
        ...
        GUARD_OSSL(EVP_PKEY_CTX_set_ec_paramgen_curve_nid(pctx, ecc_evp_params->negotiated_curve->libcrypto_nid),
                                                ...
                                                struct s2n_blob *shared_key)
{
    ...
    if (s2n_ecc_evp_write_params_point(&client_params, Yc_out) != 0) { S2N_ERROR(S2N_ERR_ECDHE_SERIALIZING); }
    return 0;
}
...
static EC_POINT *s2n_ecc_evp_blob_to_point(struct s2n_blob *blob, const EC_KEY *ec_key)
{
    ...
    if (point == NULL) { S2N_ERROR_PTR(S2N_ERR_ECDHE_UNSUPPORTED_CURVE); }
...
#if EVP_APIS_SUPPORTED
    if (ecc_evp_params->negotiated_curve->libcrypto_nid == NID_X25519) {
        if (ecc_evp_params->evp_pkey == NULL) { ecc_evp_params->evp_pkey = EVP_PKEY_new(); }
