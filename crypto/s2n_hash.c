...
int s2n_hash_is_ready_for_input( struct s2n_hash_state *state ) { return state->is_ready_for_input; }
...
static int s2n_low_level_hash_digest( struct s2n_hash_state *state, void *out, uint32_t size )
{
    ...
    switch ( state->alg ) {
        ...
        case S2N_HASH_MD5_SHA1:
            ...
            GUARD_OSSL( SHA1_Final( ( ( uint8_t * )out ) + MD5_DIGEST_LENGTH, &state->digest.low_level.md5_sha1.sha1 ),
...
static int s2n_evp_hash_init( struct s2n_hash_state *state, s2n_hash_algorithm alg )
{
    switch ( alg ) {
        case S2N_HASH_NONE:
            break;
        case S2N_HASH_MD5:
            GUARD_OSSL( EVP_DigestInit_ex( state->digest.high_level.evp.ctx, EVP_md5(), NULL ),
                        S2N_ERR_HASH_INIT_FAILED );
            break;
        case S2N_HASH_SHA1:
            GUARD_OSSL( EVP_DigestInit_ex( state->digest.high_level.evp.ctx, EVP_sha1(), NULL ),
                        S2N_ERR_HASH_INIT_FAILED );
            break;
        case S2N_HASH_SHA224:
            GUARD_OSSL( EVP_DigestInit_ex( state->digest.high_level.evp.ctx, EVP_sha224(), NULL ),
                        S2N_ERR_HASH_INIT_FAILED );
            break;
        case S2N_HASH_SHA256:
            GUARD_OSSL( EVP_DigestInit_ex( state->digest.high_level.evp.ctx, EVP_sha256(), NULL ),
                        S2N_ERR_HASH_INIT_FAILED );
            break;
        case S2N_HASH_SHA384:
            GUARD_OSSL( EVP_DigestInit_ex( state->digest.high_level.evp.ctx, EVP_sha384(), NULL ),
                        S2N_ERR_HASH_INIT_FAILED );
            break;
        case S2N_HASH_SHA512:
            GUARD_OSSL( EVP_DigestInit_ex( state->digest.high_level.evp.ctx, EVP_sha512(), NULL ),
                        S2N_ERR_HASH_INIT_FAILED );
            break;
        case S2N_HASH_MD5_SHA1:
            GUARD_OSSL( EVP_DigestInit_ex( state->digest.high_level.evp.ctx, EVP_sha1(), NULL ),
                        S2N_ERR_HASH_INIT_FAILED );
            GUARD_OSSL( EVP_DigestInit_ex( state->digest.high_level.evp_md5_secondary.ctx, EVP_md5(), NULL ),
                        S2N_ERR_HASH_INIT_FAILED );
            break;
        default:
            S2N_ERROR( S2N_ERR_HASH_INVALID_ALGORITHM );
...
static int s2n_evp_hash_update( struct s2n_hash_state *state, const void *data, uint32_t size )
{
    ...
    switch ( state->alg ) {
        ...
        case S2N_HASH_MD5_SHA1:
            GUARD_OSSL( EVP_DigestUpdate( state->digest.high_level.evp.ctx, data, size ), S2N_ERR_HASH_UPDATE_FAILED );
            GUARD_OSSL( EVP_DigestUpdate( state->digest.high_level.evp_md5_secondary.ctx, data, size ),
...
static int s2n_evp_hash_digest( struct s2n_hash_state *state, void *out, uint32_t size )
{
    ...
    switch ( state->alg ) {
        ...
        case S2N_HASH_SHA384:
        case S2N_HASH_SHA512:
            GUARD_OSSL( EVP_DigestFinal_ex( state->digest.high_level.evp.ctx, out, &digest_size ),
                        S2N_ERR_HASH_DIGEST_FAILED );
            break;
        case S2N_HASH_MD5_SHA1:
            ...
            GUARD_OSSL( EVP_DigestFinal_ex( state->digest.high_level.evp.ctx, ( ( uint8_t * )out ) + MD5_DIGEST_LENGTH,
...
static int s2n_evp_hash_copy( struct s2n_hash_state *to, struct s2n_hash_state *from )
{
    switch ( from->alg ) {
        ...
        case S2N_HASH_SHA384:
        case S2N_HASH_SHA512:
            GUARD_OSSL( EVP_MD_CTX_copy_ex( to->digest.high_level.evp.ctx, from->digest.high_level.evp.ctx ),
        ...
        case S2N_HASH_MD5_SHA1:
            ...
            GUARD_OSSL( EVP_MD_CTX_copy_ex( to->digest.high_level.evp.ctx, from->digest.high_level.evp.ctx ),
                        S2N_ERR_HASH_COPY_FAILED );
            GUARD_OSSL( EVP_MD_CTX_copy_ex( to->digest.high_level.evp_md5_secondary.ctx,
...
static int s2n_evp_hash_reset( struct s2n_hash_state *state )
{
    ...
    if ( ( state->alg == S2N_HASH_MD5 || state->alg == S2N_HASH_MD5_SHA1 )
         && s2n_digest_is_md5_allowed_for_fips( &state->digest.high_level.evp ) ) {
    ...
    if ( reset_md5_for_fips ) { GUARD( s2n_hash_allow_md5_for_fips( state ) ); }
    /* hash_init resets the ready_for_input and currently_in_hash fields */
...
int s2n_hash_init( struct s2n_hash_state *state, s2n_hash_algorithm alg )
{
    ...
    if ( s2n_hash_is_available( alg )
         || ( ( alg == S2N_HASH_MD5 || alg == S2N_HASH_MD5_SHA1 )
              && s2n_digest_is_md5_allowed_for_fips( &state->digest.high_level.evp ) ) ) {
