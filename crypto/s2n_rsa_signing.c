...
static int s2n_hash_alg_to_NID[] = {
    [S2N_HASH_MD5_SHA1] = NID_md5_sha1, [S2N_HASH_SHA1] = NID_sha1,     [S2N_HASH_SHA224] = NID_sha224,
    [S2N_HASH_SHA256] = NID_sha256,     [S2N_HASH_SHA384] = NID_sha384, [S2N_HASH_SHA512] = NID_sha512
};
...
int s2n_rsa_pkcs1v15_sign( const struct s2n_pkey *priv, struct s2n_hash_state *digest, struct s2n_blob *signature )
{
    ...
    GUARD_OSSL( RSA_sign( NID_type, digest_out, digest_length, signature->data, &signature_size, key->rsa ),
                S2N_ERR_SIGN );
...
int s2n_rsa_pkcs1v15_verify( const struct s2n_pkey *pub, struct s2n_hash_state *digest, struct s2n_blob *signature )
{
    ...
    GUARD_OSSL( RSA_verify( digest_NID_type, digest_out, digest_length, signature->data, signature->size, key->rsa ),
...
int s2n_is_rsa_pss_signing_supported() { return RSA_PSS_SIGNING_SUPPORTED; }
...
{
    GUARD_OSSL( EVP_PKEY_CTX_set_signature_md( ctx, ( EVP_MD * )( uintptr_t )digest_alg ),
                S2N_ERR_INVALID_SIGNATURE_ALGORITHM );
    GUARD_OSSL( EVP_PKEY_CTX_set_rsa_mgf1_md( ctx, ( EVP_MD * )( uintptr_t )digest_alg ),
                S2N_ERR_INVALID_SIGNATURE_ALGORITHM );
    return 0;
}

static void s2n_evp_pkey_ctx_free( EVP_PKEY_CTX **ctx ) { EVP_PKEY_CTX_free( *ctx ); }
...
int s2n_rsa_pss_verify( const struct s2n_pkey *pub, struct s2n_hash_state *digest, struct s2n_blob *signature_in )
{
    ...
    GUARD_OSSL( EVP_PKEY_CTX_set_rsa_padding( ctx, RSA_PKCS1_PSS_PADDING ), S2N_ERR_SIGN );
    GUARD( s2n_evp_pkey_ctx_set_rsa_signature_digest( ctx, digest_alg ) );
    GUARD_OSSL( EVP_PKEY_verify( ctx, signature_in->data, signature_in->size, digest_data, digest_length ),
                S2N_ERR_VERIFY_SIGNATURE );
