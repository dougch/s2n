                            ...
                            const struct s2n_blob *info, struct s2n_blob *output )
{
    ...
    if ( output->size % hash_len ) { total_rounds++; }
    ...
    for ( uint32_t curr_round = 1; curr_round <= total_rounds; curr_round++ ) {
        ...
        if ( curr_round != 1 ) { GUARD( s2n_hmac_update( hmac, prev, hash_len ) ); }
        ...
        cat_len = hash_len;
        if ( done_len + hash_len > output->size ) { cat_len = output->size - done_len; }
