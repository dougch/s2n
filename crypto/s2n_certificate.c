...
struct s2n_cert_chain_and_key *s2n_cert_chain_and_key_new( void )
{
    ...
    if ( !chain_and_key->cn_names ) { return NULL; }

    chain_and_key->san_names = s2n_array_new( sizeof( struct s2n_blob ) );
    if ( !chain_and_key->san_names ) { return NULL; }
...
static int s2n_cert_chain_and_key_set_names( struct s2n_cert_chain_and_key *chain_and_key, struct s2n_blob *leaf_bytes )
{
    ...
    if ( !cert ) { S2N_ERROR( S2N_ERR_INVALID_PEM ); }
                                             ...
                                             const struct s2n_blob *              dns_name )
{
    ...
    for ( int i = 0; i < s2n_array_num_elements( san_names ); i++ ) {
        ...
        if ( ( dns_name->size == san_name->size )
             && ( strncasecmp( ( const char * )dns_name->data, ( const char * )san_name->data, dns_name->size )
                  == 0 ) ) {
            return 1;
        }
    }
                                            ...
                                            const struct s2n_blob *              dns_name )
{
    ...
    for ( int i = 0; i < s2n_array_num_elements( cn_names ); i++ ) {
        ...
        if ( ( dns_name->size == cn_name->size )
             && ( strncasecmp( ( const char * )dns_name->data, ( const char * )cn_name->data, dns_name->size )
                  == 0 ) ) {
            return 1;
        }
    }
                                             ...
                                             const struct s2n_blob *              dns_name )
{
    if ( s2n_array_num_elements( chain_and_key->san_names ) > 0 ) {
        if ( s2n_does_cert_san_match_hostname( chain_and_key, dns_name ) ) { return 1; }
    } else {
         ...
         * in a SAN.
         */
        if ( s2n_does_cert_cn_match_hostname( chain_and_key, dns_name ) ) { return 1; }
    }
